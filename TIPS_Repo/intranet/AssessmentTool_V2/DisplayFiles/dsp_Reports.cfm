...
,ts.dtmovein ,ts.dtmoveout ,ts.itenantstatecode_id, ts.iresidencytype_id, ts.ispoints, '' as iresident_id
from tenant t
...
<!--- 	from #application.leadtrackingdbserver#.leadtracking.dbo.resident r
 ...
 --->	join house h on h.ihouse_id = rs.ihouse_id and h.dtrowdeleted is null
	...
	where r.dtrowdeleted is null and ts.itenantstatecode_id = 1 and t.dtrowdelted is null
	<cfif not isDefined("url.mo")> and t.ihouse_id = #session.House.GetId()# <cfelse> and t.ihouse_id = '' </cfif>
