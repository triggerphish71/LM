...
<cfif isDate(getTenantMoveInDate) AND isDate(getResidentmoveInDate)>
	<cfswitch expression="#DateCompare(getTenantMoveInDate,getResidentMoveInDate)#">
		<cfcase value="1">
			<cfset moveinDate = getTenantMoveInDate>
		</cfcase>
		<cfcase value="-1">
			<cfset moveinDate = getResidentMoveInDate>
		</cfcase>
		<cfdefaultcase>
			<cfset moveinDate = getTenantMoveIndate>
		</cfdefaultcase>
	</cfswitch>
<cfelseif !isDate(getTenantMoveInDate) AND !isDate(getResidentMoveInDate)>
	<cfset moveinDate = "">
<cfelseif isDate(getTenantMoveInDate) AND !isDate(getResidentMoveInDate)>
	<cfset moveinDate = getTenantMoveInDate>
<cfelseif !isDate(getTenantMoveInDate) AND isDate(getResidentMoveInDate)>
...
<form name="AssessmentForm" id="form1" action="index.cfm" method="post">
	<input type="hidden" name="moveinDate" id="moveindate" value="#MoveInDate#">
<table width="800">
	<tr>
		<td align="left">
			<table>				
					...
					<td>
						...
						<cfloop from="1" to="#ArrayLen(AssessmentToolArray)#" index="i">				
							...
							<cfelseif NOT isDefined("Assessment")AND AssessmentToolArray[i].GetSLevelTypeSet() eq session.House.GetSLevelTypeSet()>
								<option value="#AssessmentToolArray[i].GetId()#" <cfif resident.getProductLineID() EQ 2>SELECTED</cfif>>#AssessmentToolArray[i].GetDescription()#</option>
							<cfelseif NOT isdefined("Assessment") AND ListFindNoCase(assessmentToolID,AssessmentToolArray[i].getID(),",") EQ 1 AND session.housetype EQ 0>	
								<option value="#AssessmentToolArray[i].GetId()#" <cfif resident.getProductLineID() EQ 1>SELECTED</cfif>>#AssessmentToolArray[i].GetDescription()#</option>
							</cfif>
						</cfloop>
						</select>
					</td>
				...
				<td>
					<select name="reviewType" onChange="ChangeDate(this,'nextReviewDate')">						
						<cfloop from="1" to="#arrayLen(reviewTypeArray)#" index="i">
							...
							<cfelseif (assessmentcount GTE 2) AND ListFindNoCase("1,7",ReviewTypeArray[i].getID(),",") EQ 0>
								<option value="#ReviewTypeArray[i].getID()#" <cfif ReviewTypeArray[i].getID() EQ 5>SELECTED</cfif>>#ReviewTypeArray[i].getDescription()#</option>
							</cfif>
						</cfloop>
					</select>					
					</td>					
 				...
 				<tr> <!--- Modified as part of project 28821 - Jaime Cruz - 10/15/2008 --->
					...
					<td>
					     ...
					     <cfif assessmentType eq "resident" AND assessmentcount LT 1>													
...
<script>
	...
	//commented out the function below because the include files is not called.
	LoadNotes();
</script>
