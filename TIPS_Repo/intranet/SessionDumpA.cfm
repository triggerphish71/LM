...
<cfset u='ldap'>
<cfset p='paulLDAP939'>	
<!---   <br />HouseAccessListP: <cfdump var="#SESSION.HouseAccessList#" label="SESSION.HouseAccessListP">  	
<br /> --->
...
<CFQUERY NAME="qUserHouses"  DATASOURCE="#APPLICATION.datasource#">
    SELECT HouseLog.bIsPDClosed, House.iHouse_ID
		,House.cName AS HouseName 
		,House.cNumber ,House.cCity 
		,House.cStateCode,iPDUser_ID
		,OpsArea.cName AS OpsAreaName 
		,Region.cName AS RegionName 
		,HouseLog.dtCurrentTipsMonth
    FROM House
...
<!--- 	 <br /><cfdump var="#qUserHouses#"   label="qUserHouses"><br />	 ---> 	 
	...
	<cfquery name="useraccount"  datasource="DOCLINKALC">
			...
			WHERE
				cUserName like '%#session.username#%' 
	</cfquery>
 	<cfdump var="#useraccount#" label="DOCLINKALC_useraccount">
	
	<cfquery name="qryFTA" datasource="FTA">
		select * from maple.FTA.dbo.UserAccountAccess where cUserName = '#session.username#'
	</cfquery>
...
<!--- 	<CFLDAP ACTION="query" NAME="FindSubAccount" START="DC=alcco,DC=com" SCOPE="subtree" 
		...
		SERVER="CORPDC01" 
		PORT="389"  
		FILTER="(&(objectCategory=CN=Person,CN=Schema,CN=Configuration,DC=alcco,DC=com)(sAMAccountName=sfarmer))" 
		USERNAME="ldap" 
		PASSWORD="paulLDAP939">
	<cfdump var="#FindSubAccount#" label="FindSubAccount"> --->
	
	<CFLDAP ACTION="query" NAME="FindSubAccountFTAB" START="DC=alcco,DC=com" 
		SCOPE="subtree" 
		ATTRIBUTES="sAMAccountName,Title,DisplayName,physicalDeliveryOfficeName,company,userPrincipalName" 
		SERVER="#ADserver#" 
		PORT="389"  
		...
		USERNAME="ldap" 
		PASSWORD="paulLDAP939">	
	<cfdump var="#FindSubAccountFTAB#" label="FindSubAccountFTAB">
	...
	<cfdump var="#FindSubAccountFTAC#" label="FindSubAccountFTAC">	
		...
		SERVER="#ADserver#" 
		PORT="389"  
		FILTER="(&(objectCategory=CN=Person,CN=Schema,CN=Configuration,DC=alcco,DC=com)(physicalDeliveryOfficeName=#SESSION.EID#) (userPrincipalName=#session.username#@alcco.com))" 
		USERNAME="ldap" 
		PASSWORD="paulLDAP939"> 
...
<cfdump var="#qUserHouses#" label="qUserHouses">
 --->	
 <CFLDAP ACTION="query" 
 NAME="FindSubAccountEID" 
 START="DC=alcco,DC=com" 
 SCOPE="subtree" 
 ATTRIBUTES="physicalDeliveryOfficeName,company,userPrincipalName,sAMAccountName,Title,DisplayName,givenName,sn,cn,dn,member,memberof,Description" 
 SERVER="#ADserver#" 
 PORT="389"  
 FILTER="(&(objectCategory=CN=Person,CN=Schema,CN=Configuration,DC=alcco,DC=com)(physicalDeliveryOfficeName=A*) )" 
 USERNAME="ldap" 
 PASSWORD="paulLDAP939">
<cfdump var="#FindSubAccountEID#" label="FindSubAccountEID"> 
