...
<script>
	...
	// Project 20125 Modification: ssathya 06/26/08 Added phone number as required as per project 20125
	function savebuttoncheck(){ 
		if (df0.ContactList.value.length > 0 && df0.Residents.value.length > 0
			&& df0.cFirstName.value.length > 0 && df0.cLastName.value.length > 0 && df0.cPhoneNumber.value.length > 0
			...
			if (df0.cFirstName.value.length == 0 || df0.cLastName.value.length == 0 ||df0.cPhoneNumber.value.length == 0) { alert('Please make sure you provide the following information.\r First Name, Last Name, Phone Number'); }
	...
	function assessredirect(str) {
		...
		loc="../AssessmentTool_v2/index.cfm?fuse=newAssessment&tenantId="+ str + "&assessmentType=tenant"; self.location.href=loc;
		}
				...
				<tr>
					<td nowrap colspan=100% style='text-align:right;'>
						...
						<span id='residentinformation' style='display:none;'>
							...
							<!--- Project 20125 modification: ssathya 06/26/08 changed phone number it as required as per project 20125 --->
							<a onMouseOver="hoverdesc('xxx-xxx-xxxx');" onMouseOut="resetdesc();"><font style='color:red;'>Phone Number</font></a> <input class="smallfield" type='text' name='cPhoneNumber' value='#ResidentPhone#' style="text-align:center;" size=12 MAXLENGTH=12 onChange="this.value=CreditNumbers(this.value);" onBlur="trimspaces(this); phone(this);"><br>
			...
			<td style='#center# #bottomborder#'>
				<cfif qResidents.iStatus_ID neq '5' and qResidents.iStatus_ID neq '6'>
					...
					<cfif qResidents.iStatus_ID eq '7' and qresidents.dtmovein neq "">
						...
						<cfelse>
							<a style="text-decoration:none;background:##006699;color:yellow;padding: 0 2px 0 2px;" onClick="assessredirect(#trim(qResidents.itenant_id)#,#trim(qResidents.iresident_id)#)">
							<b>Move-in</b>
							</a>
						</cfif>
					</cfif>
