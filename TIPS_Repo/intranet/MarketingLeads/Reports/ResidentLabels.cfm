...
<SCRIPT>report=window.open("loading.htm","ResidentLabelsReport","toolbar=no,resizable=yes");report.moveTo(0,0);</SCRIPT>
<CFSET User="rw">
<CFSET Password="4rwriter">
<CFSCRIPT>if (isDefined("url.scp")) { scope=url.scp; } else { scope=session.qselectedhouse.cname; } </CFSCRIPT>
<form NAME="ResidentLabelsReport" action="//#crserver#/reports/it/resLabels.rpt" method="POST" TARGET="ResidentLabelsReport">
	...
	<SCRIPT>location.href='#HTTP_REFERER#';document.ResidentLabelsReport.submit();</SCRIPT>
	Loading...
</form>
</CFOUTPUT>
