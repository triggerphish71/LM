...
S Farmer     | 02/18/2016 | CustomerTrialBalance replaced with CustomerAccountingActivityReport (PDF)|
S Farmer     | 03/21/2016 | in qResidentTenants changed sorting by adding  "by len(ad.cAptNumber)" |
M shah       | 05/05/2016 | added WI medicaid GL in query TotalCharges                             |
M shah       | 07/25/2016 | Commenetd for missing bond paper work                                  |
S Farmer     | 08/03/2016 | if no rooms - new house - added check for AptcountTotal.TotalAPT       |
|sfarmer     | 2017-05-09 | 'move out date' changed to 'physical move out date'                 |
|MStriegel   | 08/21/2018 | Removed Military VA from the Missing Items query                       |
************************************************************************************************* --->
...
<!--- <cfinclude template="Shared/JavaScript/PageTimer.js">  ---><cfinclude template="Shared/JavaScript/ResrictInput.cfm">



</script>
<cfparam  name="thishouseID" default="">
...
<cfif  IsDefined("URL.SelectedHouse_ID") and url.SelectedHouse_ID neq '' 
  ...
  <cfelseif session.houseaccesslist neq ''>
<!---   yes empty house<br>
  <cfoutput>
  session.houseaccesslist: #session.houseaccesslist#<br>
 first house #listfirst(session.houseaccesslist)#
  </cfoutput> --->
     <cfquery  name="qHouse" datasource="#application.DataSource#">
		select  H.*, HL.*
		from House H 
		join HouseLog HL  on (H.iHouse_ID = HL.iHouse_ID 
			and HL.dtRowDeleted is null 
			and H.dtRowDeleted is null)
		where H.iHouse_ID = #listfirst(session.houseaccesslist)#
	</cfquery>
...
<cfoutput>
	...
	<br>CalcHouse = #qhouse.cNumber#
		...
		< session.userid=#session.userid#
		<br>session.fullname=#session.fullname#
		<cfif isdefined("url.selectedHouse_ID")><br>url.SelectedHouse_ID:: #url.SelectedHouse_ID#</cfif>
		<cfif isdefined("qHouse.cNumber")><br>CalcHouse = #qhouse.cNumber#</cfif>
		<br><cfdump var="#session#">
		</CFMAIL>
</cfoutput>
	</cfcatch>
  ...
  <!--- added 07-21-2015 if house_id didn't come over with URL and there is a session.houseaccesslist use it instead --->
  <cfif isdefined("url.selectedHouse_ID") and  url.SelectedHouse_ID is not ''>
  	<cfset thishouseID = url.SelectedHouse_ID>
<cfelse>
	<cfset thishouseID = listfirst(session.houseaccesslist)>
</cfif>
...
<cfquery name="RDOEmail" datasource="#application.datasource#">
	...
	where h.dtrowdeleted is null and h.ihouse_id = #thishouseID#
</cfquery>
...
<cfquery name="bondhouse" datasource="#application.datasource#">
	select ibondhouse, bBondHouseType,bIsMedicaid,bIsMemoryCare from house  where ihouse_id =  #thishouseID#
</cfquery>
<!--- added bIsMedicaidEligible mamta  --->
<cfquery name="bondapt" datasource="#application.datasource#">
	select a.iaptAddress_ID,a.bIsBond,a.bIsMedicaidEligible,a.bIsMemoryCareEligible from AptAddress a where a.iHouse_ID = #thishouseID#
</cfquery>
...
<cfquery name="qResidentTenants" datasource="#application.datasource#">
	...
	left join houseproductline hpl with (nolock) on hpl.ihouseproductline_id = ad.ihouseproductline_id and hpl.dtrowdeleted is null
	left join productline pl with (nolock) on pl.iproductline_id = hpl.iproductline_id and pl.dtrowdeleted is null
	left outer join	AptType APT with (nolock) on (apt.iAptType_ID = ad.iAptType_ID and apt.dtRowDeleted is null)
	left join	TenantState ts with (nolock) on ad.iAptAddress_ID = ts.iAptAddress_ID and (ts.iTenantStateCode_ID is null or ts.iTenantStateCode_ID = 2 and ts.dtRowDeleted is null)		

	left join Tenant t with (nolock) on (t.iTenant_ID = ts.iTenant_ID)
	left join	ResidencyType RT with (nolock) on (rt.iResidencyType_ID = ts.iResidencyType_ID)
	left join SLevelType ST with (nolock) on (t.cSlevelTypeSet = st.cSlevelTypeSet and ts.iSPoints <= iSPointsMax and ts.iSPoints >= iSPointsMin)
	...
	where	ad.dtRowDeleted is null	and ad.iHouse_ID = #thishouseID#	and t.dtRowDeleted is null
	and (ts.iTenantStateCode_ID is null or ts.iTenantStateCode_ID IN (2)) 
	group by
			cAptNumber,
	...
	<cfif NOT IsDefined("URL.SelectedSortOrder")  or  URL.SelectedSortOrder eq "Apt">
		<cfif #thishouseID# eq 231 or #thishouseID# eq 215>
			order by cAptNumber asc
		<cfelse>
			order by len(ad.cAptNumber), ad.cAptNumber asc
		</cfif>
	...
	<cfelseif URL.SelectedSortOrder eq "Memorycare"> order by AD.bIsMemoryCareEligible desc, ad.cAptNumber, t.cLastName, t.cFirstName 
	</cfif>
</cfquery>
...
<cfquery name="MovedOut" datasource="#application.datasource#">
	...
	,TPS.cDescription as promotions, (arb.currbal + arb.futurebal) as currbal,ts.iResidencyType_ID
	from Tenant t with (nolock)
	join TenantState ts with (nolock) on (t.iTenant_ID = ts.iTenant_ID and ts.dtRowDeleted is null and t.dtRowDeleted is null and ts.iTenantStateCode_ID IN (3,4))
	join ResidencyType RT with (nolock)  on (rt.iResidencyType_ID = ts.iResidencyType_ID and rt.dtRowDeleted is null)
...
<cfquery name="HouseStatus" datasource="#application.datasource#" CACHEDWITHIN="#CreateTimeSpan(0,0,1,0)#">
	...
	where dtRowDeleted is null and iHouse_ID = #thishouseID#
</cfquery>
...
<!--- <cfif listfindNocase(session.codeblock,21) gte 1 and listfindNocase(session.codeblock,23) eq 0>
	<cfquery name="qNoTenantsCheck" datasource="#application.datasource#">
		select t.iTenant_ID from Tenant t with (nolock)
...
<CFINCLUDE TEMPLATE="Shared/HouseSummary.cfm"><br/>
			...
			<cfquery name="AptCountTot" datasource="#APPLICATION.datasource#">
				...
				from AptAddress AA 
				where AA.iHouse_ID = #thishouseID#
			...
			<!--- Count of apt designated as Memorycareeligible-- mamta --->
			<cfquery name="AptcountMemoryCare" datasource="#APPLICATION.datasource#">
				select count(AA.iAptAddress_ID) as MemoryCareEligible 
				from AptAddress AA 
				where AA.bIsMemoryCareEligible = 1
				and AA.dtrowdeleted is null
				and AA.iHouse_ID = #thishouseID#
			</cfquery>
         ...
         <cfif #bondhouse.bIsMemoryCare# eq 1>
		      <cfset memorycarepercent= ((#AptcountMemorycare.MemoryCareEligible#/#AptcountTotal.TotalAPT#)*100)>
		  	  <cfif memorycarepercent LT 20>
				<cfset bhtmlMemoryCare="<font color='FF3300'>#NumberFormat(memorycarepercent, '__.__')#%</font></b>">
			  <cfelse>
				<cfset bhtmlMemoryCare="<font color='7CFC00'>#NumberFormat(memorycarepercent, '__.__')#%</font></b>">
			  </cfif>	
		  </cfif> 
...
<cfif AptcountTotal.TotalAPT is not 0>
	<tr>
	<th>
	...
	<cfif bondhouse.bIsMemorycare eq 1>
	<th nowrap>
	<a href="?SelectedSortOrder=memorycare" style="color: White;" onMouseOver="hoverdesc('Sort By Memorycare Apt');" onMouseOut="resetdesc();"> Memory Care APT<cfoutput>#bhtmlmemorycare#</cfoutput> </a>&nbsp;&nbsp;
	</th> 
	</cfif>
	<cfif bondhouse.ibondhouse eq 1>
	<th nowrap>
	...
	<th>
<a href="?SelectedSortOrder=MoveOut" style="color: White;" onMouseOver="hoverdesc('Sort by Physical Move Out date');" onMouseOut="resetdesc();"> MoveOut </a> 
	</th>
	<th nowrap> 
	...
	<cfquery name="TotalCharges" datasource="#application.datasource#">
		...
		from Tenant t with (nolock)
		join TenantState ts with (nolock) on (ts.iTenant_ID = t.iTenant_ID and ts.dtRowDeleted is null and ts.iTenantStateCode_ID = 2)
		and t.iHouse_ID = #thishouseID#
		join InvoiceMaster IM with (nolock) on im.csolomonkey = t.csolomonkey and IM.dtRowDeleted is null 
		...
		and Ct.cGLAccount <> 3012 and Ct.cGLAccount <> 3016 and Ct.cGLAccount not in ('3091','3092') <!---Mshah added for WI medicaid--->
		join AptAddress AD  on ad.iAptAddress_ID = ts.iAptAddress_ID and ad.dtRowDeleted is null
	...
	<cfoutput query="qResidentTenants">
	 ...
	 <cf_cttr colorOne="FFFFFF" colorTwo="EEEEEE"> 
			...
			<cfif bondhouse.bIsMemoryCare eq 1> 
				<cfif #qResidentTenants.bIsMemoryCareEligible# eq 1>
				<td style="text-align:center"><b>MemoryCare</b></td>
				<cfelse>
				<td style="text-align:center"></td>
				</cfif>
			</cfif>
...
<!--- 			<cfif qResidentTenants.bNRFPend is 1>
			...
			<td style="text-align: right;">
				<cfif thishouseID neq 200>
					<cfif qResidentTenants.iTenant_ID neq "">
					  <a href="Reports/CustomerAccountingActivityReport.cfm?prompt0=#qResidentTenants.cSolomonKey#">#LSCurrencyFormat(qResidentTenants.CurrBal)#</a> 					
 ...
 --->					<cfelse> &nbsp;	
	...
	<cfif qResidentTenants.itenant_id neq "">
			...
			CASE WHEN ts.cMilitaryVA is null THEN 1 ELSE 0 end  +
			     end mstriegel 08/*21/2018
					Mshah commented not show bond as missing item 07/25/2016	
				...
				CASE WHEN isnull(ts.bMoveInSummary,0)= 0 THEN 1 ELSE 0 end as countme  
			<!--- 	CASE WHEN isnull(ts.bContractManagementAgreement,0)= 0 THEN 1 ELSE 0 end as countme --->
				from tenant t with (nolock)
				left join Tenantstate ts with (nolock) on ts.iTenant_ID = t.iTenant_ID and ts.dtrowdeleted is null
...
<cfelse>
<tr><td> No Rooms Have Been Assigned</td></tr>	
</cfif>	
...
<cfoutput query="MovedOut">
		...
		<cf_cttr colorOne="E6E6FA" colorTwo="E6E6F0">
			...
			<td> 
				<cfif MovedOut.iTenantStateCode_ID is 3 and  MovedOut.iresidencytype_id is 3> 
				<cfif (ListContains(session.groupid,'285') gt 0)>
				<a  href="Tenant/RespiteMoveOutCorrection.cfm?tenantid=#movedout.itenant_id#">
				#MovedOut.cFirstName# #MovedOut.cLastName# </a>
				<cfelse>
				<a  href="Tenant/TenantEdit.cfm?id=#movedout.itenant_id#">		
				#MovedOut.cFirstName# #MovedOut.cLastName# </a>
				</cfif>
			<cfelse>
				<a  href="Tenant/TenantEdit.cfm?id=#movedout.itenant_id#">
			...
			or session.username is 'sfarmer'>
				<td>	
...
<!--- 			<td style="text-align: right;">						
			...
			<td style="text-align: right;">						
				<cfif thishouseID neq 200>
					<a href="Reports/CustomerAccountingActivityReport.cfm?prompt0=#MovedOut.cSolomonKey#"> #LSCurrencyFormat(MovedOut.currbal)# </a>
				<cfelseif MovedOut.iTenant_ID neq ""> ZetaTest </cfif>
			</td>
