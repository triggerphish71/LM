...
<BODY>
<cfset variables.MovetoProd = '2011-08-17'>
...
<!---<cfoutput query="GetInvoices" startrow="#GetInvoices.recordcount#">
	<cfif GetInvoices.dtMovein lt variables.MovetoProd and GetInvoices.dtInvoiceEnd lt variables.MovetoProd>
		<cfquery name="UpdateOldInvoice" datasource="#application.datasource#">
			Update InvoiceMaster
			set dtInvoiceEnd = '#GetInvoices.dtMoveOutProjectedDate#'
			where iinvoicemaster_Id = #GetInvoices.iinvoicemaster_Id#
		</cfquery>
	
		 <cfquery name="GetInvoices" datasource="#application.datasource#">
			select iInvoiceNumber, iinvoicemaster_Id ,cAppliesToAcctPeriod, dtmovein, hl.dtCurrentTipsMonth, 
			convert(varchar(10),rim.dtRowStart,101) as dtRowStart,
			bFinalized,mInvoiceTotal,iInvoiceMaster_ID,rim.cSolomonKey,
			convert(varchar(10),rim.dtInvoiceStart,101) as StartDate,
			isnull(convert(varchar(10),rim.dtInvoiceEnd,101),convert(varchar(10),rim.dtInvoiceStart,101)) as EndDate, ts.dtMoveOutProjectedDate
			from InvoiceMaster rim
			join tenant t on (t.cSolomonKey = rim.cSolomonKey)
