...
<cfcomponent displayname="Moveout" Hint="Resident Move out level care calculation">
 ...
 <cffunction access="public" name="getTimeStamp" output="false" retunrtype="query">
  ...
  <cffunction access="public" name="qHouseLog" output="false" returntype="query">
 	<cfargument name="datasource" type="string" required="false" default="#application.datasource#">
 	<cfargument name="iHouseID" type="numeric" required="true">

 	<CFQUERY NAME="qHouseLog" DATASOURCE='#APPLICATION.datasource#'>
		SELECT dtCurrentTipsMonth 
		FROM HouseLog with (nolock)
		WHERE dtrowdeleted is null 
		   and ihouse_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.iHouseID#">
	</CFQUERY>
	<cfreturn qHouseLog>
 </cffunction>
...
<!--- Memory Care SQL statements END --->
 ...
 <cffunction access="public" name="getLatestData" output="false" returntype="query" hint="Tenants previous care care">
	...
	<cfargument name="tenantID" type="numeric" required="false" >
	<cfargument name="prevdatemonth" type="string" required="false" default="">
	<cfargument name="caretype" type="string" required="false" default="">
	...
	<cfquery name="getLatestData" datasource="#arguments.datasource#">
		...
		where bIsRentAdj IS NULL		             
		      and iChargetype_id = 91		      
		      and iTenant_id =  <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.tenantID#">
		<cfswitch expression ="#arguments.caretype#">
		    <cfcase value = "MCAL"> 
		      and cAppliesToAcctPeriod = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.prevdatemonth#"> 
		      and dtrowDeleted IS NOT NULL 
		    order by iInvoicedetail_ID desc
		   </cfcase>
		   <cfcase value="FutureMCAL">
		        and cAppliesToAcctPeriod = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.prevdatemonth#"> 
		        and dtrowDeleted IS NULL 
		        order by iInvoiceMaster_ID ASC
		   </cfcase>
   ...
   <cffunction access="public" name="qZeroCharge" output="false" returntype="query" hint="Gets Tenants result set from the database.">
	 ...
	 <cfargument name="formdata" type="struct" required="true" default="">
		...
		<CFQUERY NAME="qZeroCharge" DATASOURCE="#arguments.datasource#">
			...
			<CFIF #arguments.formdata.iResidencyType_ID# EQ 2> AND cGLAccount = 3011 <CFELSE> AND bIsDaily IS not NULL AND cGLAccount = 3010 </CFIF> 
			<CFIF #arguments.formdata.iResidencyType_ID# EQ 1>AND cDescription not like '%respite%'</CFIF> 
		</CFQUERY>
  ...
  <cffunction name="updatesecondarySwitchDate" access="public" returntype="void" output="true">
	...
	<cfargument name="formdata" type="struct" required="true" default="">
		 ...
		 <CFQUERY NAME="updatesecondarySwitchDate" DATASOURCE="#arguments.datasource#" result="result">
			UPDATE tenantstate
			set dtsecondaryswitchdate = <cfqueryparam cfsqltype="cf_sql_date" value="#arguments.formdata.dtChargeThroughSec#" null="#IIF(len(arguments.formdata.dtChargeThroughSec) EQ 0,'true','false')#"> 
			where itenant_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.formdata.SecondaryTenantId#"> 
		</CFQUERY>
 	</cffunction>  
...
<cffunction name="qValidateCharge" access="public" returntype="query" output="true">
 ...
 <cffunction name="MCSwitchdate" access="public" returntype="query" output="true">
  	<cfargument name="datasource" type="string" required="false" default="#application.datasource#">
	<cfargument name="formdata" type="struct" required="true" default="">
 
		 <CFQUERY NAME="MCSwitchdate" DATASOURCE="#arguments.datasource#">
			SELECT	 top 1 dtRowStart
			FROM	 P_TenantState	with (nolock)					 
			WHERE	 dtMCSwitch is not null					
					 and iTenant_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.formdata.tenantId#">		             
			ORDER BY dtRowEnd asc
		</CFQUERY> 
	 <cfreturn MCSwitchdate> 
	</cffunction>

	<cffunction name="GetHouseClose" access="public" returntype="query" output="true">
  	<cfargument name="datasource" type="string" required="false" default="#application.datasource#">
	<cfargument name="formdata" type="struct" required="true" default="">
 
		 <CFQUERY NAME="GetHouseClose" DATASOURCE="#arguments.datasource#">
			SELECT	 dtActualEffective
			FROM	 HouseLog	with (nolock)					 
			WHERE	iHouse_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.formdata.iHouse_ID#">	
		</CFQUERY> 
	 <cfreturn GetHouseClose> 
	</cffunction>   
