...
<!--- <cfdump var="#form#" label="insert"><cfabort> --->
...
//		if (RentPeriod LT CurrPeriod){ CurrentDaysToCharge=CurrPeriodDays; } else {CurrentDaysToCharge=0;}
//		CareMonthsDifference = DateDiff('m',CurrentMonth,MoveInMonth);
...
<cfscript>
	...
	MonthList=ArrayNew(1); ChargeDays=ArrayNew(1); CARECHARGEDAYS=ArrayNew(1);
	MonthList[1]=dtRent; ChargeDays[1]=RentDaysToCharge;
...
<!--- <cfoutput>
			#RentDaysToCharge# = #RentDaysInMonth#   #variables.RentDay# 
			 #RentPeriod# LT #CurrPeriod#){ #CurrentDaysToCharge# #CurrPeriodDays# 
			#MonthsDifference# =  #CurrentMonth#,#RentMonth# 

			#CareDaysToCharge#  
			 RentPeriod  #CurrentDaysToCharge#=#CurrPeriodDays# #CurrentDaysToCharge#=0;}
			#CareMonthsDifference#  
			
			#CareMonthList#  #CareChargeDays#
			#CareMonthList[1]# #dtMoveIn#; #CareChargeDays[1]# #CareDaysToCharge#;
			
			#MonthList#   #ChargeDays#= 
			MonthList[1]= #dtRent#; ChargeDays[1]=#RentDaysToCharge#;
		
			#Carelimit#=abs(#CareMonthsDifference#)-1;
		   
				#CareMonthList[i+1]#   #MoveInMonth# );  
				#CareChargeDays[i+1]# #DaysInMonth(CareMonthList[i+1])#;
</cfoutput>
<cfabort> --->
...
=============================================================================== --->
  <cfif InvoiceCheck.RecordCount EQ 0 or MoveInInvoiceCheck.RecordCount EQ 0 
    ...
    <cfif qSolBalance.bal GT 0>
      <cfmail type="HTML" from="TIPS4-Message@alcco.com" TO="CFDevelopers@alcco.com" SUBJECT="New Move Insert with PAST SOLOMON BALANCE">
    ...
    <cfif TenantInfo.iResidencyType_ID neq 3>
      ...
      ( iInvoiceNumber ,cSolomonKey ,bMoveInInvoice ,bFinalized ,cAppliesToAcctPeriod ,cComments ,dtAcctStamp ,dtInvoiceStart ,mInvoiceTotal ,mLastInvoiceTotal ,iRowStartUser_ID ,dtRowStart, crowstartuser_id
      )values(
    ...
    update HouseNumberControl set iNextInvoice = #variables.iNewNextInvoice# where dtRowDeleted is null and iHouse_ID = #session.qSelectedHouse.iHouse_ID#
    </cfquery>
    ...
    <cfscript>
		...
		iInvoiceMaster_ID = InvoiceCheck.iInvoiceMaster_ID; iInvoiceNumber = '#InvoiceCheck.iInvoiceNumber#';
	</cfscript>
  </cfif>
  <cfif variables.iInvoiceMaster_ID EQ "">
    <cfmail type="HTML" from="TIPS4-Message@alcco.com" TO="CFDevelopers@alcco.com" SUBJECT="MoveInInsert InvoiceMaster Variable null">
  ...
  <cfif (IsDefined("Tenant.cSLevelTypeSet") and ( Tenant.cSLevelTypeSet neq "") and (Tenant.cSLevelTypeSet neq 0))>
  ...
  <!--- <cfdump var="#SLevel.isleveltype_id#, #slevel.csleveltypeset#,#slevel.ispointsmin#,  #form.iSPoints#, #Tenant.cSLevelTypeSet# ">
<cfabort> --->
...
if (FindOccupancy.RecordCount GT 0){ Occupancy = 2;} else {Occupancy = 1;} 
  ...
  where iTenant_ID = #form.iTenant_ID# and iRowStartUser_ID = 0 and iInvoiceMaster_ID = #iInvoiceMaster_ID#
  </cfquery>
  ...
  <cfif TenantInfo.iResidencyType_ID neq 2 AND TenantInfo.iResidencyType_ID neq 3>
    <cfloop from="1" TO="#ArrayLen(MonthList)#" step="1" index="I">
      ...
      <cfif StandardRent.RecordCount EQ '0'>
        ...
        <cfif TenantInfo.iResidencyType_ID neq 3>
          ...
          and c.iSLevelType_ID = #SLevel.iSLevelType_ID# and ct.bIsDaily is null
        </cfif>
        and c.iResidencyType_ID = #TenantInfo.iResidencyType_ID#
        and c.iHouse_ID = #session.qSelectedHouse.iHouse_ID#
        and ( 
        (iCharge_id = (select iCharge_id from rw.vw_Charges_history where ihouse_id = #session.qSelectedHouse.iHouse_ID# and #MonthList[i]# between dtrowstart and isNull(dtrowend,getdate())
        and iChargeType_ID = c.iChargeType_ID and iAptType_ID = c.iAptType_ID and cSLevelDescription = c.cSLevelDescription and iSLevelType_ID = #SLevel.iSLevelType_ID#)
      ...
      <!--- MLAW 08/17/2006 Add iProductline_ID filter --->
      <!--- 		<cfoutput>Occupancy - #Occupancy# :: iAptType_id - #TenantInfo.iAptType_id# ::iResidencyType_ID - #TenantInfo.iResidencyType_ID# ::iAptType_ID -  #TenantInfo.iAptType_ID# ::MonthList -  #MonthList[i]# ::cChargeSet -  #TenantInfo.cChargeSet# ::iProductLine_ID -  #TenantInfo.iProductLine_ID#</cfoutput>  --->
      ...
      <cfif DailyRent.RecordCount EQ 0>
        <center>
          <strong style='color:red;font-size:20;'> Unable to find a daily rate for this resident.<br/>
          Please verify that there is valid rate entered for this type of aparment, this residency type, and this occupancy position.<br/>
          <a href='../Registration/Registration.cfm'>Click here to continue.</a> </strong>
        </center>
        <CFABORT>
      </cfif>
      ...
      <cfoutput>
        ...
        <cfscript>
				...
				if ((variables.DaysInHouse GTE 30 or (Month(TenantInfo.dtMoveIn) EQ 2 and variables.DaysInHouse EQ DaysInMonth(TenantInfo.dtMoveIn))) and TenantInfo.iResidencyType_ID neq 3)
        ...
        <cfscript>  
				...
				//		ConvertedCareDays=CareChargeDays[i];
					...
					break;
					case 'monthly': 
						if ( TenantInfo.iResidencyType_ID neq 3 and (DaysInMonth(MonthList[i]) - Day(TenantInfo.dtMoveIn) +1 GTE 30 or ChargeDays[i] GTE 30) ) { 
					...
					break;
					default: ConvertedDays=''; ConvertedDescription=''; ConvertedChargeType=''; ConvertedAmount=''; ConvertedCareAmount=''; ConvertedCareChargeType=''; ConvertedCareDescription=''; ;
				}
			</cfscript>
		<cfelse>
        <cfscript>
      ...
      values
      ( #variables.iInvoiceMaster_ID# ,#TenantInfo.iTenant_ID# ,#ConvertedChargeType# ,'#DateFormat(MonthList[i],'yyyymm')#' ,NULL ,getdate() 
      ,#ConvertedDays# ,'#ConvertedDescription#' ,#ConvertedAmount# ,'RENT' ,#CreateODBCDateTime(session.AcctStamp)# ,0 ,getdate() )
      </cfquery>
      <cfif  (SLevel.iSPointsMin neq '0' and SLevel.iSPointsMax neq '0' and tenant.iproductline_id neq 2) 
			and ((IsDefined('qResidentCare.RecordCount') is "yes") and (IsDefined('qDailyCare.RecordCount') is "yes") 
			and ((Tenant.cBillingType is 'M' and qResidentCare.RecordCount GT 0) or (Tenant.cBillingType is 'D' 	
      ...
      <!--- <cfoutput>outside if<cfdump label="outside if" var="#SLevel.iSPointsMin#  :: #SLevel.iSPointsMax#  ::  #tenant.iproductline_id#  ::
#SLevel.iSPointsMin#  :: #SLevel.iSPointsMax#  ::  #tenant.iproductline_id#   ::
#qResidentCare.RecordCount# :: #qDailyCare.RecordCount#  ::
#Tenant.cBillingType#   ::  #qResidentCare.RecordCount#  :: #Tenant.cBillingType# 	 ::
#qDailyCare.RecordCount#"  > 	
<cfdump label="outside ConvertedDays" var="#ConvertedDays# ::
							#ConvertedDescription#  ::
							#ConvertedChargeType#   ::
							#ConvertedAmount#  ::
							#ConvertedCareAmount#  ::
							#ConvertedCareChargeType#  ::
							#ConvertedCareDescription#  "></cfoutput>
<cfabort showerror="outside ConvertedDays"> --->
      <!--- 35227 RTS - AUTO APPLY RECURRING CHARGES  --->
      <!---  <cfdump label="RECURRING CHARGES" var="#TenantInfo.iResidencyType_ID#, #Occupancy#,#GetMIAutoApplyCharges.iChargeType_ID#"> --->
          ...
          <cfloop query="GetMIAutoApplyCharges">
            <cfif GetMIAutoApplyCharges.iChargeType_ID neq 69>
              ...
              <cfif CheckforRecurringCharge.RecordCount lt 1>
                ...
                <!--- <cfif #Rstamp# lt 1115>, '12-31-#ThisYear#'<cfelse>, '12-31-#NextYear#'</cfif> --->
                ,'12/31/2020'
                <!--- end 53409 --->
              ...
              <!--- <cfdump var="#CheckforInvDtlRecord#"> --->
              ...
              <cfif GetMIAutoApplyCharges.iChargeType_ID is 91>
				...
				, #session.userid# 
                	,getdate() 
                ,
                <cfif ((IsDefined("GetNewRecurringforInvoice.iRecurringCharge_ID") is "yes") and (GetNewRecurringforInvoice.iRecurringCharge_ID is not ""))>
  ...
  <cfoutput>
    <cfloop INDEX="Field" LIST="#form.fieldnames#" DELIMITERS=",">
      <cfif FindNoCase('Deposit', Field, 1) GT 0>
        ...
        ,getDate() ,1 ,'#qgetDepositCharge.cDescription#' ,#qgetDepositCharge.mAmount# ,'Deposit' ,#CreateODBCDateTime(session.AcctStamp)# ,0 ,getDate() )
        </cfquery>
      </cfif>
    </CFLOOP>
  </cfoutput>
  <cfif TenantInfo.iResidencyType_ID EQ 2>
    ...
    <cfif qRecordCheck.RecordCount EQ 0 or qRecordCheck.RecordCount EQ "" or qRecordCheck.count EQ 0>
      ...
      where ct.dtRowDeleted is null and c.dtRowDeleted is null and ct.bIsMedicaid is not null and ct.bIsDaily is null and cGLAccount = 3011
      </cfquery>
      ...
      values
      ( #variables.iInvoiceMaster_ID# ,#TenantInfo.iTenant_ID# ,#qMedZeroCharge.iChargeType_ID# ,'#MoveInPeriod#',NULL ,getDate() 
      ,1 ,'#qMedZeroCharge.cDescription#' ,0.00 ,'Mediciad' ,#CreateODBCDateTime(session.AcctStamp)# ,0 ,getDate() )
      </cfquery>
    </cfif>
  </cfif>
  ...
  <cfif form.cFirstName neq "" and form.cLastName neq "">
    ...
    where cFirstname = '#form.cFirstName#' and cLastName = '#form.cLastName#' and cCity = '#form.cCity#' 
    ...
    <cfif ContactID.iContact_ID neq "">
      <cfif LinkTenantContactID.iContact_id eq "">
        ...
        ( iTenant_ID ,iContact_ID ,iRelationshipType_ID ,bIsPayer ,bIsPowerOfAttorney ,bIsExecutorContact,bIsMedicalProvider
        ,cPrimaryCarePhysicianContact,cComments ,iRowStartUser_ID ,dtRowStart,bIsGuarantorAgreement
        ) values (
        ...
        <cfif IsDefined("form.ContactbIsPayer")>
          #form.ContactbIsPayer#,
          <cfelse>
          null,
        </cfif>
  ...
  <!--- if the contact is not the Guarentor the tenant is the primary payer if the contact is the Guarentor than the contact is the primary payer --->
  <!---  --->
  <!---  --->
  <!---  --->
  ...
  <!--- create invoicedetail for the 1740charge --->
  ...
  <!--- create the invoicedetail for the 1741 charge --->
  <cfoutput>
    <CFQUERY NAME = "InvoiceDetail" DATASOURCE = "#APPLICATION.datasource#">
    INSERT INTO 	InvoiceDetail
    ( iInvoiceMaster_ID ,iTenant_ID ,iChargeType_ID ,cAppliesToAcctPeriod ,bIsRentAdj ,dtTransaction ,iQuantity ,cDescription
    ...
    VALUES 
    ( #MoveInInvoice.iInvoiceMaster_ID#, #form.iTenant_ID#,
    1741, #thisacctmonth#,
    1, #todaysdate#,
    1,
    '#queryChargeID1741.cDescription#', #thisamount#,	
    NULL, #CreateODBCDateTime(SESSION.AcctStamp)#,
    0,
    <!--- #SESSION.UserID#, --->
...
#formatenddate#
  ...
  <cfoutput>
    ...
    VALUES
    (#form.iTenant_ID# ,#queryChargeID.chargeID# ,#datestart# ,#dateend# ,#iQuantity# ,'#cDescription#'
    ,#mamount# ,
    <cfif IsDefined('cComments')>
      '#cComments#'
      <cfelse>
      null
    </cfif>
    ,#CreateODBCDateTime(SESSION.AcctStamp)# ,#SESSION.UserID# ,#todaysdate# ,0
    )
    </CFQUERY>
  </cfoutput> <cfoutput>
    ...
    VALUES
    (#form.iTenant_ID# ,#queryChargeID1741.chargeID# ,#datestart# ,#dateend# ,#iQuantity# ,'#queryChargeID1741.cDescription#'
    ,#thisamount# ,
    <cfif IsDefined('cComments')>
      '#cComments#'
      <cfelse>
    ...
    where  iTenant_ID = #form.iTenant_ID# and iCharge_ID = #queryChargeID1741.chargeID#
    </cfquery>
    <cfquery name="updIMD" DATASOURCE = "#APPLICATION.datasource#">
    update dbo.InvoiceDetail
    set iRecurringCharge_ID = #qryRCID.maxRCID# where iInvoiceDetail_ID = #qryIMID.maxIMID#
    </cfquery>
  </cfoutput>
  </cfif>
  </cfif>
