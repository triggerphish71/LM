...
<!--- ==============================================================================
Change the Cycle ID to Moved In in SL. (dbo.customer)
=============================================================================== --->
<CFQUERY NAME="UpdateCustCycleID" datasource = "#APPLICATION.datasource#">
	Select * from krishna.houses_app.dbo.customer
	<!---UPDATE	krishna.houses_app.dbo.customer
	SET	--->				
	WHERE	CustID = '#TenantInfo.cSolomonKey#'
</CFQUERY>
<cfdump var='#UpdateCustCycleID#'>
<!--- ==============================================================================
	...
	<CFQUERY NAME = "StandardRent" DATASOURCE = "#APPLICATION.datasource#">
		<CFIF TenantInfo.iResidencyType_ID NEQ 2>
			<CFIF Occupancy EQ 1>
				...
				order by c.cChargeSet desc <!---Mshah added here for 08/09/2017--->
			<CFELSE>
				SELECT	C.cDescription ,C.mAmount ,C.iQuantity ,CT.iChargeType_ID
				FROM	Charges C
				JOIN	ChargeType CT ON (CT.iChargeType_ID = C.iChargeType_ID 
				...
				<!--- AND	CT.bAptType_ID IS NOT NULL --->
				<!---AND CT.bIsDaily IS NULL--->
				<CFIF #TenantInfo.cbillingtype# EQ 'D'> <!---mshah added here--->
				AND	CT.bIsDaily IS not NULL 
				<cfelse>
				AND CT.bIsDaily IS NULL
				</cfif>
...
<!--- 	<br /> StandardRent2 --->
		...
		<CFQUERY NAME = "StandardRent" DATASOURCE = "#APPLICATION.datasource#">
			...
			<CFIF TenantInfo.iResidencyType_ID NEQ 3>
				...
				<!---AND		CT.bIsDaily IS NULL--->
				<CFIF #TenantInfo.cbillingtype# EQ 'D'> <!---Mshah added here for 08/09/2017--->
				AND	CT.bIsDaily IS not NULL 
				<cfelse>
				AND CT.bIsDaily IS NULL
				</cfif>
...
<!--- 	<cfoutput>
	...
	<!--- <cfoutput>moveinperiod #moveinperiod# #datepart('m', TenantInfo.dtMoveIn)#, cAppliesToAcctPeriod #cAppliesToAcctPeriod#, vQuantity- #vQuantity#</cfoutput>--->
...
<cftransaction>
		...
		<!---<cfdump var="#contactinfo#">--->
				...
				<CFQUERY NAME='qPayorImport' DATASOURCE='HOUSES_APP' result="qCustImport">
...
<cfoutput>  '#TenantInfo.csolomonkey#', '#contactinfo.Name#', '#contactinfo.AddressLine1#','#contactinfo.AddressLine2#','#contactinfo.City#','#contactinfo.State#','#contactinfo.Zip#','#contactinfo.Phone#','#Session.username#'
		  </cfoutput>
		  <cfdump var="#qPayorImport#">
	<cfabort>
	</cfcatch>
