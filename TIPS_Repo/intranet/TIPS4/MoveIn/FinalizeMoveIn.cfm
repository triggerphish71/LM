...
|            |            |and display page                                                    |
|Mshah	      | 05/12/2017| Added payor update code for SL                                     |
| Mshah	      | 06/12/2017| Added  code for Second Occupant incorrect invoice 1682 fix         |
|MStriegel   | 11/08/2017 | Autorelease = 1 and changed to call component                      |
|MStriegel   | 04/25/2018 | Replaces session.qSelectHouse.HOuseID with tenantInfo.iHouse_ID    |
 -----------------------------------------------------------------------------------------  --->
...
<!--- 		<CFQUERY NAME="MovedInState" datasource = "#APPLICATION.datasource#">
	...
	<CFQUERY NAME="qTipsMonth" DATASOURCE='#APPLICATION.datasource#'>
		select * from houselog WITH (NOLOCK) where dtrowdeleted is null and ihouse_id = #SESSION.qSelectedHouse.iHouse_id#
	</CFQUERY>
...
<CFQUERY NAME="GetEmail" DATASOURCE="#APPLICATION.datasource#">
	...
	JOIN 	#Application.AlcWebDBServer#.ALCWEB.dbo.employees DU WITH (NOLOCK)
...
<!--- <cfdump var="#MoveInInvoice#"> --->
	...
	<cfif TenantInfo.iResidencyType_ID EQ 2>
		<cfquery name="qryHouseMedicaid" datasource="#application.datasource#">
			select * from HouseMedicaid WITH (NOLOCK) where ihouse_id = #tenantinfo.ihouse_id#
		</cfquery>
	</cfif>
...
<CFLOOP QUERY='qMIDetails'>
	...
	<CFQUERY NAME='qChargeID' DATASOURCE="#APPLICATION.datasource#">
		...
		and ihouse_id = #TenantInfo.iHouse_ID#
		and cchargeset 
...
<!--- 		<br />
		...
		<CFQUERY NAME='qChargeID' DATASOURCE="#APPLICATION.datasource#">			
			...
			from tenant t WITH (NOLOCK)
			join tenantstate ts WITH (NOLOCK) on t.itenant_id  = ts.itenant_id
			join dbo.AptAddress aa  WITH (NOLOCK) on ts.iAptAddress_ID = aa.iAptAddress_ID
			join charges chg WITH (NOLOCK) on chg.ihouse_id = t.ihouse_id
			where
	...
	<cfelse>RoomOccupancy.RecordCount = 1<br />
		<CFQUERY NAME='qChargeID' DATASOURCE="#APPLICATION.datasource#">			
			...
			join tenantstate ts WITH (NOLOCK) on t.itenant_id  = ts.itenant_id
			join dbo.AptAddress aa WITH (NOLOCK) on ts.iAptAddress_ID = aa.iAptAddress_ID
			join charges chg WITH (NOLOCK) on chg.iAptType_ID = aa.iAptType_ID
			where
			chg.dtrowdeleted is null
			and 	chg.ichargetype_id = 89
			and 	chg.ihouse_id = t.ihouse_id
...
<CFQUERY NAME = "FindOccupancy" DATASOURCE = "#APPLICATION.datasource#">
	...
	JOIN 	SLevelType ST WITH (NOLOCK)	ON (ST.cSLevelTypeSet = T.cSLevelTypeSet AND TS.iSPoints between ST.iSPointsMin and ST.iSPointsMax)
	JOIN    Apttype at WITH (NOLOCK) on AD.iApttype_ID= at.iApttype_ID and (at.biscompanionsuite is null or at.biscompanionsuite=0) <!---Mshah added for not charging CF for companion--->
	WHERE	AD.dtRowDeleted IS NULL
		AND		T.iHouse_ID = #TenantInfo.iHouse_ID#
...
<CFQUERY NAME="MoveInInfo" DATASOURCE="#APPLICATION.datasource#">
	select * from InvoiceMaster WITH (NOLOCK)
...
<!--- 	<cfoutput>
	...
	<!--- mstriegel 04/25/2018 --->
		...
		<CFQUERY NAME = "GetNextInvoice" DATASOURCE = "#APPLICATION.datasource#">
			...
			WHERE	iHouse_ID = #TenantInfo.iHouse_ID#
			AND		dtRowDeleted IS NULL
		</CFQUERY>
	
		<CFSCRIPT>
			HouseNumber = TenantInfo.cNumber;
	...
	<CFQUERY NAME = "StandardRent" DATASOURCE = "#APPLICATION.datasource#">
		<CFIF TenantInfo.iResidencyType_ID NEQ 2>
			<CFIF Occupancy EQ 1>
				...
				AND C.iHouse_ID = #TenantInfo.iHouse_ID#
				...
				and c.cChargeset= '#TenantInfo.cChargeSet#' <!---Mshah added here for 1682--->
			<CFELSE>
				SELECT	C.cDescription ,C.mAmount ,C.iQuantity ,CT.iChargeType_ID
				FROM	Charges C WITH (NOLOCK)
				JOIN	ChargeType CT WITH (NOLOCK) ON (CT.iChargeType_ID = C.iChargeType_ID 
				...
				AND CT.bIsMedicaid IS NULL
				<!--- AND	CT.bAptType_ID IS NOT NULL --->
				<!---AND CT.bIsDaily IS NULL Mshah 1682--->
...
<!--- 	<br /> StandardRent2 --->
		...
		<CFQUERY NAME = "StandardRent" DATASOURCE = "#APPLICATION.datasource#">
			...
			<CFIF TenantInfo.iResidencyType_ID NEQ 3>
				...
				<!---AND		CT.bIsDaily IS NULL--->
				<CFIF #TenantInfo.cbillingtype# EQ 'D'> <!---Mshah added here for 1682--->
				AND	CT.bIsDaily IS not NULL 
				<cfelse>
				AND CT.bIsDaily IS NULL
				</cfif>
...
<!--- mstriegel 04/25/2018 --->
		<CFMAIL TYPE ="HTML" FROM = "TIPS4-Message@alcco.com" TO = "#SESSION.AREmail#" 
			...
			SUBJECT = "Standard Rate Missing for #TenantInfo.HouseName#">
			#SESSION.FullName# has attempted a Move In, 
			however the Standard Rate for #TenantInfo.HouseName#,
          ...
          <CFSCRIPT>
				Msg1 = "Standard Rate Missing for #TenantInfo.HouseName#.<BR>";
...
<!--- 		<CFQUERY NAME="DailyRent" DATASOURCE="#APPLICATION.datasource#"> --->
		<CFIF Occupancy EQ 1><!--- DailyRent E --->
			...
			<CFQUERY NAME="DailyRent" DATASOURCE="#APPLICATION.datasource#">
				...
				FROM	Charges	C WITH (NOLOCK)
				JOIN 	ResidencyTYPE RT WITH (NOLOCK)			ON C.iResidencyType_ID = RT.iResidencyType_ID
				LEFT OUTER JOIN	SLevelType ST WITH (NOLOCK)	ON C.iSLevelType_ID = ST.iSLevelType_ID
				JOIN 	ChargeType CT	WITH (nolock)			ON CT.iChargeType_ID = C.iChargeType_ID
				WHERE	C.dtRowDeleted IS NULL
				AND		CT.dtRowDeleted IS NULL
				<CFIF TenantInfo.cChargeSet NEQ ""> 
...
<!--- 		</CFQUERY> --->
	...
	<CFQUERY NAME='qResidentCare' DATASOURCE="#APPLICATION.datasource#">
		...
		AND 	CT.bIsRentAdjustment IS NULL
		WHERE	C.dtRowDeleted IS NULL AND C.iHouse_ID = #TenantInfo.iHouse_ID#
		<CFIF TenantInfo.cChargeSet NEQ ''>
			AND C.cChargeSet = '#TenantInfo.cChargeSet#'
		<CFELSE>
			AND C.cChargeSet IS NULL
	...
	TenantInfo.iTenant_ID	  #TenantInfo.iTenant_ID#
		...
		<cfif tenantinfo.iresidencytype_id is not 2>
			<cfquery name="GetRecurringRBChargeID" datasource="#application.datasource#">	
				...
				from RecurringCharge r WITH (NOLOCK)
				join Charges c WITH (NOLOCK) on (c.iCharge_ID = r.iCharge_ID
		...
		<!---<cfoutput> <br /> Automatic Next Month Daily Base Rate #Variables.iInvoiceMaster_ID# ::
		 ...
		 TenantInfo.cBillingType #TenantInfo.cBillingType#
		 			...
		 			<cfif standardrent.recordcount gt 0>
						...
						<cfelse>
							<!---<cfset mamont = #TenantInfo.mBSFOrig#> Mshah 1682 --->
							<cfset mamont = #standardrent.mAmount#>
						</cfif>
...
<!--- 			<cfoutput>yes std rent 
	...
	<!--- 35227 RTS 2/2/2010 - Get all other recurring charges created during MI for insert --->
		<cfquery name="CheckforExistingRecurringCharge" datasource="#application.datasource#">
			...
			join Charges c WITH (NOLOCK) on (c.iCharge_ID = rc.iCharge_ID)
				...
				#CheckforExistingRecurringCharge.mAmount# ::
				 ...
				 #CheckforExistingRecurringCharge.iRecurringCharge_ID#
					...
					<cfif (CheckforExistingRecurringCharge.ichargetype_id is 31) >
						<cfquery name="qryMediaidRate" dbtype="query"  >
							select mamount,irecurringcharge_id 
							from GetRecurringRBChargeID
							where  ichargetype_id = 31
						</cfquery>
						<cfset iquant =	1>
						<cfset mamont = #qryMediaidRate.mamount# >	
...
<!--- 					<cfelseif IsDefined('GetRecurringRBChargeID.iRecurringCharge_ID')and 
	...
	AND qResidentCare.RecordCount GT 0 and #moveinperiod# LTE #cAppliesToAcctPeriod# >
		...
		<CFQUERY NAME="InsertCare" DATASOURCE="#APPLICATION.datasource#" result ="InsertCare1">
			INSERT INTO InvoiceDetail
			( iInvoiceMaster_ID 
			,iTenant_ID 
			,iChargeType_ID 
...
<CFIF (TenantInfo.ihouse_id NEQ 200)>

<!--- ==============================================================================
	Generate MoveIn Solomon Batch
	=============================================================================== --->
	<!--- mstriegel:01/18/2018 --->
	<cfset argsCollection = {iHouseID=#trim(TenantInfo.iHouse_ID)#,period="",invoice=#trim(MoveInInvoice.iInvoiceNumber)#,batchType="movein",tenantID=#TenantInfo.iTenant_ID#}>	
	...
	<CFCATCH type="Any">
		...
		TO="mstriegel@enlivant.com;CFDevelopers@enlivant.com" 
	...
	<cfquery name="getSLDetails" datasource='HOUSES_APP'>
		select name, SlsSub from customer WITH (NOLOCK) where custID = '#TenantInfo.csolomonkey#'
	</cfquery>
...
<!---Mshah added code for updating payor information/ move in finalization will update payor info in SL --->
		<!--- Retreive contact information --->
	
<cftry>

		<cfquery name="contactinfo" datasource="#application.datasource#">
			select t.cfirstname+ ' ' + t.cLastName as name, coalesce(h.cAddressLine1,'') as AddressLine1 ,coalesce(h.cAddressLine2,'') as AddressLine2,
			coalesce(h.cCity,'') as city, coalesce(h.cStateCode,'') as state, coalesce(h.cZipCode,'') as Zip, coalesce(h.cPhoneNumber1,'') as Phone
			from tenant t with(nolock)
			join house h with(nolock) on h.iHouse_ID=t.iHouse_ID
		...
		<!---if ltc is not payer--->
		<cfif contactinfo.recordcount eq 0>
		<cfquery name="contactinfo" datasource="#application.datasource#">
			select c.cfirstname+ ' ' + c.cLastName as name, coalesce(c.cAddressLine1,'') as AddressLine1 ,coalesce(c.cAddressLine2,'') as AddressLine2,
			 coalesce(c.cCity,'') as city, coalesce(c.cStateCode,'') as state, coalesce(c.cZipCode,'') as Zip, coalesce(c.cPhoneNumber1,'') as Phone
			from tenant T with(nolock)
			...
			and ltc.bispayer = 1
				...
				<CFQUERY NAME="qPayorImport" DATASOURCE="HOUSES_APP" result="qPayorImport">
					 exec [dbo].[TSP_TenantPayorUpdate]   
							 '#TenantInfo.csolomonkey#', '#contactinfo.Name#', '#contactinfo.AddressLine1#','#contactinfo.AddressLine2#','#contactinfo.City#','#contactinfo.State#','#contactinfo.Zip#','#contactinfo.Phone#','#Session.username#'
				</cfquery>
				
		
	<cfcatch type="any">
	Update of Payor in solomon failed.
	<cfdump var="#cfcatch#">
	</cfcatch>

</cftry>

<!---Mshah end for update payor info --->
...
<CF_MoveIn iHouse_ID=#TenantInfo.iHouse_ID# TipsMonth=#DateFormat(SESSION.TipsMonth,"yyyy-mm-dd")# iTenant_ID=#TenantInfo.iTenant_ID#>
...
<cfif TenantInfo.ihouse_id neq 200>
	<CFIF TenantInfo.iResidencyType_id EQ 2>
		<CFSCRIPT>
			medmi = "A new tenant has been moved into #TenantInfo.HouseName# by #SESSION.FullName#.<BR>";
	...
	<CFELSE>
		<CFSCRIPT>
				newmi = "A new tenant has been moved into #TenantInfo.HouseName# by #SESSION.FullName#.<BR>";
			...
			<cfif LEN(getEmail.AREmail) GT 0>
				<cfset local.toEmail = getEmail.AREmail>
			<cfelse>
				<cfset local.toEMail = 'cfdeveloper@enlivant.com'>
			</cfif>
			<CFMAIL TYPE="HTML" 
				FROM="TIPS4-Message@alcco.com" 
				TO="#local.toEmail#" 
