...
| MStriegel  | 12/18/2017 | Added logic for penny off issue. Added WITH (NOLOCK), removed dumps  |
 ------------------------------------------------------------------------------------------------->
<CFOUTPUT>
	...
	<cfelse>
		<CFQUERY NAME="InvoiceNumber" DATASOURCE = "#APPLICATION.datasource#">
			...
			and IM.iInvoiceMaster_ID = (select max(im2.iInvoiceMaster_ID)
									from InvoiceMaster im2 WITH (NOLOCK)
	...
	<CFIF InvoiceNumber.RecordCount EQ 0 and resType neq 3>
		<CFTRANSACTION>
			...
			<CFQUERY NAME = "LastTotal" DATASOURCE = "#APPLICATION.datasource#">
				SELECT	mInvoiceTotal as InvoiceTotal FROM InvoiceMaster WITH (NOLOCK)
			...
			<CFQUERY NAME = "WriteInvoiceMaster" DATASOURCE ="#APPLICATION.datasource#">
				...
				join invoicemaster im WITH (NOLOCK) on im.iinvoicemaster_id = inv.iinvoicemaster_id and im.dtrowdeleted is null
					and inv.dtrowdeleted is null
			...
			<CFQUERY NAME = "InvoiceNumber" DATASOURCE = "#APPLICATION.datasource#">
				SELECT * FROM InvoiceMaster IM WITH (NOLOCK) WHERE cSolomonKey = '#Tenant.cSolomonKey#'
				AND	IM.dtRowDeleted IS NULL AND bMoveInInvoice IS NULL AND bMoveOutInvoice IS NULL AND bFinalized IS NULL
			</CFQUERY>
		</CFTRANSACTION>
	<CFELSE>
		...
		<CFELSEIF NOT IsDefined("url.id")>
			...
			<TD STYLE="width: 25%;">#Tenant.cLastName#, #Tenant.cFirstname#<INPUT TYPE="Hidden" NAME="iTenant_ID" VALUE="#Tenant.iTenant_ID#"></TD>
				...
				<cfelse>
					<CFIF Charge.bIsModifiableAmount GT 0 OR (listfindNocase(session.codeblock,25) GTE 1 OR listfindNocase(session.codeblock,23) GTE 1)><!--- Sr. Acct or AR --->
						<TD>Amount
						<INPUT TYPE="text" NAME="mAmount" SIZE="7" STYLE="text-align:center;" VALUE="1000.00"  onBlur="this.value=Money(this.value); this.value=cent(round(this.value));" onkeypress="return numbers(event)"></TD>
					<CFELSE>
						<TD><U>Amount:</U> <BR> #LSCurrencyFormat(Charge.mAmount)# <INPUT TYPE="Hidden" NAME="mAmount" VALUE="#Charge.mAmount#"></TD>
					</CFIF>
				...
				<CFIF (IsDefined("Charge.bISModifiableAmount") AND Charge.bISModifiableAmount GT 0) OR TenantDetail.bIsMedicaid GT 0>
					<TD>
						...
						<Cfquery name="getRecurring" datasource="#application.datasource#">
							SELECT bIsDaily, mAmount from recurringcharge WITH (NOLOCK) WHERE iTenant_ID = #Tenant.iTenant_ID# AND iCharge_ID = 86 AND dtRowDeleted IS NULL
...
<CFQUERY NAME="qRecords" DATASOURCE="#APPLICATION.datasource#">
	SELECT	(T.cLastName + ', ' + T.cFirstName) as fullname
		...
		,CT.iChargeType_ID
		,CT.cGLAccount
		<!--- mstriegel 12/15/2017 added ROUND to resolve the penny off issue --->
		,ROUND(isNull(INV.mAMount,0.00),2) as mAmount
		,Sum(ROUND(isNull(INV.mAMount,0.00),2) * ROUND(isNull(INV.iQuantity,1),2)) as ExtendedPrice
		<!--- end mstriegel --->
		...
		,(Select currbal+futurebal from #application.houses_appdbserver#.houses_app.dbo.ar_balances where custid=T.cSolomonKey and cpnyID=h.cCompanyID) <!---Mshah added--->
		<cfelse> ,0 </cfif> as currbal
	FROM	Tenant T (NOLOCK)
	JOIN 	House h (NOLOCK) on h.ihouse_ID=t.ihouse_ID
	...
	GROUP BY T.cLastName
		...
		,INV.mAmount ,INV.iQuantity ,IM.iInvoiceMaster_ID ,INV.iInvoiceDetail_ID ,T.cSolomonKey,h.cCompanyID
