...
<SCRIPT LANGUAGE="JavaScript">
  function validatedate()
  {
	   ...
	   if (AppliesDate > TipsDate)
	   {
		...
		//alert (AppliesDate);
	    //alert (TipsDate);
	  if(document.getElementsByName('AppliesToMonth')[0].value == '')
  	   {alert('Enter the month value 01 - 12');
		return false;}
	  if(document.getElementsByName('AppliesToYear')[0].value == '')
  	   {alert('select the year');
		return false;}
		//alert (document.getElementById('iQuantity').value);
	  if(document.getElementById('iQuantity').value == 0|| document.getElementById('iQuantity').value == '')
  	   {alert('Enter quantity/ number of days');
		return false;}
     }
  ...
  {	//alert ('test');	
	   var monthDropDown = document.getElementsByName('AppliesToMonth')[0];  
	    //alert (monthDropDown.value);
	   var yearDropDown = document.getElementsByName('AppliesToYear')[0];  
	    //alert (yearDropDown.value);
	   var numberofday= new Date(yearDropDown.value, monthDropDown.value, 0).getDate();
		var crdr = document.getElementsByName("crdr");
		var crdrselected;

		for(var i = 0; i < crdr.length; i++) {
		   if(crdr[i].checked)
			   crdrselected = crdr[i].value;
		 }
	   //alert (numberofday);
	   Totalamount = document.getElementById('HmAmount').value;
	  // alert (Totalamount);
	   NewAmount= (Totalamount/numberofday)*(crdrselected);
	   //alert (NewAmount);
	   document.getElementById('mAmount').value= round(NewAmount,2);
...
=============================================================================== --->
	...
	<!---Mshah added query for renteffctive date--->
	<CFQUERY NAME="TenantDetailRentEffective" DATASOURCE = "#APPLICATION.datasource#">
		Select * from tenantstate ts join tenant t on t.itenant_ID=ts.itenant_ID 
...
<CFIF IsDefined("form.iCharge_ID") OR IsDefined("url.detail")>
	...
	Retrieve specific Charge information
	=============================================================================== --->
	<!---Mshah added here if the form has monthly chargetype the show resident recurringcharge on charge box--->
	
		<CFQUERY NAME="recurringcharge" DATASOURCE =  "#APPLICATION.datasource#">
			SELECT	C.*, CT.*, C.cDescription as ChargeDescription, rc.mamount as recurringamount
			FROM 	Charges C (NOLOCK)
			JOIN 	ChargeType CT (NOLOCK)	ON CT.iChargeType_ID = C.iChargeType_ID
			JOIN    Recurringcharge RC (NOLOCK) ON RC.icharge_ID=c.icharge_ID and rc.dtrowdeleted is null
			WHERE	
			C.iCharge_ID = <CFIF IsDefined("form.iCharge_ID")>	#form.iCharge_ID# 
							<CFELSEIF url.Detail NEQ ""> #url.detail# <CFELSE> 0 </CFIF>
			<CFIF IsDefined("form.itenant_ID")>
			and rc.itenant_ID = #form.itenant_ID# 
			</CFIF>
			and rc.dtrowdeleted is null
		</CFQUERY>
		<!---<cfdump var="#recurringcharge#">--->
		<CFQUERY NAME="Charge" DATASOURCE =  "#APPLICATION.datasource#">
			SELECT	C.*, CT.*, C.cDescription as ChargeDescription
			FROM 	Charges C (NOLOCK)
			JOIN 	ChargeType CT (NOLOCK)	ON CT.iChargeType_ID = C.iChargeType_ID
		...
		<!---<CFQUERY NAME="residentapttypehistory" DATASOURCE =  "#APPLICATION.datasource#">
			Select distinct ts.iAptAddress_ID, apt.cDescription from rw.VW_tenantstate_history ts (NOLOCK)
			join AptAddress ad (NOLOCK) on ad.iaptaddress_ID=ts.iAptAddress_ID 
			join apttype apt (NOLOCK) on apt.iapttype_ID=ad.iAptType_ID
			where ts.iTenant_ID= #form.itenant_ID#
		</cfquery>
		<cfdump var= "#residentapttypehistory#">--->
</CFIF>
	...
	<TR STYLE = "font-weight: bold;">
		...
		<CFELSEIF NOT IsDefined("url.id")>
			...
			<TD STYLE="width: 25%;">#Tenant.cLastName#, #Tenant.cFirstname#<INPUT TYPE="Hidden" NAME="iTenant_ID" VALUE="#Tenant.iTenant_ID#"></TD>
				...
				<!---Mshah start comment <cfif ListContains(session.groupid,'192')>
	...
	<CFIF 	(
		...
		<!---Mshah start--->
			...
			<TD STYLE="text-align: left;">
				...
				<SELECT NAME="AppliesToMonth" onchange="displayrate();" onblur="displayrate();">
				    <CFIF IsDefined("form.iChargeType_ID") AND (#form.iChargeType_ID# eq 1748 or #form.iChargeType_ID# eq 1682)>
							<cfset moveinyearcheck = #year(TenantDetailRentEffective.dtrenteffective)#>
											<cfset currentYear = #year(now())# >
											
											<cfif 	currentYear eq 	moveinyearcheck>
												<cfset startmonth = #month(TenantDetailRentEffective.dtrenteffective)#>
											<cfelse>
												<cfset startmonth = 1>
											</cfif>
															
							<OPTION VALUE =""> </OPTION>
							<CFLOOP INDEX=I FROM=#startmonth# TO=12 STEP=1>
								<OPTION VALUE ="#NumberFormat(I, '09')#"> #NumberFormat(I, "09")# </OPTION>
							</CFLOOP>
					<cfelse>
				...
				<SELECT NAME ="AppliesToYear" onchange="displayrate();">
				  <CFIF IsDefined("form.iChargeType_ID") AND (#form.iChargeType_ID# eq 1748 or #form.iChargeType_ID# eq 1682)>
				  <OPTION value=""></OPTION>
				  </cfif>
			...
			<!---<TD></TD>
			<TD></TD>--->
			<CFIF IsDefined("recurringcharge.iChargeType_ID") AND (#recurringcharge.iChargeType_ID# eq 1748 or #recurringcharge.iChargeType_ID# eq 1682)>
				<TD> Charge <INPUT NAME='crdr' TYPE='radio' VALUE='1' onClick='displayrate()' checked>
				Credit <INPUT NAME='crdr' TYPE='radio' VALUE='-1' onClick= 'displayrate()' ></TD>
			</cfif>								
		<!---Mshah end for appliesperiod------>
		<!---Mshah start idaysbilled--->
		<!---set variable to show amount--->
		<cfif isDefined("recurringcharge.recurringamount") and #recurringcharge.recurringamount# neq ''> 
			 <cfset Monthlyrecurring = #TRIM(recurringcharge.recurringamount)#>
		<cfelse>
			<cfset Monthlyrecurring = #TRIM(Charge.mAmount)#>
		</cfif>
		</TR>
		<TR>
		<!--- Mshah end--->
				<cfif ListContains(session.groupid,'192')>
					<CFIF Charge.bIsModifiableAmount GT 0 OR (listfindNocase(session.codeblock,25) GTE 1 OR listfindNocase(session.codeblock,23) GTE 1)><!--- Sr. Acct or AR --->
						<TD>Amount
						<!---Mshah showing recurring charge instead charge amount for 1748 and 1682--->
						<cfif(#form.iChargeType_ID# eq 1748 or #form.iChargeType_ID# eq 1682)  and isDefined("recurringcharge.recurringamount") and #recurringcharge.recurringamount# neq ''<!---commented MShah---> >
							<Input Type="hidden" Name="HmAmount" VALUE="#TRIM(LSNumberFormat(Monthlyrecurring, "99999.99"))#">
							<INPUT TYPE="text" NAME="mAmount" SIZE="7" STYLE="text-align:center;background:lightgrey;" VALUE="#TRIM(LSNumberFormat(Monthlyrecurring, "99999.99"))#"  onBlur="this.value=Money(this.value); this.value=cent(round(this.value));" readonly ></TD>
						<cfelse>
							<INPUT TYPE="text" NAME="mAmount" SIZE="7" STYLE="text-align:center;" VALUE="#TRIM(LSNumberFormat(Charge.mAmount, "99999.99"))#"  onBlur="this.value=Money(this.value); this.value=cent(round(this.value));" ></TD>
						</cfif>
					<CFELSE>
				...
				<CFIF Charge.bIsModifiableQty GT 0>
					...
					<cfif Charge.iChargeType_ID is 8>
						<!--- If showdailyflag exsits, then this is a State Medicaid Daily charge, so show checkbox --->
						<input type="checkbox" name="bIsDaily" value="1"> Daily or
					</cfif> Qty <INPUT TYPE="text" NAME="iQuantity" ID="iQuantity" VALUE=" <cfif isDefined("recurringcharge.recurringamount") and #recurringcharge.recurringamount# neq ''><cfelse> #Charge.iQuantity# </cfif> " SIZE="2" STYLE="text-align:center;" MAXLENGHT="2" onBlur="this.value=Math.abs(Money(this.value));"></TD>
				<CFELSE>
					<TD><U>Quantity</U> <BR> #Charge.iQuantity# <INPUT TYPE="Hidden" NAME="iQuantity" VALUE="#Charge.iQuantity#"></TD>
				</CFIF> 
