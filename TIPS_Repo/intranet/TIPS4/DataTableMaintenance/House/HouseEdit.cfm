...
| MStriegel  | 11/13/2017 |  no lock to all queries              |
----------------------------------------------------------------------------------------------->
...
Following query will be used after proper table data is available
	...
	SELECT	*
	FROM	HOUSE
==============================================================added  mamta bIsMedicaid,bIsMemoryCare--->
<CFQUERY NAME = "HouseEdit" DATASOURCE = "#APPLICATION.datasource#">
	...
	,h.cNurseUser_id,h.bIsMedicaid,h.bIsMemoryCare,h.bIsCensusMedicaidOnly
	,h.County, h.OPCO, h.OPCO_EIN
	FROM 	OPSAREA O WITH (NOLOCK)
	INNER JOIN HOUSE H WITH (NOLOCK) ON	(O.iOPSArea_ID = H.iOPSArea_ID AND H.dtRowDeleted IS NULL)
...
<CFQUERY NAME="qGetHouseMedicaid" DATASOURCE="#APPLICATION.datasource#">
	...
	INNER join statecode s WITH (NOLOCK) on h.cstatecode=s.cstatecode 
	<CFIF IsDefined("url.HID")>
		WHERE	iHouse_ID = #url.HID#	
	</CFIF>
...
<CFQUERY NAME="PDName" DATASOURCE="DMS">
	...
	INNER JOIN ALCWEB.dbo.Employees e WITH (NOLOCK)	ON	u.employeeid = e.employee_ndx
...
<CFQUERY NAME="AcctID"	DATASOURCE="DMS">
	SELECT	u.employeeid, e.employee_ndx, e.fname, e.lname
	FROM	USERS	u WITH (NOLOCK)
	JOIN ALCWEB.dbo.Employees E WITH (NOLOCK)	ON	u.employeeid = e.employee_ndx
	<CFIF NOT IsDefined("url.Insert")>
...
<CFQUERY NAME="AssessmentCycle"	DATASOURCE="#APPLICATION.datasource#">
	...
	FROM AssessmentCycle WITH (NOLOCK)
...
<INPUT NAME="Message" TYPE="TEXT" VALUE="Required are listed in red." SIZE="75" STYLE="Color: Red; Font-Weight: bold; Font-Size: 16; text-align: center;">
	...
	<TABLE WIDTH="640">
		...
		<TR>	
			<TD>County</TD>
			<TD><INPUT TYPE="type" NAME="county"	VALUE="#TRIM(HouseEdit.County)#" MAXLENGTH = "20" STYLE = "text-align: left;"></TD>	
		</TR>	
		<TR>	
			<TD>OPCO</TD>
			<TD>
	<INPUT TYPE="type" NAME="OPCO" VALUE="#TRIM(HouseEdit.OPCO)#" size="60" MAXLENGTH = "75" STYLE = "text-align: left;">
			</TD>	
		</TR>	
		<TR>	
			<TD>OPCO EIN</TD>
			<TD>
	<INPUT TYPE="type" NAME="OPCO_EIN"	VALUE="#TRIM(HouseEdit.OPCO_EIN)#" MAXLENGTH = "10" STYLE = "text-align: left;">
		</TD>	
		</TR>				
		<TR>
		<TR> <!---mamta added--->	
		<cfif #TRIM(qGetHouseMedicaid.StateMedicaid)# eq 1>
			...
			<TR> <!---mamta added for Memorycare--->	
			<TD> Memory Care ? </TD>
			<cfif #TRIM(HouseEdit.bIsMemoryCare)# eq 1>
			<TD><INPUT TYPE="checkbox" NAME="bIsMemoryCare" VALUE="#TRIM(HouseEdit.bIsMemoryCare)#" STYLE = "text-align: Center;" checked="checked"</TD>	
			<cfelse> <TD><INPUT TYPE="checkbox" NAME="bIsMemoryCare" VALUE="#TRIM(HouseEdit.bIsMemoryCare)#" STYLE = "text-align: Center;" </TD>	
			 </cfif>
			</TR><!---mamta added end--->		
		<TR>
