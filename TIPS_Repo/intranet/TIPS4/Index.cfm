...
<CFQUERY NAME="qUserHouses"  DATASOURCE="#APPLICATION.datasource#"> 
	...
	<CFELSE>	
		...
		And (house.AssetClass is not null or house.ihouse_ID= 200) <!---MShah added this --->
	</CFIF>
...
<CFIF IsDefined("SESSION.APPLICATION") AND SESSION.APPLICATION EQ "TIPS4" AND (IsDefined("session.codeblock") AND listfindNocase(session.codeblock,23) GTE 1)>
	...
	<CFQUERY NAME="qDeletedHouses" DATASOURCE="#APPLICATION.datasource#">
		...
		FROM <!---rw.vw_house_history RVH MShah commented VW and added house table --->
		House rvh
		JOIN OpsArea OA ON (OA.iOpsArea_ID = RVH.iOpsArea_ID)
		JOIN Region R ON (R.iRegion_ID = OA.iRegion_ID)
		WHERE (RVH.dtRowDeleted IS NOT NULL or RVH.assetclass is null)
		AND	RVH.iAcctUser_ID IS NOT NULL 
		<!---MShah added this--->
		and RVH.ihouse_ID not in (200)  		
		<!---GROUP BY RVH.iHouse_ID, RVH.cName, RVH.cStateCode, RVH.cCity, RVH.iOPSArea_ID, RVH.cNumber, OA.cName, R.iRegion_ID, R.cName, RVH.cCompanyID 
