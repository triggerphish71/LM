...
| Mshah      | 03/29/2017 | Proj for DE medicaid removed same as NJ							   							|
| Hans Lim 	| 8/15/2018		| Added cfelse condition to allow insert into recurring charges for future dates |
----------------------------------------------------------------------------------------------->

<cftransaction>
<!--- Initialize User Field --->
<cfset User=SESSION.userid>
<cfparam name="cAppliesToAcctPeriod" default="">
<cfparam name="InvoiceNumber.iInvoiceMaster_ID" default="">
...
<!---
<cfoutput>
cfif (#dateFormat(TimeStamp,'mm/dd/yyyy')# GTE #dateFormat(dtEffectiveStart,'mm/dd/yyyy')# and #dateFormat(TimeStamp,'mm/dd/yyyy')# LTE #dateFormat(dtEffectiveEnd,'mm/dd/yyyy')#) OR (#dateFormat(SESSION.TipsMonth,'mm/dd/yyyy')# GTE #dateFormat(dtEffectiveStart,'mm/dd/yyyy')# and #dateFormat(SESSION.TipsMonth,'mm/dd/yyyy')# LTE #dateFormat(dtEffectiveEnd,'mm/dd/yyyy')#) <br>
--true<br>
else<br>
--false<br>
cfif<br><br>
...
<!--- Hans testing --->
		...
		<!--- start: When the effective date is before or on the TIPS month closing date --->
		<cfif (TimeStamp GTE dtEffectiveStart and TimeStamp LTE dtEffectiveEnd) OR (SESSION.TipsMonth GTE dtEffectiveStart and SESSION.TipsMonth LTE dtEffectiveEnd)>
			...
			<cfelse>
				...
				<cfif isDefined("NewAmount") and ChargeInfo.iChargeType_ID is 1661 and getStateMedicaid.recordcount is not 0>
					...
					where iInvoiceDetail_ID = #getStateMedicaid.iInvoiceDetail_ID#
						...
						<!--- need to add update to the tenantState ts.mMedicaidCoPay. --->
						<cfquery datasource="#application.datasource#">
						update tenantState
						set mMedicaidCoPay = #newAmount#
		...
		<!--- end: When the effective date is before or on the TIPS month closing date --->





<!--- Add the new Charge to the Recurring Table : NOTE FROM KATIE 9/29/03: Why would we add a new recurring charge for the SAME chargetype??? --->
...
<cfif #InvoiceNumber.iInvoiceMaster_ID# is ''>
	<form action="Recurring.cfm#@start" method="post" name="RecurringChargeError">
	<cfoutput>
	<tr>
...
<cfif IsDefined('ChargeInfo.iChargeType_ID')>
	...
	<cfquery name="UpdateInvDtlWRecChgID" datasource="#application.datasource#" result="UpdateInvDtlWRecChgID1">
	update InvoiceDetail
