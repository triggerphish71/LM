...
join opsarea ops with (NOLOCK) on h.iopsarea_id = ops.iopsarea_id
join region reg with (NOLOCK) on ops.iregion_id = reg.iregion_id
...
<CFQUERY NAME="ChargeList" DATASOURCE = "#APPLICATION.datasource#">
	<CFIF NOT IsDefined("url.TypeID")>
		...
		join ChargeType CT with (NOLOCK)on C.iChargeType_ID = CT.iChargeType_ID and CT.dtRowDeleted is null and c.dtRowDeleted is null
	...
	<CFELSE>
		...
		JOIN	Charges C with (NOLOCK) ON (C.icharge_ID = RC.iCharge_ID and C.dtRowDeleted is null and rc.dtRowDeleted is null)
...
<CFQUERY NAME = "CurrentRecurring" DATASOURCE = "#APPLICATION.datasource#">
	...
	FROM	RecurringCharge RC with (NOLOCK)
	JOIN	Charges C with (NOLOCK) ON C.iCharge_ID = RC.iCharge_ID and C.dtRowDeleted is null
	JOIN 	ChargeType CT with (NOLOCK) ON CT.ichargetype_id = C.iChargeType_ID and CT.dtrowdeleted is null
	JOIN	Tenant T with (NOLOCK) ON RC.iTenant_ID = T. iTenant_ID and T.dtRowDeleted is null and RC.dtRowDeleted is null
	...
	JOIN	House H	with (NOLOCK) ON H.iHouse_ID = T.iHouse_ID and H.dtRowDeleted is null and H.iHouse_ID = #SESSION.qSelectedHouse.iHouse_ID#
...
<CFQUERY NAME = "FindMissingRecurring" DATASOURCE = "#APPLICATION.datasource#">
		select t.cfirstname+', '+t.clastname as Residentname,t.csolomonkey, t.ihouse_ID from tenant t with (NOLOCK)
		...
		FROM	RecurringCharge RC with (NOLOCK)
		JOIN	Charges C with (NOLOCK) ON C.iCharge_ID = RC.iCharge_ID and C.dtRowDeleted is null
		JOIN 	ChargeType CT with (NOLOCK) ON CT.ichargetype_id = C.iChargeType_ID and CT.dtrowdeleted is null and CT.ichargetype_ID in (89,1748,1682)
		JOIN	Tenant T with (NOLOCK) ON RC.iTenant_ID = T. iTenant_ID and T.dtRowDeleted is null and RC.dtRowDeleted is null
		JOIN	TenantState TS with (NOLOCK) ON TS.iTenant_ID = T.iTenant_ID and TS.dtRowDeleted is null 
	...
	<CFQUERY NAME = "Findincorrectrate" DATASOURCE = "#APPLICATION.datasource#">
		...
		from tenantstate ts with (NOLOCK) join tenant t with (NOLOCK) on t.itenant_ID= ts.itenant_ID and t.dtrowdeleted is null and ts.dtrowdeleted is null
		 join house h with (NOLOCK) on h.ihouse_ID=t.ihouse_ID and h.dtrowdeleted is null and h.bissandbox=0 and h.ihouse_ID = #SESSION.qSelectedHouse.iHouse_ID#
		 join recurringcharge rc with (NOLOCK) on rc.itenant_ID = t.itenant_ID and rc.dtrowdeleted is null and rc.dteffectiveend >= '#SESSION.TIPSMonth#'
		 join charges c with (NOLOCK) on c.icharge_ID=rc.icharge_ID and c.dtrowdeleted is null and c.ichargetype_ID in (89,1748,1682) 
		 join aptaddress ad with (NOLOCK) on ad.iaptaddress_ID= ts.iaptaddress_ID and ad.dtrowdeleted is null 
		 join apttype at with (NOLOCK) on at.iapttype_ID=ad.iapttype_ID and (at.biscompanionsuite is null or at.biscompanionsuite = 0)
...
<TABLE>
	...
	<CFIF IsDefined("form.iTenant_ID") and IsDefined("form.iCharge_ID")>
		<!---project primary/secondary--->
			...
			select	 ts.iaptaddress_ID from Tenant T with (NOLOCK)
			join tenantstate ts with (NOLOCK) on ts.itenant_ID=t.itenant_ID and ts.dtrowdeleted is null
			join House H with (NOLOCK) ON T.iHouse_ID = H.iHouse_ID and H.dtRowDeleted IS NULL 
			and  T.dtRowDeleted IS NULL and T.iTenant_ID = #form.iTenant_ID#
			</cfquery>
		
			<cfquery name="FindChargeType" datasource="#APPLICATION.datasource#"> 
			Select iChargeType_ID from charges with (NOLOCK) where icharge_ID=#form.iCharge_ID#
			</cfquery>
			...
			<cfquery name="Findroommate" datasource="#APPLICATION.datasource#">
				...
				from 
				tenantstate ts with (NOLOCK) join tenant t with (NOLOCK) on ts.itenant_ID=t.itenant_ID
				join aptaddress ad with (NOLOCK) on ad.iaptaddress_ID=ts.iaptaddress_id
				join apttype at with (NOLOCK) on at.iapttype_ID=ad.iapttype_ID
				left join recurringcharge r with (NOLOCK) on r.itenant_ID=t.itenant_ID and r.dtRowDeleted is NULL
 				left join charges c with (NOLOCK) on c.icharge_ID=r.icharge_ID and c.dtRowDeleted is NULL  and c.ichargetype_ID in (89,1682,1748,31,1749) <!---Mshah added here for medicaid--->
				where
			...
			<cfquery name="findoccupancyadded" datasource="#APPLICATION.datasource#">
			 Select * from charges with (NOLOCK)
