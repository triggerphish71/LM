...
s Farmer     | 07/23/2015 | Removal of house log-on if url.selectedhouse_id is null use            |
             |            | session.houseaccesslist and then set variable thishouseID              |
********************************************************************************************** --->
...
<cfoutput>		session.userid=#session.userid#
	...
	<br>CalcHouse = #qhouse.cNumber#
		...
		<br>session.fullname=#session.fullname#
		<br>houseaccesslist: #session.houseaccesslist#
		<br>ad: #session.ad#		
		<br>groupid: #session.groupid#		
		<br>url.SelectedHouse_ID:: #url.SelectedHouse_ID#
		<br>CalcHouse = #qhouse.cNumber#
  ...
  <cfif url.SelectedHouse_ID is not ''>
...
<cfquery name="bondhouse" datasource="#application.datasource#">
	select ibondhouse, bBondHouseType,bIsMedicaid from house  where ihouse_id =  #thishouseID#
</cfquery>
