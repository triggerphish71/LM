...
<!---	<tr><td NOWRAP colspan=100%> <a href="TipsAccountBalance.cfm">TIPS Account Balance Summary</a></td></tr>
	<tr><td NOWRAP colspan=100%> <a href="assessmentupdate.cfm">June 1st</A> </td></tr>--->
...
<!--- 	<cfif ListFindNoCase(session.codeblock,21) GT 0><tr><td NOWRAP colspan=100%> <a href="InvalidSSNReport.cfm">Resident SSN Validation</A></td></tr></cfif> --->
...
<!--- 	<tr>
		<form name="MoveInSummaryByHouseReport" action="MoveInSummaryByHouseReport.cfm" method="POST">									
			<td nowrap="nowrap" style="width: 25%;">
				<a href ="" onClick="submit(); return false">Move In Summary By House Report</A>
			</td>
			<td style="width: 25%;">
...
<!--- 	<tr> ticket 103125
	...
	<tr>
		<td nowrap="nowrap"><a href="HouseMailingLabels.cfm">Mailing Labels</a></td>
<!--- 		<td nowrap="nowrap"> <a href="MailingLabels.cfm"> Mailing Labels </A> </td> --->
		<td style="width: 25%;"></td>
		<td style="width: 25%;"></td>
		<td style="width: 25%;"></td>
	</tr>	
...
<!--- 	<cfif (ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0)>
	<tr>
			...
			<td style="width: 50%;" colspan=1>
				...
				<tr><td>End Date: &nbsp;&nbsp;<input type="TEXT" name="MICheckInfoEnd" value="#DateFormat(Now(),'m/d/yyyy')#">(mm/dd/yyyy)</td></tr></table>
			<td style="width: 25%; padding-top:60px"><input type="Button" name="Go" value="GO" style="font-size: 12; color: navy; height: 20px; width: 30px;" onmouseover=""onClick="return DateCheck();submit(); return false;"></td>
			</form>
	</tr>
	</cfif> --->
...
<!--- 	<tr>
		...
		<td nowrap="nowrap"> <a href="" onClick="submit(); return false;"> Customer Trial Balance </A> </td>
		<td style="width: 25%;">
			<cfquery name="Tenants" datasource="#application.datasource#">
				select *, (T.cLastName + ', ' + T.cFirstName) as FullName 
			...
			<select name="prompt0">
				<cfloop query="Tenants">
				<option value="#Tenants.cSolomonKey#"> #Tenants.FullName# (#Tenants.cSolomonKey#)</option>	
				</cfloop>
			</select>
		</td>
 ...
 ---><!--- 	<tr>
	...
	<tr>
		...
		<td style="width: 25%;">
			...
			<input  type="hidden" name="prompt4" value="#session.housenumber#" />
			<input type="Button" name="Go" value="GO" style="font-size: 12; color: navy; height: 20px; width: 30px;" onClick="submit(); return false;">
		</td>
		<td style="width: 25%;">Comments: <input type="TEXT" name="cComments" value=""></td>
		</form>
...
<!--- 	<tr>
		...
		<td nowrap="nowrap"> <a href="" onClick="submit(); return false;"> Print Invoice History </A> </td>
		<td style="width: 25%;">
			<cfquery name="Tenants" datasource="#application.datasource#">
				select *, (T.cLastName + ', ' + T.cFirstName) as FullName 
				from Tenant T (nolock)
...
<!--- 	<tr>
		...
		<td nowrap="nowrap"> <a href="" onClick="submit(); return false;"> Resident Activity Summary </A> </td>
		<td style="width: 25%;">
			<cfquery name="Tenants" datasource="#application.datasource#">
				select *, (T.cLastName + ', ' + T.cFirstName) as FullName 
				from Tenant T (nolock)
...
<!--- 	 <cfif (ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0)>
	<tr>
	<form name="TenantPromotion" action="TenantPromotionReport.cfm" method="POST">
		<td nowrap="nowrap"> <a href="" onClick="submit(); return false;"> List of Resident Promotion </A> </td>
		<td style="width: 25%;">
			<select name="prompt0">
			<option value="ALL"> All Houses </option>
			<cfloop query="Houses"><option value="#Houses.iHouse_id#"> #Houses.cName#</option>	</cfloop>
			</select>
		</td>
...
<!--- 	<tr>
		<td>  Moved Out Tenant Collection Notes: </td>
		<td style="width: 25%;">
			<form name="CollectionNotes">
				<select name="TenantColNotes">
					<cfloop query="MovedOutTenants"><option value="#MovedOutTenants.cSolomonKey#"> #MovedOutTenants.FullName# (#MovedOutTenants.cSolomonKey#)</option></cfloop>
				</select>
			</form>
		</td>
		<td>
			...
			<script>
				function Run(){
					var location = "../Tenant/CollectionNotes.cfm?SolID=" + document.CollectionNotes.TenantColNotes.options[document.CollectionNotes.TenantColNotes.selectedIndex].value;
					window.open(location,"Notes","toolbar=no,width=900,height=600,resizable=yes,scrollbars= yes,top=10, left=150 ");
				}
			</script>
		</td>
		<td style="width: 25%;"></td>
