...
04/26/2017  | gthota    |         | added code to allow multiple months invoice for single resident       |
 --->	
	...
	<cfparam name="bUsesEFT" default="">
  	<cfif prompt0 is 'All'>
	<cflocation url="InvoiceReportC_test.cfm?prompt0=#prompt0#&prompt1=#prompt1#&prompt2=#prompt2#&prompt21=#prompt21#&prompt3=#prompt3#&prompt4=#prompt4#&bUsesEFT=#bUsesEFT#">
	</cfif>
	
	<cfoutput>
	<cfif #prompt0# EQ ""><font color="red"><b> Resident option not selected, Please select valid resident!.</b></font><br/>
	 <br/>
	<input type="button" name="Backbtn" value="Back to Report Main Page" onClick="javascript:window.history.go(-1);">	
	<cfabort> </cfif>
	
	<cfset   promt21 = '#prompt21#'>
	<cfset   promt22 = '#prompt2#'>
	
	<cfset year2 = left(promt21,4)>
	<cfset month2 = mid(promt21,5,2)>
	<cfset year1 = left(promt22,4)>
	<cfset month1 = mid(promt22,5,2)>
	<cfset month11 = "#year1#-#month1#-01">
	<cfset month22 = "#year2#-#month2#-01">  
	
	 <cfset value12 = datediff("m","#month11#","#month22#")>  
	
	 <cfif #prompt21# EQ #prompt2#><font color="red"><b> Both From and To dates are same, Please select proper date range values!.</b></font><br/>
	 <br/>
	<input type="button" name="Backbtn" value="Back to Report Main Page" onClick="javascript:window.history.go(-1);">	
	<cfabort> </cfif>
	 <cfif #prompt21# lt #prompt2#><font color="red"><b> Please select proper FROM date range values, It should be always lesser than TO date .</b></font><br/> 
	 <br/>
	<input type="button" name="Backbtn" value="Back to Report Main Page" onClick="javascript:window.history.go(-1);">	
	<cfabort></cfif>
	<cfif #value12# GT 12><font color="red"><b> Please select FROM and TO dates with in 12 months range.</b></font><br/> 
	 <br/>
	<input type="button" name="Backbtn" value="Back to Report Main Page" onClick="javascript:window.history.go(-1);">	
	<cfabort></cfif>
	...
	<!---	and im.cappliestoacctperiod = '#prompt2#'  --->
	 and im.cappliestoacctperiod between '#prompt2#' AND '#prompt21#'
...
<cfset count = count + 1>
	...
	<cfquery name="sp_Invoices_report" datasource="#application.datasource#">
		EXEC rw.sp_Invoices_report_test
			  @HouseNumber =   '#HouseData.cnumber#' , 
			@AcctPeriod =   '#prompt2#' ,
			@AcctPeriod2 =   '#prompt21#' ,
			@SolomonKey =   '#prompt0#'  
	</cfquery> 
...
<!---
 ...
 <cfloop query = 'sp_Invoices_report'> 
<cfoutput>
...
<cfelse>
	...
	<cfquery name="qryTransactions" datasource="#application.datasource#">
		...
		FROM     dbprod02.Houses_APP.dbo.vw_tips_GetTran   vw_tips_GetTran (NOLOCK)
		where custid = '#sp_Invoices_report.csolomonkey#' and 
		(vw_tips_GetTran.user7 between '#sp_Invoices_report.dtInvoiceStart#' and '#sp_Invoices_report.dtInvoiceEnd#'     )
		and docdesc <> 'AR Opening Balance'		
	</cfquery>

	<cfquery name="qryInvoiceCharges" datasource="#application.datasource#">
			EXEC  rw.sp_InvoiceChargesInvoiceReport_test
			@iInvoiceNumber =  '#sp_Invoices_report.iInvoicenumber#'
	</cfquery>
<</cfloop>
</cfoutput>

 <cfdump var="#qryInvoiceCharges#"> <cfabort>  --->
</CFOUTPUT>
...
<table  width="95%" style="border-bottom:thin">
	...
	<cfif qryTenant.bIsPayer is 1>
	  <cfset name = #sp_Invoices_report.cTenantName#>
		<tr> 
		<td style="font-size:12px;"> #qryTenant.Residentname# </td>
		</tr> 
		<tr>
	...
	<tr>
		<td style="font-size:12px;">#sp_Invoices_report.iinvoicenumber# <!--- /#iInvoicenumber# ---></td>
		<td style="font-size:12px;">
					<!---<cfif right(sp_Invoices_report.cappliestoacctperiod,2) is 01>
					January
					<cfelseif right(sp_Invoices_report.cappliestoacctperiod,2) is 02>
