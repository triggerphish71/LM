...
<!--- <cfoutput>	<cfdump var="#form#">
	...
	<cfif prompt0 is 'All'>
		...
		select t.csolomonkey  from tenant t (NOLOCK) 
		join tenantstate ts  (NOLOCK) on t.itenant_id = ts.itenant_id
		join invoicemaster im (NOLOCK)  on im.csolomonkey = t.csolomonkey and im.cappliestoacctperiod = '#prompt2#'
...
<body> 
	<cfdocument  format="PDF" orientation="portrait" margintop="1" marginbottom="1" 
	 ...
	 filename="\\fs01\ALC_IT\ReportCreation_DEV\#HouseData.cname#-#prompt2#\InvoiceReport-#qryTenant.Residentname#-#prompt2#.pdf" >	
