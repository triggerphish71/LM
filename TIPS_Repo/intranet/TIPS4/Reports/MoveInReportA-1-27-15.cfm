		 ...
		 <style>
			...
			<cfquery name="MoveInInvoice"  datasource="#application.datasource#">
				...
				,case when ichargetype_id in (89, 7, 91) then '1MonthlyFee'
						when ichargetype_id not in (69,7,89,91,83, 53, 1741) then '2AncillaryFee'
					...
					<table width="95%"  cellspacing="2" cellpadding="2"  > 
						<cfoutput query="qResident">
							...
							<tr>
								...
								<td   style="text-align:left">#MoveInInvoice.iinvoiceMaster_id#</td>
							</tr>						
						</cfoutput>	
						</table>
							...
							<cfif ((cdescription is 'New Resident Fee') or 
								...
								<cfelse>
									<tr>
									  ...
									  <cfif  (qResident.iMonthsDeferred is   0) or (qResident.iMonthsDeferred is   1)>  
										  <cfset CFinstallment = mamount>
										<tr>
											<td  colspan="1"  style=" font-size:12px;" >
											Installment (Month 1)</td>
											<td  style="text-align:center;font-size:12px;"  >									#numberformat(right(acctperiod,2),00)#/01/#left(acctperiod,4)#
												</td>
