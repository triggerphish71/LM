...
restypMed 2 
restypResp 3
<cfif IsDefined("restypMed") and restypMed is 2))>
...
<cfelseif (( isDefined("restypResp")) and (restypResp is 3))>
<CFQUERY NAME="qryCommunity" DATASOURCE="#APPLICATION.datasource#">
	SELECT	H.iHouse_ID, h.cName 'Community', H.cNumber,oa.cname as 'Region', OA.cNumber as OPS, r.cname as 'Division', R.cNumber as Region
	FROM	House H
	JOIN	OPSArea OA	ON	OA.iOPSArea_ID = H.iOPSArea_ID
...
<cfif residentselect is ''>
	<cfoutput>
		<form action="CensusMonthlyReportXLSSel2.cfm" name="CensusSelect" method="post" onsubmit="return valthisform();">
			...
			<tr>
				<td colspan="4" style="color:##FF0000; font-weight:bold; text-align:center">
					Enter Date of Report Selection, Enter 'YYYYMM' for the year and month required for the report
					<br />Select Region OR Select up to 5 individual Communities.
					<br />Select Resident Types: All, Private, Medicaid, Respite.
					<br />Click Submit 
					<br />Report will return as an Excel Spreadsheet
				</td>
			</tr>		 
			<tr>
				<td colspan="4" style="text-align:center">
					Enter date for Report as 'YYYYMM' (Current Month is default)<br />
					<input type="text" name="CensusMonth" value="#CensusMonth#" />
				</td>
			</tr>
...
<cfelseif residentselect is 'Yes'> --->
<cfoutput>
