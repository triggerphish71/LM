...
|------------|------------|--------------------------------------------------------------------|| 
| gthota    | 09/05/2017 | Created page for Transfer residents only                             |
------------------------------------------------------------------------------------------------->
...
<body> 
	...
	<cfquery name="checkdailycensustrack" datasource="#application.datasource#">
		...
		where 
		<!---	RL_FromHouse_ID = #checkhouse.FromHouseID#  --->
...
<cfquery name="validatehouse" datasource="#application.datasource#">
		...
		from [dbo].[RL_RES_STG] a (nolock)		
		 join	TenantState ts (nolock) on a.iTenant_ID = ts.iTenant_ID			
		 join Tenant t (nolock) 	on (t.iTenant_ID = ts.iTenant_ID)
		
		left join     
			 DailyCensusTrack_RL dct on dct.iTenant_ID = t.iTenant_ID    
				  and    dct.Census_Date = '#CompareDate#'  
		left join     
			 DailyCensusTrack_RL pdct on pdct.iTenant_ID = t.iTenant_ID    
				  and    pdct.Census_Date = '#BeginDate#'   
		where					
			toHouseid = #SESSION.qSelectedHouse.iHouse_ID#
			and t.dtRowDeleted is null
			and a.Active = 'Y'	
			<cfif sortby is ''>
				order by len(a.room) , a.room
			<cfelseif sortby is "Room">
				order by len(a.room) , a.room
			<cfelseif sortby is "residentid"> 
				order by isNull(t.iTenant_ID, 99999) , len(a.room) , a.room 
			<cfelseif sortby is "residentname"> 
				order by  IsNull(t.cLastName, 'zzzzz') , len(a.room) , a.room		
			</cfif>	
	</cfquery>	
	...
	<form name="TrackDailyCensus" action="TrackDailyCensus_RL.cfm" method="post">
		<input type="hidden" name="CompareDate" value="<cfoutput>#CompareDate#</cfoutput>">
		<input type="hidden" name="FromHouseID" value="<cfoutput>#checkhouse.FromHouseID#</cfoutput>">
		<input type="hidden" name="ToHouseID" value="<cfoutput>#checkhouse.ToHouseID#</cfoutput>">
	    <table border="1" align="center">
          ...
          <cfoutput query="validatehouse">
            ...
            <tr align="center">
              <td nowrap="nowrap"><b> #Start_Community# </b></td>
            	<td width="30"> #room# </td>
              	<td> #iTenant_ID# </td>
              	<cfif #iTenant_ID# neq ''>
