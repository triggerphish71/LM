...
<CFQUERY NAME = "AvailableTenants" DATASOURCE = "#APPLICATION.datasource#" CACHEDWITHIN="#Cachetime#">
	SELECT	T.iTenant_ID
		,(SELECT currbal=currbal+futurebal from #SolomonDBServer#.Houses_app.dbo.ar_balances where custid = T.cSolomonKey) as bal
	...
	WHERE	(TS.iTenantStateCode_ID < 4 
			OR (TS.iTenantStateCode_ID = 4 AND  0 < (SELECT currbal=currbal+futurebal from #SolomonDBServer#.Houses_app.dbo.ar_balances where custid = T.cSolomonKey) ) )
		...
		<CFELSE>
			...
			<CFELSE>
				...
				<CFIF Variables.Month NEQ 1> 
					<INPUT TYPE="Text" NAME="Year" Value="#Year#" SIZE="3" readonly onChange="dayslist(document.forms[0].Month, document.forms[0].Day, document.forms[0].Year); dtcheck();">
				<CFELSE>
