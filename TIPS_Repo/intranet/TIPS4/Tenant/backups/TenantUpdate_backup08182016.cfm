...
<cfif isDefined("bIsPayer") and isDefined("ContactbIsPayer")>
	<cfquery name="PayorCheck" datasource="#APPLICATION.datasource#">
		select * from LinkTenantContact ltc
		join Contact c on (ltc.iContact_id = c.iContact_id and c.dtRowDeleted is null)
		join Tenant t on (t.iTenant_id = ltc.iTenant_id and t.dtrowdeleted is null)
...
<cfquery name="GetHouseChargeSet" datasource="#application.datasource#">
	SELECT C.cName
	FROM ChargeSet C
	INNER JOIN House H ON H.iChargeSet_ID = C.iChargeSet_ID
...
<cftransaction>
		...
		<cfquery name="updatetenantresidenceagreement" datasource="#APPLICATION.datasource#">
			...
			<cfif (ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0)>
                ...
                <cfif  Isdefined("bDropIsPayer") and  bDropIsPayer is "X">   <!--- Added 93502 project code issue --->
				 <cfelse>          
			     bIsPayer = <cfif isdefined("form.bIsPayer")> #form.bIsPayer# <cfelse> null </cfif>,
				 </cfif>
	...
	<cfquery name="CheckMICheckDate"  datasource="#APPLICATION.datasource#">
		select isnull(dtMICheckReceived,'') as dtMICheckReceived from tenantstate where itenant_id = #form.iTenant_id#
	</cfquery>
 <!--- 95010 late fee exempt when using eft --->
	<cfquery name="ServicePoints" datasource="#APPLICATION.datasource#">
		...
		<cfif isdefined("form.cMilitaryVA") and form.cMilitaryVA neq "">,cMilitaryVA = #form.cMilitaryVA# <cfelse>,cMilitaryVA =  null </cfif>
		
		<cfif isdefined("form.cMilitaryVA") and form.cMilitaryVA neq 0> 
			<cfif isdefined("form.cMilitaryStartDate") and form.cMilitaryStartDate neq "">,cMilitaryStartDate = '#form.cMilitaryStartDate#'<cfelse>,cMilitaryStartDate = null</cfif>
			<cfif isdefined("form.cMilitaryEndDate") and form.cMilitaryEndDate neq "">,cMilitaryEndDate =  '#form.cMilitaryEndDate#'<cfelse>,cMilitaryEndDate =  null</cfif>
			<cfif isdefined("form.VaBenefits")>,VaBenefits =  #form.VaBenefits#<cfelse>,VaBenefits =  null</cfif>
			<cfif IsDefined("form.VaRepresentativeContacted")>,VaRepresentativeContacted = #trim(form.VaRepresentativeContacted)# <cfelse>,VaRepresentativeContacted = null</cfif>
			<cfif isdefined("form.VaBranchOfMilitary") and form.VaBranchOfMilitary neq "">,VaBranchOfMilitary = '#form.VaBranchOfMilitary#'<cfelse>,VaBranchOfMilitary =  null</cfif>
		 <cfelse>
		...
		<cfif isDefined("bMoveInSummary")>,bMoveInSummary = 1</cfif>
	...
	<cfif isDefined("session.CodeBlock") and ListFindNoCase(session.CodeBlock,25) GT 0>  <!--- only AR can update --->
		<cfif isDefined("form.bUsesEFT") and form.bUsesEFT neq "">
			...
			<cfquery name="existingEFT" datasource="#application.datasource#">
				select iEFTAccount_id from EFTAccount where cSolomonKey = '#trim(Tenant.cSolomonKey)#'
			</cfquery>
