...
Craig Ebbott            09/20/2011      Reworked entire page                                                           |
 ...
 sfarmer             | 08/08/2013 |     project 106456 EFT Updates                                                     |
  sfarmer            | 11/27/2013 |     project 107887 Corrections to make Contact Name, Address, etc required if Payer| |sfarmer             | 12/10/2013 | 112478 - adjustments to Billing Information                                        |
...
<!--- 	<cfquery name="TenantInfo" DATASOURCE="#APPLICATION.datasource#">
		SELECT ts.bUsesEFT, t.cSolomonKey,iproductline_id,t.iTenant_ID,
			...
			iPrimaryDiagnosis, iSecondaryDiagnosis, dtMICheckReceived,
			...
			cEmail, T.cComments as TenantComments, cMonthRASigned
			  ,ts.bExEFTSrvFee, ts.dDeferral, ts.dSocSec, ts.dMiscPayment, dMakeupAmt
			  , ts.bIsPrimaryPayer , ts.bIsNRFDeferred, ts.bIsVADeferred, ts.cMCOIDNbr, ts.dtMedicaidEligibility	
		FROM tenant T
	...
	<cfquery name="KeyChanges" DATASOURCE="#APPLICATION.datasource#">
		...
		join P_Tenant PT WITH (NOLOCK) on T.iTenant_ID = PT.iTenant_ID 
	...
	<cfquery name="getAmt"  datasource="#application.datasource#">
		...
		from Tenant tn WITH (NOLOCK)
		 join TenantState tns WITH (NOLOCK)  on (tns.iTenant_ID = tn.iTenant_ID and tns.dtRowDeleted is null and tns.iTenantStateCode_ID = 2)
		and tn.iHouse_ID = #session.qselectedhouse.ihouse_id#
		 join InvoiceMaster IM WITH (NOLOCK)  on im.csolomonkey = tn.csolomonkey and IM.dtRowDeleted is null 
		...
		join AptAddress AD WITH (NOLOCK)  on ad.iAptAddress_ID = tns.iAptAddress_ID and ad.dtRowDeleted is null
	...
	<cfquery name="EFTinfo" datasource="#application.datasource#">
		SELECT 
			...
			FROM  dbo.tenant T WITH (NOLOCK)
			 join dbo.EFTAccount EFTA WITH (NOLOCK) on  EFTA.cSolomonKey = T.cSolomonKey
			 join  tenantstate ts WITH (NOLOCK) on ts.itenant_id = t.itenant_id
			WHERE
				T.cSolomonKey = '#TenantInfo.cSolomonKey#'		
			 	<!--- and EFTA.dtRowDeleted is  null --->	
			 	and EFTA.iContact_id is  null
				<!---and ts.bUsesEFT =   1 	12/01 Mshah commented this as when cancelling EFT, the tag gets dropped--->			
		union
			SELECT
				t.clastname + ', ' + T.cfirstname	as 'TenantName'	
				,t.csolomonkey	
	...
	<cfquery name="qryInvAmt" datasource="#application.datasource#"> 
		Select IM.mLastInvoiceTotal
			...
			where IM.iInvoiceMaster_ID = INV.iInvoiceMaster_ID and INV.dtrowdeleted is null and inv.ichargetype_ID <> 69) as 'TipsSum'
		from
		InvoiceMaster IM
	...
	<!--- <cfquery name="qryInvAmt2" datasource="#application.datasource#">
	select  inv.iquantity, inv.mamount,   sum(inv.iquantity * inv.mamount) as tipstotal2  from  InvoiceDetail INV 
			where INV.iInvoiceMaster_ID =  #qryInvAmt.iInvoiceMaster_ID#   and INV.dtrowdeleted is null
	</cfquery> --->
	
	<cfquery name="qrysolomon" datasource="#application.datasource#">	
		select  isNull(Sum(amount ) , 0) as  'SolomonTotal'  
		from rw.vw_Get_Trx WITH (NOLOCK)
	...
	<cfquery name="sumpaymnt"  datasource="#application.datasource#"> 
		...
		from  --->  
		 ...
		 where  dtRowDeleted is null and csolomonkey = '#TenantInfo.csolomonkey#' 
	</cfquery>	
...
<!--- <cfoutput>#LastInvoiceTotal# + #thisSolomonTotal#  + #thisTipsSum#  + #offset#></cfoutput> --->
	<CFQUERY NAME="GetNRF" DATASOURCE="#APPLICATION.datasource#">  
		...
		and IM.cSolomonKey = '#TenantInfo.cSolomonKey#'	
		 JOIN	ChargeType CT WITH (NOLOCK)	ON CT.iChargeType_ID = INV.iChargeType_ID AND CT.dtRowDeleted IS NULL AND CT.iChargeType_ID  = 1740
...
<!---Mshah end--->
	...
	function hasexecutor()
		...
		hasexecutor();
			...
			//Check that only a contact OR tenant are marked as payer, not both
	//		if(df0.bIsPayer.value != 2){ //only people with access to cheange field will not - 2.  Houses will always equal 2
	//			if(document.getElementById("ContactPayers").value == 0) {  // No contacts are marked as payers at page load
	//				if ((df0.bIsPayer.checked == false && (df0.ContactbIsPayer != undefined && df0.ContactbIsPayer.checked == false)) 
	//					|| df0.bIsPayer.checked == false && df0.ContactbIsPayer == undefined)
	//					failed = true;
	//			}
	//			else{  //Have at LEAST one contact marked as a payer
	//				if (document.getElementById("CurrentContactPayer").value > 0){		
	//					if ((df0.bIsPayer.checked == true && df0.ContactbIsPayer.checked == true)||(df0.bIsPayer.checked == false && df0.ContactbIsPayer.checked == false))
	//						failed = true;
	//				}
	//				else{
	//					if (df0.bIsPayer.checked == true)
	//						failed = true;
	//				}		
	//			}
				// Alert if error found with who is marked as payer.
	//			if (failed == true){
	//				alert('Please choose either a contact or the tenant as the payor.\r(Both the tenant & contact as payor is not allowed.)');
	//				return false;			
	//			}
	//		}
	...
	// 		var  selstate = false;
	//		nbrstates = document.getElementById('cPreviousState').length;
	//		 alert(nbrstates);
	//		for(q=0; q < document.getElementById('cPreviousState').length; q++)
	//		{ 
	//			if(document.getElementById('cPreviousState').selectid)
	//			{
	//				alert('found state: ' + q);
	//				selstate = true;
	//			}
	...
	//	    if(selstate == false)
	//		{
	//			document.getElementById('cPreviousState').focus();
...
<cfoutput>
	...
	<tr>
		...
		</td><!--- onblur="maskZIP(this);" onkeypress="return allowOnlyNumberOnKey(event)" --->
	</tr>
<!--- 
	<TR style="background-color:##FFFF99"><TD colspan="5"><strong>OUTSIDE ADDRESS</strong></TD></TR>
	<tr  style="background-color:##FFFF99">
		<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Address Line 1</td>
		<td colspan="4"><input type="text" name="cOutsideAddressLine1"  id="cOutsideAddressLine1" value="#TenantInfo.cOutsideAddressLine1#" SIZE="40" 
       					 maxlength="40" onblur="upperCase(this);">
        </td>
	</tr>
	<tr  style="background-color:##FFFF99">
		<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Address Line 2</td>
		<td colspan="4"><input type="text" name="cOutsideAddressLine2"  id="cOutsideAddressLine2"  value="#TenantInfo.cOutsideAddressLine2#" SIZE="40" 
        				maxlength="40" onblur="upperCase(this);">
       </td>
	</tr>
	<tr  style="background-color:##FFFF99">
		<td colspan="5">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;City
			<input type="text" name="cOutsideCity"  id="cOutsideCity"  value="#TenantInfo.cOutsideCity#" 
            onkeypress="return allowOnlyLettersOnKey(event);" onblur="upperCase(this);">

			State
			<select name= "cOutsideStateCode"  id="cOutsideStateCode"  onBlur= "required();">
				<cfscript>
					if (TenantInfo.cOutsideStateCode NEQ "") 
						{ cOutsideStateCode=TenantInfo.cOutsideStateCode; }
					else 
						{ cOutsideState=House.cStateCode; }					
				</cfscript>

				<option value=""> None </option>	
				<cfloop Query = "StateCodes">
					 <cfscript>
						if (TenantInfo.cOutsideStateCode EQ cStateCode) 
							{ Selected = 'Selected'; }
						else 
							{ Selected = ''; }
					</cfscript> 
			...
			Zip (Postal) Code 
			<input  type="text" name="cOutsidezipCode" id="cOutsidezipCode" value="#TenantInfo.cOutsidezipCode#" size="10" maxlength="10"  onblur="maskZIP(this);"  > 
...
<!---  --->
	<tr>
		...
		<Cfelse>
		  <option value= "#TenantProductline.iproductline_id#">#TenantProductline.cDescription#	</option>		
		</cfif>
		</select>	
		</td>
	</tr>
	...
	<tr>
		...
		<!---Mshah Added this to give AR authority to change the residency type--->	
		...
		<!--- <td>#tenantinfo.Residency# <input type="Hidden" name="iResidencyType_ID" value="#tenantinfo.iResidencyType_ID#"></td> 
		Mshah test end --->
	</tr>
...
<!--- 		<cfif (ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0)> --->
			<cfif TenantInfo.bIsPayer EQ 1>
				...
				<!--- <td> The Tenant is also a Payor	</td> --->
				<!--- <td> If Yes, Click Here --><input type= "checkbox" name="bIsPayer" value="1" #CHECKED# onClick="required();"></td> --->	
			<cfelse>
				<cfset Checked=''>
...
<!--- 		<cfelse>
			<td><input type="hidden" name="bIsPayer" value=2>
				<cfif TenantInfo.bIsPayer IS 1> 
					Yes 
				<cfelse> 
					No
				</cfif>
...
<!--- 	<cfif TenantInfo.bIsPayer EQ 1 and ((ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0))>	 --->
	<tr style="font-weight: bold;">
...
<!--- 	</cfif>	 --->
<!--- <cfif TenantInfo.bIsPayer EQ 1 or ((ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0))>	
...
<cfif (ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0)>
    ...
    <tr>
        <td colspan="2" style="Font-weight: bold;"> Was a Move In Check received? 
		  <cfset bit= iif(TenantInfo.bMICheckReceived IS 1, DE('Checked'), DE('Unchecked'))>
			...
			<!---<cfif TenantInfo.bMICheckReceived is '' or TenantInfo.bMICheckReceived eq 0> <cfset MICCheck="Unchecked"> <cfelse> <cfset MICCheck="Checked"> </cfif>	
            #TenantInfo.dtMICheckReceived#<input type="Checkbox" name="cMICheckReceived" value="1" style="text-align: center;"  #Variables.MICCheck#>--->
        </td>
    </tr>
	<tr>
	...
	<!---test- mamta added secondary switch date
	<tr>
		<td colspan="2" style="Font-weight: bold;"> Secondary resident switch date
		<input type="text" Name="dtSecondaryswitch" value = "#DateFormat(TenantInfo.dbirthdate,"mm/dd/yyyy")#" size="6" maxlength="10">
			<b style="color:red;">(MM/DD/YYYY)</b>
		</td>
	</tr>
   ends here--->
	<!--- commented out as required by Justin, under review-mamta
	<tr>
		<td colspan="4" style="Font-weight: bold;">Has the resident signed the Care Management Agreement & Consent?		
		<cfset bit= iif(TenantInfo.bContractManagementAgreement IS 1, DE('Checked'), DE('Unchecked'))>
			<input type= "CheckBox" name= "bContractManagementAgreement" Value = "1" #Variables.bit#>
		</td>
	</tr>---->
	<tr>
		...
		<cfset bit= iif(TenantInfo.bMoveInSummary IS 1, DE('Checked'), DE('Unchecked'))>
			<input type= "CheckBox" name= "bMoveInSummary" Value = "1" #Variables.bit#>
		</td>
	</tr>
	<tr>
...
<!--- 	<cfif (ListContains(session.groupid,'240') gt 0) or (ListContains(session.groupid,'192') gt 0)>
	<tr>
			<td colspan="2" style="Font-weight: bold;">Does the Resident qualify for Deferred Payments?
			<cfset bitdp= iif(TenantInfo.bDeferredPayment IS 1, DE('Checked'), DE('Unchecked'))>
			<input type= "CheckBox" name= "cDeferredPayment" Value = "1" #Variables.bitdp#>
			</td>
	</tr>
	</cfif> --->

	<tr>
		<cfif TenantInfo.cMilitaryVA eq 1>
			<cfset Checked ='Checked'>
		<cfelseif TenantInfo.cMilitaryVA eq 0 >
			<cfset Checked1 ='Checked'>
		<cfelse >
			<cfset Checked =''>
		 	<cfset Checked1 =''>
		...
		<td colspan="3" style="font-weight: bold;">Have you or your spouse served in Military?    
			<input type="hidden" name="CheckVA" value="#TenantInfo.cMilitaryVA#" />
			Yes <input type="radio" id="cMilitaryVA" name="cMilitaryVA" value="1" #Checked#  onClick='document.getElementById("CheckVA").value = 1;ShowHideVA()'> 
			No  <input type="radio" id="cMilitaryVA" name="cMilitaryVA" value="0" #Checked1# onClick='document.getElementById("CheckVA").value = 0;ShowHideVA()'>
		</td>
	</tr>

	
		<!--- mstriegel 11/13/2017 Added logic to display bundled pricing check box ---->
	...
	<tr>
		<td></td>
		<td colspan="5" style="font-weight: bold;"><div id="VA" style="display:none" align="justify">
			Branch of Military served :
			<cfif TenantInfo.VaBranchOfMilitary eq ''>
			...
			<cfelse>
			  <input type="hidden" name="VaBranchOfMilitary" value="#TenantInfo.VaBranchOfMilitary#">#TenantInfo.VaBranchOfMilitary#
			</cfif>
		</div></td>
	</tr>
	...
	<tr>
		<td></td>
		<td colspan="5" style="font-weight: bold;"><div id="VA1" style="display:none">
	...
	<tr>
		<td></td>
		<td colspan="5" style="font-weight: bold;"><div id="VA2" style="display:none">
	...
	<tr>
		<td></td>
		<td colspan="5" style="font-weight: bold;"><div id="VA3" style="display:none">
			Dates of Service: Start 
			<input type="text" name="cMilitaryStartDate" value="#DateFormat(Tenantinfo.cMilitaryStartDate,"mm/dd/yyyy")#"  size="10" maxlength="10" onblur="dateformat(this)">
			&nbsp&nbsp End <input type="text" name="cMilitaryEndDate" value="#DateFormat(Tenantinfo.cMilitaryEndDate,"mm/dd/yyyy")#" size="10" maxlength="10" onblur="dateformat(this)">
	...
	<cfif isDefined("session.CodeBlock") and ListFindNoCase(session.CodeBlock,25) GT 0>
		...
		<tr style=" background:##CCCCFF">
			...
			<td > $<input type="text"   name="dSocSec"   value="#numberformat(TenantInfo.dSocSec, '999999.99')#"   /></td>
			<td>Miscellaneous Monthly Payments:<br/> (Enter total here and enter details in Comments Box Below)</td>
			<td  colspan="3" > $<input type="text"   name="dMiscPayment"   value="#numberformat(TenantInfo.dMiscPayment, '999999.99')#"   /></td>
		</tr>	
		<tr>
			<td> Comments: </td>
		   ...
		   <cfif #session.qselectedhouse.cStatecode# neq 'WI'> <!---mshah added here---> 
			...
			<tr id="typMedicaid3"  >
        		...
        		<cfif #session.qselectedhouse.cStatecode# neq 'WI'> <!---mshah added here--->
					...
					<!---<tr id="typMedicaid7"  >
						<td>Enter SICD</td>
						<td><input type="text" name="iSICD"  id="iSICD" value="#Tenantinfo.iSICD#"/></td>
					</tr >
					<tr id="typMedicaid8"  >
						<td>Enter TICD</td>
						<td><input type="text" name="iTICD"  id="iTICD" value="#Tenantinfo.iTICD#"/></td>
					</tr>--->
			    </cfif> <!---cfif end--->
		</table>
		</td>
	...
	<cfelse>		
		...
		<table>	
			...
			<tr>
       		 ...
       		 <!--- Mshah commented this as not required	
			<tr>
				<td>SICD</td>
				<td>#Tenantinfo.iSICD#</td>
			</tr >
			<tr >
				<td>TICD</td>
				<td>#Tenantinfo.iTICD#</td>
			</tr>--->
	        	</cfif> <!---cfif end--->
		</table>
		</td>
...
<!--- 	<tr>
		<td colspan="4">New Resident Fee Charged: <cfif GetNRF.mAdjNRF is not '' and GetNRF.mAdjNRF is not 0>#dollarformat(GetNRF.mAdjNRF)#<cfelse>#dollarformat(GetNRF.mBaseNRF)#</cfif>
		&nbsp;&nbsp;<cfif GetNRF.mAmtDeferred is not '' and GetNRF.mAmtDeferred is not 0>Amount NRF Deferred: #dollarformat(GetNRF.mAmtDeferred)#</cfif>
		&nbsp;&nbsp;<cfif GetNRF.iMonthsDeferred is not '' and GetNRF.iMonthsDeferred is not 0>Months Deferred: #GetNRF.iMonthsDeferred#</cfif>		
	</tr> --->
	...
	<cfif IsDefined("URL.CID")>
		...
		<tr>
			...
			<!--- <cfelse>
				<cfif Contact.bIsPayer IS 1> <cfset bit1="Checked"> <cfelse> <cfset bit1="Unchecked"> </cfif>
				<cfif Contact.bIsPayer IS 1> <cfset bit2=1> <cfelse> <cfset bit2=0> </cfif>
	 			<cfif #bit1# eq "Checked">
					Yes <input type="hidden" name="ContactbIsPayer" value=#bit2#>
				<cfelse> 
					No <input type="hidden" name="ContactbIsPayer" value=#bit2#>
				</cfif> 
				</td>  --->
			<!--- </cfif> --->
		</tr>
<!--- 		<tr>
			<td colspan="2" style="Font-weight: bold;"> Is this Contact the Move Out Payor?			
				<cfset bit= iif(Contact.bIsMoveOutPayer IS 1, DE('Checked'), DE('Unchecked'))>
				<input type="CheckBox"  id="ContactbIsMoveOutPayer" name="ContactbIsMoveOutPayer" value="1" #Variables.bit#>	
				</td>
