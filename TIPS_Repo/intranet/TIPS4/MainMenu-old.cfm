...
<!--- Retreive List of all available apartments --->
<cfinclude template="Shared/Queries/AvailableApartments.cfm">
<cfquery name="bondhouse" datasource="#application.datasource#">
	select ibondhouse, bBondHouseType from house  where ihouse_id =  #url.SelectedHouse_id#
</cfquery>

<cfquery name="bondapt" datasource="#application.datasource#">
	select a.iaptAddress_ID,a.bIsBond from AptAddress a where a.iHouse_ID = #url.SelectedHouse_id#
</cfquery>
...
<cfquery name="qResidentTenants" datasource="#application.datasource#">
	...
	<cfelseif URL.SelectedSortOrder eq "LateFee"> order by case WHEN t.iTenant_ID is null then NULL else sum(IsNull(tlf.mLateFeeAmount,0))-IsNull(PartialPayment,0) end desc, ad.cAptNumber
...
<cfquery name="MovedOut" datasource="#application.datasource#">
	...
	cAptnumber, cFirstName, cLastName, isPoints, cSLevelTypeSet, T.itenant_ID, DTMoveIn
...
<table>
	<tr>
	<th><a href="?SelectedSortOrder=Apt" style="color: White;" onMouseOver="hoverdesc('Sort by Apartment Number');" onMouseOut="resetdesc();"> Apt </a> </th>
	<th><a href="?SelectedSortOrder=Id" style="color: White;" onMouseOver="hoverdesc('Sort by Apartment Type');" onMouseOut="resetdesc();"> Type </a> </th>
	<th><a href="?SelectedSortOrder=Id" style="color: White;" onMouseOver="hoverdesc('Sort by Resident ID');" onMouseOut="resetdesc();"> RID </a> </th>
	<th><a href="?SelectedSortOrder=Name" style="color: White;" onMouseOver="hoverdesc('Sort by Name');" onMouseOut="resetdesc();"> Name </a> </th>
	<cfif bondhouse.ibondhouse eq 1>
	<th nowrap><a href="?SelectedSortOrder=Bond" style="color: White;" onMouseOver="hoverdesc('Sort By Bond Apt');" onMouseOut="resetdesc();"> BOND APT <cfoutput>#bhtml#</cfoutput> </a></th> 
	</cfif>
	<th><a href="?SelectedSortOrder=Type" style="color: White;" onMouseOver="hoverdesc('Sort by Residency Type');" onMouseOut="resetdesc();"> Type </a> </th>
	<th><a href="?SelectedSortOrder=Points" style="color: White;" onMouseOver="hoverdesc('Sort by Service Points');" onMouseOut="resetdesc();"> SPts. </a> </th>
	<th><a href="?SelectedSortOrder=MoveIn" style="color: White;" onMouseOver="hoverdesc('Sort by Financial Possession Date');" onMouseOut="resetdesc();">Financial Possession Date <!--- Physical Move-In Date  ---></a> </th>
	<th><a href="?SelectedSortOrder=MoveOut" style="color: White;" onMouseOver="hoverdesc('Sort by Move Out date');" onMouseOut="resetdesc();"> MoveOut </a> </th>
	<th nowrap> <a href="?SelectedSortOrder=LastBalance" style="color: White;" onMouseOver="hoverdesc('Sort by Current Balance');" onMouseOut="resetdesc();"> Curr. Bal. </a> </th>
	<th nowrap> <a href="?SelectedSortOrder=LateFee" style="color: White;" onMouseOver="hoverdesc('Sort by Late Fee');" onMouseOut="resetdesc();"> Late Fee </a> </th>
	<cfoutput>
		<cfset Month = DateAdd("m", +1,"#Now()#")>
		<th nowrap> <a href="?SelectedSortOrder=PaymentsSince" style="color: White;" onMouseOver="hoverdesc('Sort by Charges  ');" onMouseOut="resetdesc();"> #DateFormat(Session.TIPSMonth,"mmm")#<br>Charges </a> </th>
	</cfoutput>
	...
	<cfoutput query="qResidentTenants">
		...
		<cfif qResidentTenants.aptproductline eq 2><cfset atc="class='memcare'"><cfelse><cfset atc=''></cfif>
		<cfif qResidentTenants.iproductline_id eq 2><cfset tc="title='#qResidentTenants.cLastName#, #qResidentTenants.cFirstName# - memory care' class='memcare'"><cfelse><cfset tc=''></cfif>
				...
				<cfif IsRejApprover is 'Y'>
					<td style="width: 5%;  text-align: center; color:red;" > 
						...
						<cfset thislastname = qResidentTenants.cNRFDiscAppUsername>
							 ...
							 <a href="MoveIn/ReviseMoveIn.cfm?ID=#qResidentTenants.iTenant_ID#" style="color:red">NRF ADJUSTMENT APPROVAL IS REJECTED</a><!--- <br>#thislastname#<br>#qryWhoAreYou.cROle#<br>#qryARAnalyst.JobTitle# --->
					</td>
...
<!--- 				<cfif SESSION.qSelectedHouse.iopsarea_ID is  44> --->
				<td style="width: 5%; text-align: center; #MoveInCheck#">
						<cfif trim(qResidentTenants.dtRentEffective) neq "">
							<a href="Reports/MoveInReport.cfm?prompt0=#qResidentTenants.iTenant_ID#">#DATEFORMAT(qResidentTenants.dtRentEffective, "mm/dd/yyyy")#</a>
...
<!--- 			<cfif qResidentTenants.bNRFPend is 1>
			...
			<cfif qResidentTenants.dtMoveOut neq "">
				<td style="text-align: center;">
					<input class="MoveOutButton" type="button" name="Pending" VALUE="Pending" style="color: blue;" onClick="self.location.href='MoveOut/MoveOutForm.cfm?ID=#qResidentTenants.iTenant_ID#&edit=1'">
				</td>
			<cfelseif qResidentTenants.iTenant_ID neq "">
				<td style="text-align: center;">
					<input class="MoveOutButton" type="button" name="MoveOut" VALUE="MoveOut" onClick="self.location.href='MoveOut/MoveOutForm.cfm?ID=#qResidentTenants.iTenant_ID#'">
				</td>
			<cfelse> <td></td> </cfif>
			...
			<cfif qResidentTenants.iTenant_ID neq "">
				<cfif ListFindNoCase(TenantList, qResidentTenants.iTenant_ID, ",") gt 0>
					...
					<td style="text-align: right;">
						<cftry> #LSCurrencyFormat(ListGetAt(TenantTotal, pos, ","))#  <cfcatch type="ANY"> #Len(TenantTotal)# == #TenantTotal# </cfcatch> </cftry>
					</td>
				<cfelse>
	...
	<cfif qResidentTenants.itenant_id neq "">
			<cfquery name="MissingItems" datasource="#application.datasource#">
				...
				CASE WHEN ( select ibondhouse from house  where ihouse_id = #session.qSelectedHouse.iHouse_ID#)=1 
							and (select cBondHouseEligible from tenantstate where itenant_id = #qResidentTenants.itenant_id# and dtrowdeleted is null) is null THEN 1 ELSE 0 end +
	...
	<cfoutput query="MovedOut">
		...
		<cf_cttr colorOne="E6E6FA" colorTwo="E6E6F0">
			...
			<td> <a href="Reports/MoveInReport.cfm?prompt0=#MovedOut.iTenant_ID#"> #DATEFORMAT(MovedOut.dtMoveIn, "mm/dd/yyyy")# </a> </td>
			
			<cfif ListFindNoCase(session.codeblock,23) or ListFindNoCase(session.codeblock,24) or ListFindNoCase(session.codeblock,25)>
				<td>	
					<cfscript> if (MovedOut.iTenantStateCode_ID eq 4) { Button='Approved'; } else { Button='#DATEFORMAT(MovedOut.dtMoveOut, "mm/dd/yyyy")#'; } </cfscript>
					<input type="button" name="MoveOut" VALUE="#Button#" style="color: blue; font-size: xx-small; width: 65px; height: 20px; text-align: center; vertical-align: middle;" onClick="self.location.href='MoveOut/MoveOutFormSummary.cfm?ID=#MovedOut.iTenant_ID#'">
				</td>
			<cfelse>
				<cfif MovedOut.iTenantStateCode_ID eq 4>
					<td ID="modate#MovedOut.iTenant_ID#" style="font-size:12; text-align: center;" onMouseOver="hoverdesc('#DATEFORMAT(MovedOut.dtMoveOut, 'mm/dd/yyyy')#');" onMouseOut="resetdesc();"> Approved </td>
				<cfelse>
					...
					<td> <a href="./MoveOut/MoveOutReport.cfm?ID=#MovedOut.iTenant_ID#&MID=#qGetMOInvoice.iInvoiceMaster_ID#">#DATEFORMAT(MovedOut.dtMoveOut, "mm/dd/yyyy")#</a> </td>
				</cfif>
			</cfif>
	...
	<tr>
		<td colspan="14"><input class="ReturnButton" type="button" name="viewallhouseeft" Value="View House EFT's"   onClick="location.href='tenant/TenantEFTHouse.cfm'"  ></td>
	</tr>
</table>
<br/><br/>
