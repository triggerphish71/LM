<!---  -----------------------------------------------------------------------------------------
| S Farmer   | 09/19/2011 |Project 12352 changed sort order in query qCareCharges  c.mamount   |
|            |            | replaced cDescription                                              |
----------------------------------------------------------------------------------------------->
<cfoutput>
...
order by c.cchargeset desc, c.mamount asc
</cfquery>

<!--- retrieve last set changed "from" ---> <!--- RTS 11/30/08 added  'rw.' to the view name --->
...
<table>
	<tr><th colspan="4">#SESSION.HouseName# Resident Care Charges Administration</th></tr>
	<tr>
		<td>Description</td>
		<td>Amount</td>
	...
	<cfif qcarecharges.recordcount gt 0>
		<cfloop query='qCareCharges'>
			...
			<tr>
				<td>#qCareCharges.cDescription#</td>
	...
	<cfelse>
		...
		<cfloop query="qhouseset">
			...
			<cfif qhouseset.currentrow eq 1>
				...
				<dd style="font-size:xx-small;"> <b>** pre-populated with 'last' assinged rates</b> </dd>
				</td>
