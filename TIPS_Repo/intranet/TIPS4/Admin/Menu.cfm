...
| mstriegel  | 11/06/2017 | Modified the display so that it uses the new columns from the      |
| mstriegel  | 11/15/2017 | Replaced the Solomon Key column header with Resident ID            |
| mstriegel  | 01/10/2018 | Added query and logic to only run sp_EOM1A if there is un-approved |
|							assessments.													   |
|mstriegel   | 03/07/2018 | Added Respite Only to the link                                     |
----------------------------------------------------------------------------------------------->



<cfoutput>
<CFIF NOT IsDefined("SESSION.USERID") OR SESSION.UserId EQ "" OR NOT IsDefined("SESSION.qSelectedHouse.iHouse_ID") OR SESSION.qSelectedHouse.iHouse_ID EQ "" OR !IsDefined("session.qSelectedHouse.cNumber") OR session.qSelectedHouse.cNumber EQ "">
...
<!--- mstriegel 01/09/2018  add to reduce deadlock issues --->
	...
	<CFTRY>
		...
		NO parameters signifying MID MONTH ROLL thus rollingback all changes and not commiting any changes. --->
		  <cfstoredproc procedure="rw.sp_EOM1A" datasource="#APPLICATION.datasource#" RETURNCODE="YES" debug="Yes"> 		
			...
			<cfprocresult name="tempTbl" resultset="2">
			<cfprocresult name="tenantRent" resultset="3">
...
<cfif isdefined("url.debug") and url.debug EQ "show">
	<cfdump var="#midmonth#" label="Proposed Changes table"> 
	<cfdump var="#temptbl#" label="tempTable">
	<!---
	<cfdump var="#tenantrent#" label="Tenant Rent"> --->
</cfif>
		<CFCATCH TYPE="Lock">
...
<!--- mstriegel 01/10/2018 added logic --->
</cfif>
<!--- end mstriegel --->
...
<cfif #MonthAsString(Month(Now()))# EQ 'December'>
    <cfset month1 = #MonthAsString(Month(Now())-11)# >
<cfelse>
    <cfset month1 = #MonthAsString(Month(Now())+1)# >
</cfif> 

<cfoutput>  housemonth - #housemonth#   / month12 - #MonthAsString(Month(Now()))#  </cfoutput>
...
<CFIF IsDefined("ICNT") AND ICNT GT 0>
	...
	<TABLE STYLE="text-align: right;">
		<TR>
			...
			<!--- mstriegel:11/14/17 changed Sol Key to Resident ID --->
			<TH NOWRAP STYLE="text-align:left;">Resident Id</TH>
			<!--- end: mstriegel 11/14/17 --->
		...
		<CFLOOP QUERY="MidMonth" >
			<CFQUERY NAME="MidMonthInfo" DATASOURCE="#APPLICATION.datasource#">
				SELECT cLastName, cFirstName, cSolomonkey 
			...
			<CFQUERY NAME="EndofYearInfo" DATASOURCE="#APPLICATION.datasource#">
				SELECT inv.crowstartuser_id as crowstartuser_id 
...
<cfelseif midMonth.mNewAmount EQ midMonth.mOldAmount>
...
<cfelse>
			...
			<TR>
				...
				<TD #backcolor# style="text-align:left;">#LSCurrencyFormat(MidMonth.mDisplayOldAmount)#</TD>
				<TD #backcolor# style="text-align:left;">#LSCurrencyFormat(MidMonth.mDisplayNewAmount)#</TD>
				<!--- end mstriegel: 11/09/17 --->
				...
				<TD #backcolor# style="text-align:left;">#MidMonth.cAction#</TD>
			</TR>
	</cfif>
		</CFLOOP>
...
<!--- 		<TR>
		...
		<TR>
			...
			<!--- <CFIF session.username is 'gthota' or session.username is 'jgedelman'>  --->  <!--- ganga 3/28 comment code open for all admin user --->
		...
		<TR>
			<!--- mstriegel 3/7/18 --->
			<TD><a href="../Tenant/TenantAREdit.cfm">Edit Physical Move Out Dates (Respite Only)</a></TD><!---  and Invoice Totals --->
			<!--- end mstriegel 3/7/18 --->
			<TD>&nbsp;</TD>
		</TR> 		
		<TR>
