...
| mlaw       | 03/23/2007 | Exclude empty rooms                                                |
----------------------------------------------------------------------------------------------->
...
<cfquery name="GetAllTenants" datasource="#application.datasource#">
	select 
		...
		, mp.bisMedicaidPending as MedicaidPending
	from 
		AptAddress a (nolock)
	...
	left join TenantMedicaidPending mp
		on mp.iTenant_ID = t.itenant_ID
		and mp.dtrowdeleted is NULL
	where	
