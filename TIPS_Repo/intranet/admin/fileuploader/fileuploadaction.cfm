...
<cffile action="UPLOAD"
        ...
        accept="application/vnd.ms-excel,image/gif,application/msword,application/pdf,image/jpeg,video/quicktime,application/x-msexcel,image/pjpeg,text/html">
	...
	Select uniqueid
	From locationtype
	Where locationtypename = '#location#'
	</cfquery>
	
	<cfif childfile GT 0>
	<!--- comment: the order of column insert had to be swapped due to last minute design changes. Eventually the variable names will be switched to reflect the true roles of the variables. --AL--->
		<cfquery name="insertassociation" datasource="#datasource#" dbtype="ODBC">
		Insert Into docassociation(childdoc,parentdoc)
		Values(#getmediainfondx.uniqueid#,#childfile#)
		</cfquery>
	</cfif>
