...
<cfset Page = "Create House Visit"><head>
...
// alert(theEntryDate  + " : " + theBaseDate + " : " + document.getElementById('dtEntryDate').value + " : " + document.getElementById('idthisbasedate').value);
 if (abs(theBaseDate - theEntryDate) > 5){
//   document.getElementById('dtEntryDate').value =  theBaseDate;
 alert("WARNING: The Report Date CANNOT be more than 5 days back from today");
 	}
 if (theEntryDate > theBaseDate) {
 //  document.getElementById('dtEntryDate').value =  theBaseDate;
 alert("WARNING: The Report Date CANNOT be in the future");
 	}	
 }
	...
	<form id="frmHouseVisitOpEntry" name="frmHouseVisitOpEntry" method="post" <cfoutput> action="HouseVisitOperations_Action.cfm?iHouse_ID=#url.iHouse_ID#&url_SubAccount=#url.SubAccount#&DateToUse=#url.DateToUse#"</cfoutput> />
		<table width="80%">
			<cfoutput>
				...
				<tr>
					<td  align="left" style="font-family: verdana; font-weight: bold;" bgcolor="#freezeColor#" >Name: #userFullName#</td>
				</tr>
				<tr>
					<td  align="center"    nowrap="nowrap" style="font-family: verdana; font-weight: bold; text-align:left;" bgcolor="#freezeColor#" >
						 Date: 	<input type="text" name="dtEntryDate"   id="dtEntryDate" class="date-pick" style="text-align: CENTER;" value="#DateFormat(Now(), 'mm/dd/yyyy')#"  onBlur="chkEntryDate(this)"/>	
