...
<head>
	...
	<form id="frmHouseVisitOpEntry" name="frmHouseVisitOpEntry" method="post" <cfoutput> action="HouseVisitOperations_Action.cfm?ccllcHouse=#ccllcHouse#&iHouse_ID=#url.iHouse_ID#&SubAccount=#url.SubAccount#&DateToUse=#url.DateToUse#"</cfoutput> />
		<table width="80%">
			<cfoutput>
				...
				<cfelse>
					<input type="hidden"  name="EntryuserRole" 			id="idEntryuserRole" 		value="#url.userRole#"/>				
				</cfif>
					...
					<tr>
						<td>
							<table  id="#trim(dsGroups.cGroupName)#" width="90%" >
								...
								<cfELSEif dsGroups.IGROUPID is 6 or dsGroups.IGROUPID is 26>
									<cfset dsGroupQuestions = helperObj.FetchHouseVisitQuestionsII(dsGroups.iGroupId)>
									<cfset i = 0>
									<cfloop index="indexname" from="1" to="#maxrep#" step="1">
										<cfset i = i + 1>
								...
								<cfelseif  ((dsGroups.IGROUPID is 7) or  (dsGroups.IGROUPID is 25))>
									<cfset i = 1>
									<cfset dsGroupQuestions = helperObj.FetchHouseVisitQuestionsII(dsGroups.iGroupId)>
									<TR>
										<td style="text-align:left"><!--- #dsGroups.iGroupId# #dsGroupQuestions.iQuestionId# #i# --->
								...
								<CFELSE>	
									...
									<cfloop index="indexname" from="1" to="#maxrep#" step="1">
										...
										<tr  style=" background-color: ##ffffff;" >
											...
											<cfelse>
												<cfloop query="dsGroupQuestions">
													<td style="text-align:left" title="#dsGroupQuestions.cQuestion#" ><input type="text" name="txtanswer_#thisgroup#_#dsGroupQuestions.iQuestionId#_#i#" id="#trim(dsGroupQuestions.cIDName)##i#" value="#trim(defaultvalue)#" size="#dsGroupQuestions.cColSize#"  onBlur="#trim(dsGroupQuestions.cOnChange)#" <cfif dsGroupQuestions.readonly is "Y">readonly="true" </cfif>   style="text-align:left"; /> #dsGroupQuestions.posttitle#</td>
												</cfloop>
											</cfif>
										</tr>
									</cfloop>		
