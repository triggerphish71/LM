		...
		<font face="arial">
			<form method="post" action="labortracking.cfm">
				...
				<cfif rollup is 0>
					...
					<cfset dsLaborTrackingWDSInfo = #helperObj.FetchLaborTrackingWDSDaily(houseId, DateFormat(now()-1, "mm/dd/yyyy"),PtoPFormat,rollup)#>
					...
					<cfset dsHousesWithWD = #helperObj.FetchHousesWithWD(houseId, PtoPFormat)#>
						...
						<cfelseif dsLaborTrackingWDSInfo.nStatus is "L" and dsLaborTrackingWDSInfo.dTermDate is "" and (dsLaborTrackingWDSInfo.dDateCreated gte dsLaborTrackingWDSInfo.StartDate and dsLaborTrackingWDSInfo.dDateCreated lte dsLaborTrackingWDSInfo.EndDate)>
...
<cfif rollup is 0>
	<cfif ccllcHouse is 1> 
		<div id="tbl-container">
			<table id="tbl" cellspacing="0" cellpadding="1" border="1px" style="width: expression(screen.availWidth - 105);"><!--- style="width: 920px;">--->
	<cfelse>
		<div id="tbl-container">
...
<cfelse>
	<cfif rollup is 2>
		<div id="tbl-container" style="height:auto;"><!---expression(screen.availHeight - 300);" style="height:expression(getReportTableHeight()-100);"--->
			<table id="tbl" cellspacing="0" cellpadding="1" border="1px" style="width: 3200px; height:auto;">
	<cfelseif rollup is 1>
		<div id="tbl-container" style="height:auto;"><!--- style="height:expression(getReportTableHeight()-50);">--->
			<table id="tbl" cellspacing="0" cellpadding="1" border="1px" style="width: 3200px; height:auto;">
	<cfelse>
			...
			<cfif ccllcHouse is 0>
				<cfif cLaborTrackingCategory is not "WD Salary" and cLaborTrackingCategory is not "Kitchen Training">			
					...
					<cfif cLaborTrackingCategory is "Resident Care" or cLaborTrackingCategory is "Nurse Consultant" or cLaborTrackingCategory is "LPN - LVN" or cLaborTrackingCategory is "Kitchen" or cLaborTrackingCategory is "WD Hourly">
						...
						<cfif  cLaborTrackingCategory is "Resident Care" or cLaborTrackingCategory is "Nurse Consultant" or cLaborTrackingCategory is "LPN - LVN" or cLaborTrackingCategory is "WD Hourly">	
							<th bgcolor="#headerCellColor#" colspan="3">
						<cfelse>
							...
							<th bgcolor="#budgetcellcolor#" style="#topPaneStyle#">
								<cfif PtoPFormat gte "201303">
									<font size="-1">
										...
										<cfset TooltipDivStyles = "font-size: 8pt; font-family: tahoma; z-index: 140; padding: 2px; background-color: ##d9d9c2; display: none;">
										<a style="width: 100%; height: 100%; text-align: center; vertical-align: top; padding-top: 8px; cursor: hand; text-decoration: none; color: black;" title="Click to see the Description" titleDisplay="Variable Budget for Open WD Description" href='###TooltipDivName#' class='load-local' rel='###TooltipDivName#'>
											Var Bgt  
										</a>
...
<cfif rollup is not 0>
	...
	<cfif rollup is 3>
		<cfset dsCensusDetails = #helperObj.FetchDashboardRollupOccupancyDetails(RegionId, PtoPFormat, FromDate, ThruDate, rollup)#><!---FetchAverageDailyCensusDetailsRollup(RegionID, FromDate, ThruDate,monthforqueries)#>--->
	...
	<cfloop query="dsCensusDetails">
		<tr>
			<td class="locked" colspan="4" bgcolor="#emptyCellColor#" align="right" style="#leftPaneStyle#">
				...
				<cfelseif rollup is 1>
					<a href="labortracking.cfm?<cfif isDefined("url.NumberOfMonths")>NumberOfMonths=#url.NumberOfMonths#&Role=<cfif IsDefined('url.Role')>#url.Role#&<cfelse>0&</cfif>&ccllcHouse=#ccllcHouse#&rollup=2&Division_ID=#dsCensusDetails.iRegion_ID#
					<cfif isDefined("url.datetouse")>
						<cfset datetouse = #url.datetouse#>
					</cfif>
						&DateToUse=#DateFormat(datetouse,'mmmm yyyy')#
			...
			<cfloop query="dsLaborTrackingHours">
				...
				<!--- Stores all of the styles required for the standard tooltip Div element. --->
				<cfset houseTooltipDivStyles = "font-size: 8pt; font-family: tahoma; z-index: 140; padding: 2px; background-color: ##d9d9c2; display: none;">

				<cfif cLaborTrackingCategory is not "Kitchen Training" and cLaborTrackingCategory is not "WD Salary">
					<!--- Check if the current column should be displayed. --->
					<cfif bIsVisible eq true>
						<cfif rollup is 3>
							<cfset dsLaborTrackingWDSInfo = #helperObj.FetchLaborTrackingWDSDaily(dsCensusDetails.iHouse_ID, DateFormat(ThruDate, "mm/dd/yyyy"),PtoPFormat,rollup)#> 
						<cfelseif rollup is 2>
							<cfset dsLaborTrackingWDSInfo = #helperObj.FetchLaborTrackingWDSDaily(dsCensusDetails.iOPSArea_ID, DateFormat(ThruDate, "mm/dd/yyyy"),PtoPFormat,rollup)#>
						<cfelseif rollup is 1>
							<cfset dsLaborTrackingWDSInfo = #helperObj.FetchLaborTrackingWDSDaily(dsCensusDetails.iRegion_ID, DateFormat(ThruDate, "mm/dd/yyyy"),PtoPFormat,rollup)#>
						</cfif>
						<cfswitch expression="#cLaborTrackingCategory#">
							...
							<cfcase value="WD Hourly">
								<cfif rollup is 2><cfset dsWDHVarBgt = #helperObj.FetchLaborTrackingDivisionalWDHVarBgt(dsCensusDetails.iOPSArea_ID, PtoPFormat, true)#></cfif>
							...
							<cfcase value="Kitchen">
								...
								<cfset houseTooltipNursingDivStyles = "font-size: 8pt; font-family: tahoma; z-index: 140; padding: 2px; background-color: ##d9d9c2; display: none;">
	...
	<tr>
		...
		<cfloop query="dsMTDLaborCtgyHours">
			...
			<cfif dsMTDLaborCtgyHours.cLaborTrackingCategory is not "Kitchen Training" and dsMTDLaborCtgyHours.cLaborTrackingCategory is not "WD Salary">
				<!--- Check if the current column should be displayed. --->
					...
					<cfswitch expression="#cLaborTrackingCategory#">
						...
						<cfcase value="WD Hourly">
							...
							<cfif open is 1 or 
								...
								((samePeriod is 1 or currPeriod is 1) and (WDStermed is 0 and DateFormat(dsLaborTrackingWDSInfo.dHiredate-1, "dd") gte currentDay)) or
		...
		<cfelse> <!---ccllchouse is 1 --->
			...
			<cfif dsCCLLCSalariedEmpInfo.recordcount is not 0>	
				...
				<cfset dsSundaysOfMonth = #helperObj.FetchSundaysOfMonth(currentDay, PtoPFormat)#>
					<cfif dsSundaysOfMonth.name eq "Sunday">	<cfset sunday = 1>
					<cfelse>	<cfset sunday is 0>		</cfif>
			<cfelse>
				<cfset ccllcSalEmp = 0>
			</cfif>
