...
<cfset rollup= 0> <!--- rollup=0 (House); rollup=1 (consolidated); rollup=2 (division); rollup=3 (region) --->
	...
	<cfif Session.GroupList contains ("_ALC Corporate Office")>
	...
	<cfelseif Session.GroupList contains ("RDO") or Session.GroupList contains ("RDSM") or Session.GroupList contains ("RDQCS") or Session.GroupList contains ("OPS")>
		<cfset level="Region">
	<cfelse> <cfset level="House">
	</cfif>
<!---<cfdump var = "#Session.GroupList#">--->
<!---level is <cfdump var = "#level#">--->
<!---<cfdump var = "#Session#">--->
...
<cfif FindSubAccount.company is 0>
	...
	<cfquery name="dsGetUserInfo" datasource="#application.datasource#">
		SELECT [cFullName]
			  ...
			  ,[cScope]
			  ,[bCCLLCAccess]
		  FROM [Maple].[FTA].[dbo].[UserAccountAccess]
	...
	<cfquery name="dsGetCCLLCHouses" datasource="#TimeCard#">
		select 'CCLLC - ' + cName as cName
			   ...
			   ,cGLsubaccount 
		from Maple.fta.dbo.CCLLCHouse h
	...
	<cfquery name="dsGetDivisions" datasource="#application.datasource#">
		select cName as Division, iRegion_ID as DivisionID from Maple.fta.dbo.vw_Region where iRegion_ID <> 12 order by cName
	</cfquery>


	<cfif level is "Division">
		<cfloop query="dsGetDivisions">
			<cfif Session.GroupList contains ("#dsGetDivisions.Division#") or Session.ADDescription contains ("#dsGetDivisions.Division#")>
	...
	<cfquery name="dsGetRegions" datasource="#application.datasource#">
		...
		from Maple.fta.dbo.vw_OpsArea r
		join Maple.fta.dbo.vw_Region d on d.iRegion_ID = r.iRegion_ID
						...
						style="border: ##C0C0C0 inset 1px; font: 10pt Courier New, Courier, mono; padding: 1px; color: ##550000; background: ##FDFBEB;">						
							...
							<cfif FindSubAccount.company is 0>
								...
								<cfquery name="dsGetDivision" datasource="#application.datasource#">
									select cName as Division, iRegion_ID as DivisionID from Maple.fta.dbo.vw_Region where iRegion_ID = 12
								</cfquery>
							</cfif>
							...
							<!---<option value="location.href='#cgi.SCRIPT_NAME#?Role=<cfif IsDefined('url.Role')>#url.Role#&<cfelse>0&</cfif><cfif isDefined("url.NumberOfMonths")>NumberOfMonths=#url.NumberOfMonths#&<cfelse>NumberOfMonths=3&</cfif>ccllcHouse=0&rollup=1&Division_ID=#DivisionID#&Region_ID=#RegionID#<cfif dateToUse Is Not "">&DateToUse=#DateFormat(dateToUse, "mmmm yyyy")#</cfif>'"
								<cfif (rollup eq 1)> 
										SELECTED
								</cfif>>
								ALC Consolidated
							...
							<cfif FindSubAccount.company is 0>
									<!--- Fetch all of the regions. --->
								<cfquery name="dsGetRegions" datasource="#application.datasource#">
									select cName as Region, iOPSArea_ID as RegionID from Maple.fta.dbo.vw_OpsArea where iOPSArea_ID not in (32,27,28) order by cName
								</cfquery>
							</cfif>
							<option value=""></option>	
					...
					<cfif level is "Division">
						...
						style="border: ##C0C0C0 inset 1px; font: 10pt Courier New, Courier, mono; padding: 1px; color: ##550000; background: ##FDFBEB;">						
							...
							<cfif FindSubAccount.company is 0>
									<!--- Fetch all of the regions. --->
								<cfquery name="dsGetDivision" datasource="#application.datasource#">
									select cName as Division, iRegion_ID as DivisionID from Maple.fta.dbo.vw_Region where iRegion_ID = #DivisionID# and iRegion_ID <> 12
								</cfquery>
							</cfif>
							<cfoutput query="dsGetDivision">
								<option value="location.href='#cgi.SCRIPT_NAME#?Role=<cfif IsDefined('url.Role')>#url.Role#&<cfelse>0&</cfif><cfif isDefined("url.NumberOfMonths")>NumberOfMonths=#url.NumberOfMonths#&<cfelse>NumberOfMonths=3&</cfif>ccllcHouse=0&rollup=2&Division_ID=#DivisionID#<cfif dateToUse Is Not "">&DateToUse=#DateFormat(dateToUse, "mmmm yyyy")#</cfif>'"
							...
							<cfif FindSubAccount.company is 0>
									<!--- Fetch all of the regions. --->
								<cfquery name="dsGetRegions" datasource="#application.datasource#">
									select cName as Region, iOPSArea_ID as RegionID from Maple.fta.dbo.vw_OpsArea where iRegion_ID = #dsGetDivision.DivisionID#
							...
							<cfif FindSubAccount.company is 0>
								...
								<cfquery name="dsGetHouses" datasource="#application.datasource#">
									...
									inner join Maple.fta.dbo.vw_OpsArea ops on h.iOpsArea_ID = ops.iOpsArea_ID
						...
						style="border: ##C0C0C0 inset 1px; font: 10pt Courier New, Courier, mono; padding: 1px; color: ##550000; background: ##FDFBEB;">						
							...
							<cfif FindSubAccount.company is 0>
								...
								<cfquery name="dsGetRegion" datasource="#application.datasource#">
									<!---select distinct substring(n_orgLevel2,7,len(n_orgLevel2)) as Region, i_OrgLevel2Code as RegionID from Maple.timecard.dbo.tbl_OrgLevels--->
									select cName as Region, iOPSArea_ID as RegionID from Maple.fta.dbo.vw_OpsArea where iOPSArea_ID = #RegionID# and iOPSArea_ID not in (32,27,28)
								</cfquery>
							</cfif>
								...
								<cfif FindSubAccount.company is 0>
									...
									<cfquery name="dsGetHouses" datasource="#application.datasource#">
										SELECT
											...
											cGLSubAccount
										FROM dbo.House
										inner join Maple.timecard.dbo.tbl_OrgLevels o on o.i_OrgLevel3Code = substring(cGLsubaccount, 7,9)
										WHERE dtRowDeleted IS NULL AND bIsSandbox = 0 and o.i_OrgLevel2Code = #regid#
										<cfif isDefined("RDOrestrict")>
										AND iOpsArea_ID = #RDOrestrict#
										</cfif>
