...
<cfset y = "November 2004"><select name="datetouse" onchange="doSel(this)"><option value=""></option><cfloop condition="#DateCompare(x,y,'m')# GTE 0"><option value="location.href='labortracking.cfm?iHouse_ID=#LookUpSubAcct.iHouse_ID#&DateToUse=#DateFormat(x,'mmmm yyyy')#'"<cfif isDefined("datetouse") and datetouse is DateFormat(x,'mmmm yyyy')> SELECTED</cfif>>#DateFormat(x,'mmmm yyyy')#</option><cfset x = #DateAdd('m',-1,x)#></cfloop></select>
</td>
</Table>
...
<cfif getSalaried.recordcount is not "0">
	...
	<cfset QueryForSalaried = #QueryNew("Title,Hours")#>
		...
		<cfif vcLaborCategory is "Resident Care" or vcLaborCategory is "Nurse Consultant" or vcLaborCategory is "LPN - LVN" or vcLaborCategory is "Kitchen">
			...
			<cfif vcLaborCategory is "Nurse Consultant">
				<cfif isDefined("QueryForSalaried")>
					<cfquery name="CheckSalariedForNursing" dbtype="query">
						select title, sum(hours) as TotalNurseHours from QueryForSalaried where Title like '%RN Consultant%' group by title
					</cfquery><!--- <cfdump var="#CheckSalariedForNursing#"> --->
					<cfif CheckSalariedForNursing.recordcount is "0" OR CheckSalariedForNursing.TotalNurseHours is ""><cfset HoursToAddSalaried = "0"><cfelse><cfset HoursToAddSalaried = "#DecimalFormat(CheckSalariedForNursing.TotalNurseHours)#"></cfif>
					<cfset HoursToAdd = #hourstoadd# + #hourstoaddsalaried#>
				</cfif>
			</cfif>
			<td align=right>
			<cfset TotalAmountDayCategory = 0>
			<cfif findDayLaborCategoryRegular.recordcount is not "0" OR isDefined("CheckSalariedforNursing")>
			...
			<cfif vcLaborCategory is "Administrator">
				<cfif isDefined("QueryForSalaried")>
					<cfquery name="CheckSalariedForAdmin" dbtype="query">
						select title, sum(hours) as TotalAdminHours from QueryForSalaried where Title like 'Administrator%' group by title
...
<td align=right bgcolor="#variancecellcolor#">
	...
	<font size=-1><strong><cfif variance LT 0><font color="red"></cfif>#DecimalFormat(varianceTotal)#</strong>
</td>
<td>&##160;</td>
</tr>
...
<cfelse>
	<cfset colspannumber2 = #getColumns.recordcount# + 15>
	<td colspan=#colspannumber2# align=center><font size=-1><i>Since it is the first of the current month and calculations for each day run nightly at midnight, no actual wages have yet been determined for this month.</i></font></td>
</cfif>
...
<p>
<cfif isDefined("QueryForSalaried")>QueryForSalaried:<BR>
