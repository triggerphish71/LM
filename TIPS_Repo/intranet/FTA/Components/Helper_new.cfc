...
<!---<cffunction access="public" name="FetchSpendDetails" displayname="FetchSpendDetails" returntype="Query"
			description="Fetches the Spend Details DataSet.">
...
<cfargument name="iYear" type="numeric" required="true">
	<cfquery name="tmpFetchBudgetDetails" datasource="#FtaDsName#">
		SELECT iHouseId, cAccountid, cDescription, cAcctperday,	cAcctperMonth,	dtCreatedate,	cCreatedUser,	cDeletedUser 
	   FROM dbo.SpenddownAcct WHERE dtRowDeleted is null
	</cfquery>
	<cfreturn tmpFetchSpendDetails>
</cffunction>--->
...
<!---<cffunction name="updateNewSpend" output="false" access="public" returntype="void" >
  <cfargument name="ihouse_id" type="string" required="yes">
  <cfargument name="dropdown_acct" type="string" required="yes">
  <cfargument name="amount" type="string" required="yes">
  
   <cfquery name="updatenewspend" datasource="#FtaDsName#">
       insert into Spenddown (iHouseid,cAccountNum,cAmount,dtTransactionDate,cCreatedUser,cNotes) 
           values(
		     <cfqueryparam value="#arguments.ihouse_id#" cfsqltype="cf_sql_integer"/>
			 <cfqueryparam value="#arguments.dropdown_acct#" cfsqltype="cf_sql_integer"/>
			 <cfqueryparam value="#arguments.amount#" cfsqltype="cf_sql_float"/>
			 <cfqueryparam value="#arguments.notes#" cfsqltype="cf_sql_char"/>  <!---#form.iHouse_ID#,#form.DROPDOWN_ACCT#,#form.amount#,#form.datepicker#,#session.username#,#form.Notes#)--->
		    )
    </cfquery>
...
<!--- START Ganga--->
<cffunction access="public" name="FetchSpenddownDetails" displayname="FetchSpendDetails" returntype="Query"
			description="Fetches the Spend Details DataSet.">
<cfargument name="iHouseId" type="numeric" required="true">
<!---<cfargument name="iPeriod" type="string" required="true">--->
	<cfquery name="tmpFetchSpenddownDetails" datasource="#FtaDsName#">
		SELECT *
	     FROM 
		      SpendDownCategoryDetails 
	     WHERE  iHouseid = #iHouseId# AND
		      <!---cperiod = '#iPeriod#' AND--->
		       dtRowDeleted is null
			   
	</cfquery>
	<!---<cfdump var="#tmpFetchSpenddownDetails#">--->
	<cfreturn tmpFetchSpenddownDetails>
</cffunction>
<!---  END --->
...
<!--- Spend Down daily census budget table information --->
<!---<cffunction access="public" name="FetchSpendDownCensusBudget" displayname="FetchSpendDownCensusBudget" returntype="Numeric"
			description="Fetches the SpendDown Census Budget for the specified Month.">
<cfargument name="iHouseId" type="Numeric" required="true">
<cfargument name="iYear" type="Numeric" required="true">
<cfargument name="iMonthCode" type="string" required="true">
	<cfquery name="tmpFetchSpendDownOccupancyBudget" datasource="#FtaDsName#">
		SELECT TOP 1
			#iMonthCode# AS fOccupancyBudget
		FROM 
			HouseSpendDownCensusBudget
		WHERE 
			iYear = #iYear# AND 
			iHouseId = #iHouseId#;
	</cfquery>
	<cfreturn tmpFetchSpendDownOccupancyBudget.fOccupancyBudget>	
</cffunction>--->
...
<cfargument name="iDivisionId" type="string" required="true">
	...
	<cfquery name="tmpFetchDivisionInfo" datasource="#FtaDsName#">
		select cName as Division, iRegion_ID as DivisionID from vmMapleDev01.fta.dbo.vw_Region where iRegion_ID = cast('#iDivisionId#' as Int)
...
<cfargument name="iRegionId" type="string" required="true">
	...
	<cfquery name="tmpFetchRegionInfo" datasource="#FtaDsName#">
		select cName as Region, iOPSArea_ID as RegionID from vmMapleDev01.fta.dbo.vw_OpsArea where iOPSArea_ID = cast('#iRegionId#' as Int)
...
<cffunction access="public" name="FetchSpenddownRollupOccupancyDetails" displayname="FetchSpenddownRollupOccupancyDetails" returntype="Query"
			description="Returns a Regional rollup information for the dashboard occupancy.">
<cfargument name="iRegionIdOrDivisionId" type="string" required="true">
<cfargument name="iPeriod" type="String" required="True">
<cfargument name="iFrom" type="Date" required="true">
<cfargument name="iTo" type="Date" required="true">
<cfargument name="rollup" type="numeric" required="True">
	<cfquery name="tmpFetchSpenddownRollupOccupancyDetails" datasource="#FtaDsName#">
	...
	<cfreturn tmpFetchSpenddownRollupOccupancyDetails>		
</cffunction>
...
<cfargument name="iMonthCode" type="string" required="true">
	<cfquery name="tmpFetchCurrentCensusDetailsDivisionalRollup" datasource="#FtaDsName#">
		SELECT  sum(t.fTenants) as fTenants, sum(Coalesce(o.fOccupancy, 0.0)) AS fOccupancy, sum(o.fAvailable) as fAvailableUnits,
			...
			sum(ob.#iMonthCode#) as fOccupancyBudget, sum(isnull(p.iTwoPersonAssists,0)) as iTwoPersonAssists--, h.cName,
			 h.iHouse_ID,h.cGLSubAccount
		FROM HouseTenantCensus t 
		LEFT JOIN vw_HouseOccupancy o ON t.iHouseId = o.iHouseId AND t.dtOccupancy = o.dtOccupancy
		LEFT JOIN HouseAcuity a ON t.iHouseId = a.iHouseId AND t.dtOccupancy = a.dtOccupancy AND t.cType = a.cType
...
<cfargument name="iMonthCode" type="string" required="true">
	<cfquery name="tmpFetchAverageDailyCensusDetailsRollup" datasource="#FtaDsName#">
		...
		left outer join (SELECT convert(decimal(5,1),sum(iTwoPersonAssists),101)/convert(decimal(5,1),day('#iTo#'),101) as iTwoPersonAssists,
		 iHouseID
					FROM dbo.TwoPersonAssists t
					inner join DBProd01.Tips4.dbo.House h on h.ihouse_id = t.iHouseId AND h.bIsSandbox = 0 AND h.dtrowdeleted IS NULL
		...
		left outer join (SELECT  t.fTenants, Coalesce(o.fOccupancy, 0.0) AS fOccupancy, o.fAvailable,
						...
						(select count(iHouse_ID) as ccount from DBProd01.Tips4.dbo.House where iOPSArea_ID = cast('#iRegionId#' as Int)) as ccount,
						ob.#iMonthCode# as fOccupancyBudget, isnull(p.iTwoPersonAssists,0) as iTwoPersonAssists, h.cName,
						 h.iHouse_ID,h.cGLSubAccount
					FROM HouseTenantCensus t 
					LEFT JOIN vw_HouseOccupancy o ON t.iHouseId = o.iHouseId AND t.dtOccupancy = o.dtOccupancy
					LEFT JOIN HouseAcuity a ON t.iHouseId = a.iHouseId AND t.dtOccupancy = a.dtOccupancy AND t.cType = a.cType
		...
		inner join (select sum(fTenants) as fTenants, sum(fOccupancy) as fOccupancy, sum(fPoints) as fPoints
		, sum(fPoints)/sum(fTenants) as AvgServicePoints,iopsarea_id,ihouseid
					from (
							select sum(fTenants) as fTenants, sum(fOccupancy) as fOccupancy, sum(fPoints) as fPoints, a.ihouseid,h.iopsarea_id
							from (
									SELECT (DateDiff(Day, '#iFrom#', t.dtOccupancy) + 1) AS iDay,t.fTenants
									, Coalesce(o.fOccupancy, 0.0) AS fOccupancy,
											(CASE WHEN a.fAcuity IS NULL THEN 0.0 ELSE a.fAcuity * t.fTenants END) AS fPoints,t.iHouseId
									FROM HouseTenantCensus t 
									LEFT JOIN vw_HouseOccupancy o ON t.iHouseId = o.iHouseId AND t.dtOccupancy = o.dtOccupancy
...
<!---<cfargument name="islaborpage" type="boolean" required="true">--->
	<cfquery name="tmpFetchAverageDailyCensusDetailsConsolidatedRollup" datasource="#FtaDsName#">
		...
		FROM	(select	a.cName, c.fTenants as currTenants, c.foccupancy as currOccupancy, c.fPoints as currPoints
				, d.fPoints as sumOfPoints, sum(a.fTenants) as TotalTenants,
				c.fAvailable as currAvailableUnits, c.fOccupancyBudget as currOccupancyBudget, c.iTwoPersonAssists as currTwoPersonAssists,
				convert(decimal(10,1), (sum(a.fTenants)/day('#iTo#')),101) as fTenants, 
				convert(decimal(10,1), (sum(a.fOccupancy)/day('#iTo#')),101) as fOccupancy, 
				convert(decimal(10,1), d.AvgServicePoints,101) as fPoints,
			...
			inner join (select sum(fTenants) as fTenants, sum(fOccupancy) as fOccupancy, sum(fPoints) as fPoints
			, sum(fPoints)/sum(fTenants) as AvgServicePoints,iregion_id,ihouseid
						from (
								select sum(fTenants) as fTenants, sum(fOccupancy) as fOccupancy, sum(fPoints) as fPoints, a.ihouseid,h.iopsarea_id
								, o.iregion_id
								from (
	...
	<cfif islaborpage is false>
		...
		FROM
			...
			, iHouseID
						FROM dbo.TwoPersonAssists t
						inner join DBProd01.Tips4.dbo.House h on h.ihouse_id = t.iHouseId AND h.bIsSandbox = 0 AND h.dtrowdeleted IS NULL
						WHERE h.iOPSArea_ID = cast('#iRegionIdOriDivisionID#' as Int) and dtDeleted IS NULL 
						AND dtCompareDate BETWEEN '#iFrom#' AND '#iTo#'
						group by iHouseID
				) b on b.ihouseid = a.iHouse_ID
			left outer join (SELECT  t.fTenants, Coalesce(o.fOccupancy, 0.0) AS fOccupancy, o.fAvailable,
							...
							END) AS fPoints,
							(select count(iHouse_ID) as ccount from DBProd01.Tips4.dbo.House 
							where iOPSArea_ID = cast('#iRegionIdOriDivisionID#' as Int)) as ccount,
							#iMonthCode# as fOccupancyBudget, isnull(p.iTwoPersonAssists,0) as iTwoPersonAssists, h.cName
							, h.iHouse_ID,h.cGLSubAccount
						FROM HouseTenantCensus t 
						LEFT JOIN vw_HouseOccupancy o ON t.iHouseId = o.iHouseId AND t.dtOccupancy = o.dtOccupancy
						LEFT JOIN HouseAcuity a ON t.iHouseId = a.iHouseId AND t.dtOccupancy = a.dtOccupancy AND t.cType = a.cType
			...
			, sum(fPoints)/sum(fTenants) as AvgServicePoints,iopsarea_id,ihouseid
						from (
								select sum(fTenants) as fTenants, sum(fOccupancy) as fOccupancy, sum(fPoints) as fPoints, a.ihouseid
								,h.iopsarea_id
								from (
										SELECT (DateDiff(Day, '#iFrom#', t.dtOccupancy) + 1) AS iDay,t.fTenants
										, Coalesce(o.fOccupancy, 0.0) AS fOccupancy,
...
<cfif islaborpage is true>
		...
		From (SELECT (DateDiff(Day, '#iFrom#', t.dtOccupancy) + 1) AS iDay, t.fTenants, Coalesce(o.fOccupancy, 0.0) AS fOccupancy, o.fAvailable,
						(CASE WHEN a.fAcuity IS NULL THEN 0.0 ELSE a.fAcuity * t.fTenants END) AS fPoints, ops.cName, h.iOPSArea_ID,
						(select count(iOPSArea_ID) as ccount from FTA.dbo.vw_OPSArea 
						where iRegion_ID = cast('#iRegionIdOriDivisionID#' as Int)) as ccount
				FROM HouseTenantCensus t 
				LEFT JOIN vw_HouseOccupancy o ON t.iHouseId = o.iHouseId AND t.dtOccupancy = o.dtOccupancy
				LEFT JOIN HouseAcuity a ON t.iHouseId = a.iHouseId AND t.dtOccupancy = a.dtOccupancy AND t.cType = a.cType
				left join HouseCensusBudget b on t.ihouseid = b.ihouseid and iyear = year('#iTo#')
		...
		, o.iopsarea_id
					...
					inner join DBProd01.Tips4.dbo.House h on h.ihouse_id = t.iHouseId AND h.bIsSandbox = 0 AND h.dtrowdeleted IS NULL
					inner join FTA.dbo.vw_OPSArea o on o.iOPSArea_ID = h.iOPSArea_ID
					WHERE o.iRegion_ID = cast('#iRegionIdOriDivisionID#' as Int) and dtDeleted IS NULL 
					AND dtCompareDate BETWEEN '#iFrom#' AND '#iTo#'
					group by o.iopsarea_id
				) b on b.iOPSArea_ID = a.iOPSArea_ID
		inner join (select sum(fTenants) as fTenants, sum(fOccupancy) as fOccupancy, sum(fPoints) as fpoints
		, sum(fPoints)/sum(fTenants) as AvgServicePoints,iopsarea_id,iregion_id
					from (
...
<cfargument name="iHouseId" type="numeric" required="true">
	<cfquery name="tmpFetchCCLLCHouses" datasource="#FtaDsName#">
		...
		inner join TimeCard.timecard.dbo.tbl_OrgLevels o on o.i_OrgLevel3Code = h.iOrgLevel3Code
...
<cfargument name="iRollup" type="numeric" required="True">
	<cfquery name="tmpFetchLaborTrackingWDS" datasource="#FtaDsName#">
		...
		LEFT OUTER JOIN TimeCard.timecard.dbo.tbl_OrgLevels o on SUBSTRING(h.cGLsubaccount, 7, LEN(h.cGLsubaccount)) = o.i_OrgLevel3Code
		join fta.dbo.vw_OPSArea ops on ops.iOPSArea_Id = h.iOPSArea_ID
		outer apply (select top 1  iOrgLevel3Code, count(*) as cnt,
								sum(fWorkWeekAllocation) as fWorkWeekAllocation
								, sum(convert(decimal(10,2)
								, (fAllocationPercentage * 100))) as fAllocationPercentage 
...
<cfargument name="iPeriod" type="String" required="True">
	...
	<cfquery name="tmpFetchHousesWithWDHAndWDS" datasource="#FtaDsName#">
		...
		INNER JOIN LaborTrackingWDSDaily w ON w.iOrgLevel3Code = SUBSTRING(h.cGLsubaccount, 7, LEN(h.cGLsubaccount))
			and nPeriod = #iPeriod#
		INNER JOIN TimeCard.timecard.dbo.tbl_TimeCardCertification t on t.i_OrgLevel3Code = SUBSTRING(h.cGLsubaccount, 7, LEN(h.cGLsubaccount))
			and n_JobCode = 'WDH' 
