...
|------------|------------|--------------------------------------------------------------------|
| ranklam    | 01/17/2006 | Added flowerbox.                                                   |
| ranklam    | 01/17/2006 | Changed RCHDailyBudget set to only calculate if numeric.           |
| ranklam    | 01/23/2006 | wrapped droptmp query with try/catch so no errors are thrown.      |
| ranklam    | 01/24/2006 | Added a check for ##employeesBreakdown and delete it if exists.    |
| ranklam    | 01/24/2006 | numberofaveresidents neesd to be less than 61 or the               |
|                         | FindEfficiency query will throw an error.  THIS IS A TEMPORARY     |                       
|                         | FIX ONLY staffingguidepositionresidents table NEEDS TO BE EXPANDED |
| ranklam    | 04/14/2006 | this code is setting yesterday to 2 days from today, fixed it.     |
| ranklam    | 10/30/2006 | made sure that the amount is not an empty string                   |
| ranklam    | 01/08/2007 | Check for blank value in category2 when settnig the hours value in |
|            |            | QueryForCategoryHPRCD.                                             |
| mlaw       | 04/20/2007 | add paycodename = 'Worked Hours' to the findotherhours query       |
----------------------------------------------------------------------------------------------->
...
<cfif isDefined("session.EID") and Session.EID is not "">
	...
	select cGLsubaccount, EHSIFacilityID from HOUSE where dtRowDeleted IS NULL and cGLSubAccount = '#SubAccountNumber#'
	</cfquery>
	...
	<cfelse>
		...
		<cfquery name="getSubAccount" datasource="TIPS4">
		select cGLsubaccount, EHSIFacilityID from HOUSE where dtRowDeleted IS NULL and iHouse_ID = #url.iHouse_ID#
		</cfquery>
...
<cfelse>
	...
	<cfif isDefined("monthnowmonth")>
		...
		<cfelse><!--- use yesterday --->
			<!--- remove this line, the date is already yesterday --->
...
<!--- re-write for EV3 --->
<!--- 4/19/2005: All House emps for FTA will be in E-Time even if salaried pull all hours from e-Time. 
<Cfquery name="getSalaried" datasource="PROD3">
	SELECT
	J.AL_STD_HOURS,
	JC.DESCR,
	JC.DESCRLONG,
	...
	J.PAYGROUP,
	J.LOCATION,
	J.EXT_EHSI_DEPT, <!--- this is Dept# ie: 41000 --->
	J.PAYGROUP || J.FILE_NBR PERSONNUM
	FROM  PS_JOB            J,
		  PS_JOBCODE_TBL    JC,
		  PS_LOCATION_TBL   L
	WHERE J.EFFDT  = (SELECT MAX(EFFDT)
					   ...
					   WHERE EMPLID       = J.EMPLID
						 ...
						 AND ACTION_DT   <= TO_DATE('#lastdayofdatetouse#', 'MM-DD-YYYY'))
	...
	AND   J.EMPL_STATUS = 'A'
	AND   J.RATE_TYPE   = '4'
	AND   J.LOCATION = '#getSubAccount.EHSIFacilityID#'
...
<cfif getSalaried.recordcount is not "0">
	...
	<cfloop query ="getSalaried">
		...
		<cfset adddataTotal = #querysetcell(QueryForSalaried,"HomeDept",trim(getSalaried.EXT_EHSI_DEPT))#>
...
<!--- 4/19/05; temporarily disable allocations for phase 2
...
WHERE	VP_ALLTOTALS.APPLYDATE between '#currentfullmonthNoTime#' and '#lastdayofdatetouse#'<!--- 10/14/05: changed from #TODAY#, because doing one day old --->
AND  	VP_ALLTOTALS.WFCLABORLEVELNAME5  = '#getSubAccount.EHSIFacilityID#' <!--- '#Right(LookUpSubAcct.cGLsubaccount,3)#' --->
...
AND VP_ALLTOTALS.WFCLABORLEVELNAME6 <> '41002' <!--- wellndess director exclude --->
GROUP BY VP_ALLTOTALS.PERSONFULLNAME, VP_ALLTOTALS.WFCLABORLEVELDSC6, VP_ALLTOTALS.WFCLABORLEVELNAME6, VP_ALLTOTALS.PERSONNUM, VP_ALLTOTALS.PAYCODENAME, VP_ALLTOTALS.APPLYDATE
ORDER BY VP_ALLTOTALS.PERSONFULLNAME ASC
</cfquery>

<cfquery name="getColumns" datasource="#ftads#">
	select * from LaborCategory where vcRowDeleted is null ORDER BY iOrder
</cfquery>
<cfset CategoryList = #ValueList(getColumns.vcLaborCategory)#>
<cfset CategoryList = #ListAppend(CategoryList,"Administrators")#>
<cfset Dept_Num_List_All = "41000,41003,41050,41008,41010,41070,41009,41011,41015,41016,41017,41018,41019,41075,41020,41022,41024,41027,41030,41031,41032,41038,41078,41040">
...
<cfquery name="breakdown2" datasource="adpeet">
	...
	FROM	VP_ALLTOTALS WITH (NOLOCK)  
	WHERE	VP_ALLTOTALS.APPLYDATE between '#currentfullmonthNoTime#' and '#lastdayofdatetouse#'<!--- 10/14/05: changed from #TODAY#, because doing one day old --->
	AND  	VP_ALLTOTALS.WFCLABORLEVELNAME5  = '#getSubAccount.EHSIFacilityID#'
	AND VP_ALLTOTALS.WFCLABORLEVELNAME6 <> '41002' <!--- wellndess director exclude --->
...
(VP_ALLTOTALS.WFCTIMEINSECONDS <> 0 AND VP_ALLTOTALS.PAYCODENAME = 'PTO')
 OR
 (VP_ALLTOTALS.WFCLABORLEVELNAME6 NOT IN (#ListQualify(Dept_Num_List_All,"'")#))
OR
...
<!--- debugging only --->
<!--- <cfif isDefined("url.ihouse_id") AND session.username eq 'ranklam'>
	<cfquery name="breakdownselect" datasource="adpeet">
	select * from ##employeesbreakdown
	</cfquery>
	<cfdump var="#breakdown2select#">
	<cfdump var="#breakdownselect#">

	<cfquery name="debug1" datasource="adpeet">
		SELECT 
			 SUM(Seconds)
			,applydate
		FROM
...
<cfloop from=1 to="#currentd#" index="dayx">
	...
	<cfif getoccupancy.recordcount is not "0">
		...
		<cfif isNumeric(getoccupancy.iUnitsOccupied) AND isNumeric(RCHPRCD)>
	...
	<Cfloop query="getColumns">
		...
		<cfelseif vcLaborCategory is "MA - AA"><cfset Dept_Num_List = "41003,41050">
		<cfelseif vcLaborCategory is "Nurse Consultant"><cfset Dept_Num_List = "41008,41010,41070">
		<cfelseif vcLaborCategory is "LPN - LVN"><cfset Dept_Num_List = "41009,41011">
		<cfelseif vcLaborCategory is "Resident Care"><cfset Dept_Num_List = "41015,41016,41017,41018,41019,41075">
		<cfelseif vcLaborCategory is "Kitchen"><cfset Dept_Num_List = "41020,41022,41024,41027">
		<cfelseif vcLaborCategory is "Maintenance"><cfset Dept_Num_List = "41030,41031,41032">
		<cfelseif vcLaborCategory is "Housekeeping"><cfset Dept_Num_List = "41038,41078">
		<cfelseif vcLaborCategory is "Activities"><cfset Dept_Num_List = "41040">
		</cfif> 
		...
		<cfif vcLaborCategory is "Resident Care" or vcLaborCategory is "Nurse Consultant" or vcLaborCategory is "LPN - LVN" or vcLaborCategory is "Kitchen">
			<cfquery name="findDayLaborCategoryRegular" datasource="adpeet">
				select sum(cast(seconds as float))/3600 as Hours from ##employeesBreakdown where ApplyDate = '#theAMdateforQuery#' and Dept_Num IN (#ListQualify(Dept_Num_List,"'")#) and PayCodeName = 'Regular'
			</cfquery> 
			...
			<cfquery name="findDayLaborCategoryOvertime" datasource="adpeet">
				select sum(cast(seconds as float))/3600 as Hours from ##employeesBreakdown where ApplyDate = '#theAMdateforQuery#' and Dept_Num IN (#ListQualify(Dept_Num_List,"'")#) and PayCodeName = 'Overtime'
			</cfquery> 
			<td align=right>
			<cfset TotalAmountDayCategory = 0>
			...
			<cfquery name="findDayLaborCategoryOther" datasource="adpeet">
				select sum(cast(seconds as float))/3600 as Hours from ##employeesBreakdown where ApplyDate = '#theAMdateforQuery#' and Dept_Num IN (#ListQualify(Dept_Num_List,"'")#) and PayCodeName <> 'Overtime' and PayCodeName <> 'Regular'
			</cfquery> 
			<td align=right>
			<cfset TotalAmountDayCategory = 0>
	...
	<cfquery name="findOtherHours" datasource="adpeet">
		Select sum(cast(seconds as float))/3600 as Hours from ##employees2breakdown where <!--- Category NOT IN (#ListQualify(CategoryList,"'")#) ---> Dept_Num NOT IN (#ListQualify(Dept_Num_List_All,"'")#) and ApplyDate = '#theAMdateforQuery#' and Dept_Num <> '41001'<!--- community sales ---> and paycodename = 'Worked Hours'
	</cfquery> 
...
<cfelse>
	<cfset colspannumber2 = #getColumns.recordcount# + 15>
	<td colspan=#colspannumber2# align=center><!--- <font size=-1><i>Since it is the first of the current month and calculations for each day run nightly at midnight, no actual wages have yet been determined for this month.</i></font> ---><font color="red" face="arial" size=-1>The nightly process may have not run correctly.  Please contact the Help Desk.</font></td>
</cfif>
