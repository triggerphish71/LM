...
|mstriegel  | 04-09-2018  | fixed error with evaldate not being defined                        |
|gthota    | 10/31/2017  |Added code to check the 91 charges from old and new chargeset        |
    |
----------------------------------------------------------------------------------------------->
		...
		<cfif (not isdefined("URL.fuse") or URL.fuse EQ "viewAssessment") > 
			<cfquery name="QueryGetInformation" datasource="#variables.dsn#">
				SELECT
					  ...
					  ,IsNull(atm.iSlPoints,'') AS iSlPoints
...
<!--- This changes Ends here for project 41315-B (Documented this for ease of the developer who is moving this code) --->
   ...
   <cfelse>
       ...
       <cfquery name="GetServicesQuery" datasource="#variables.dsn#">
		...
		<cfscript>
			...
			if(theQuery.RecordCount gt 0)
			{
				...
				variables.lpoints = theQuery["iSlPoints"][1];
	...
	<!---	<cfargument name="lpoints" type="numeric" required="false" default="0"> --->
		...
		<cftransaction>
			...
			<cfif IsArray(arguments.ServiceArray) AND ArrayLen(arguments.ServiceArray) gt 0>
				<cfquery name="ServicePointsQuery" datasource="#arguments.dsn#">
					SELECT
						...
						SUM(flWeight) as ServicelPoints
					FROM
						ServiceList
					WHERE
			...
			<cfelse>
				...
				<cfset ServicePointsQuery.ServicelPoints = 0>
			</cfif>
			...
			<cfif IsArray(arguments.SubServiceArray) AND ArrayLen(arguments.SubServiceArray) gt 0>
				<cfquery name="SubServicePointsQuery" datasource="#arguments.dsn#">
					SELECT
						...
						SUM(flWeight) as SubServicelPoints
					FROM
						SubServiceList
					WHERE
			...
			<cfelse>
				...
				<cfset SubServicePointsQuery.SubServicelPoints = 0>
			</cfif>
			
			<cfset totalPoints = ServicePointsQuery.ServicePoints + SubServicePointsQuery.SubServicePoints>
		   <cfset totallaborPoints = ServicePointsQuery.ServicelPoints + SubServicePointsQuery.SubServicelPoints> 
...
<!---This changes Ends here for project 41315-B (Documented this for ease of the developer who is moving this code) --->	
	...
	<cffunction name="Save" access="public" output="true" returntype="void">
		...
		<cftransaction>		
			...
			<cfelse>
				<cfset SubServicePointsQuery.SubServicePoints = 0>
				<cfset SubServicePointsQuery.SubServicelPoints = 0>		<!--- whoever is adding labor points isn't adding this statement sfarmer --->		
			</cfif>
...
<!--- 		<cfif qresident.recordcount gt 0>
			...
			<cfquery name="WriteActivity" datasource="#variables.dsn#">
				...
				values(10
					  ...
					  <cfif variables.points neq 0>,#variables.points#<cfelse>,1</cfif>  <!---,#variables.points#--->
					   <cfif variables.lpoints neq 0>,#variables.lpoints#<cfelse>,1</cfif> 
		...
		<cftransaction>
			...
			<cfif TenantQuery.iresidencytype_id neq 2 and tenantState neq 4>
					...
					<cfloop from="0" to="#activeMonthDiff#" step="#step#" index="i">
						...
						<cfif qmonthdata.recordcount eq 0>
							...
							<cfelse>
								<!--- mstriegel 04/09/2018 --->
								<cfset thisevaldate =createOdbcDateTime(dateformat(arguments.billingActiveDate,"mm/dd/yyyy") & " 23:59:59")>
								<!--- end mstriegel 04/09/2018 --->
							</cfif>
							...
							<cfquery name="qptMarketData" datasource="#dsn#">
								...
								join 
									rw.vw_tenantstate_history vts on vts.itenant_id = vt.itenant_id
										and vt.dtrowdeleted is null 
										and #thisevaldate# between vt.dtrowstart and isNull(vt.dtrowend,getdate())
										and vts.dtrowdeleted is null 
										and #thisevaldate# between vts.dtrowstart and isNull(vts.dtrowend,getdate())
									and vts.itenantstatecode_id = 2
								join 
									rw.vw_aptaddress_history vah on vah.iaptaddress_id = vts.iaptaddress_id
										and vah.dtrowdeleted is null 
										and #thisevaldate# between vah.dtrowstart and isNull(vah.dtrowend,getdate())
								join 
									rw.vw_apttype_history vap on vap.iapttype_id = vah.iapttype_id
										and vap.dtrowdeleted is null 
										and #thisevaldate# between vap.dtrowstart 
										and isNull(vap.dtrowend,getdate())
								join 
									rw.vw_servicelevel_history vsl on vsl.csleveltypeset = vt.csleveltypeset 
										and vsl.dtrowdeleted is null
										and vts.ispoints between vsl.ispointsmin and vsl.ispointsmax
										and #thisevaldate# between vsl.dtrowstart and isNull(vsl.dtrowend,getdate())
								join 
									rw.vw_charges_history vch on vch.ihouse_id = vt.ihouse_id 
										and vch.dtrowdeleted is null
										and vch.iSLevelType_ID = vsl.iSLevelType_ID
										and #thisevaldate# between vch.dteffectivestart and vch.dteffectiveend
										and #thisevaldate# between vch.dtrowstart and isNull(vch.dtrowend,getdate())
								join 
									rw.vw_chargetype_history vcth on vcth.ichargetype_id = vch.ichargetype_id 
										and vcth.cGrouping='RD'
										and #thisevaldate# between vcth.dtrowstart and isNull(vcth.dtrowend,getdate())
								where 
						...
						<!--- Gthota 09/18/2017 - added for lookup old chargeset --->
						    
								 
						   <CFIF qnewRates.RecordCount GT 0>
						  	<cfelse> 
						  	  <cfquery name="qchargeset" datasource="#variables.dsn#">
								   Select cchargeset from tenant with (nolock) where  itenant_id = #variables.tenantId#								
								 </cfquery>
						
						       <cfquery name="qnewchargeset" datasource="#variables.dsn#">
								   Select top 1 dtrowend, cchargeset from p_tenant with (nolock)
									where cchargeset <>'#qchargeset.cchargeset#' and  itenant_id = #variables.tenantId#
									order by dtrowend desc
								 </cfquery>  
								 
								 <cfquery name="qMonthDataold" datasource="#variables.dsn#">
								  select top 1 * from Invoicedetail where itenant_id = #variables.tenantId# and iChargetype_id=91 
								  and cAppliesToAcctPeriod = '#trim(dateformat(now(),"yyyymm"))#'
								  order by iInvoicedetail_id desc
								</cfquery>
				<!--- gthota - change the logic --->				
								<cfif qMonthData1.mAmount eq qMonthDataold.mAmount>
								  <cfset oldamount = qMonthData1.mAmount>
								<cfelse>
								  <cfset oldamount = qMonthDataold.mAmount>
								</cfif>
								 						  	
									<cfquery name="qnewRatesold" datasource="#variables.dsn#">
										select  c.*
											,sl.cdescription AS level
											,c.cdescription chargedesc
										from 
										...
										join chargetype ct with (nolock) on ct.ichargetype_id = c.ichargetype_id 
												...
												and c.ihouse_id = #GetTenant().GetHouse().GetId()# 
												and ct.cgrouping='RD'
										join	tenant t with (nolock) on t.ihouse_id = c.ihouse_id 
												and t.dtrowdeleted is null 
											<!---	and isNull(t.cchargeset,1) = isNull(c.cchargeset,1)  --->
										join tenantstate ts with (nolock) on ts.itenant_id = t.itenant_id 
												and ts.dtrowdeleted is null
										join sleveltype sl with (nolock) on sl.isleveltype_id = c.isleveltype_id 
												and sl.dtrowdeleted is null
												and #variables.points# between sl.ispointsmin and sl.ispointsmax 
												and sl.csleveltypeset='#GetAssessmentTool().GetSLevelTypeSet()#' 
										where t.itenant_id = #variables.tenantId#  AND c.cchargeset = '#qnewchargeset.cchargeset#' 
										and #CreateODBCDateTime(arguments.billingActiveDate)# between c.dtEffectiveStart and c.dtEffectiveEnd  
										order by 
											c.dtrowstart desc
									</cfquery> 
						<!---- TIPS current month    ---->
									<cfquery name="qchargesetnext" datasource="#variables.dsn#">
								   Select cchargeset from tenant with (nolock) where  itenant_id = #variables.tenantId#								
								 </cfquery>
											 						  	
									<cfquery name="qnewRatesnext" datasource="#variables.dsn#">
										select  c.*
											,sl.cdescription AS level
											,c.cdescription chargedesc
										from 
										...
										join sleveltype sl with (nolock) on sl.isleveltype_id = c.isleveltype_id 
												...
												and #variables.points# between sl.ispointsmin and sl.ispointsmax 
												and sl.csleveltypeset='#GetAssessmentTool().GetSLevelTypeSet()#' 
										where t.itenant_id = #variables.tenantId#  AND c.cchargeset = '#qchargesetnext.cchargeset#' 
									<!---	and getdate() between c.dtEffectiveStart and c.dtEffectiveEnd  --->
										order by 
											c.dtrowstart desc
									</cfquery>
									
									<!--- rate information for next period month of level care --->		
										<cfif qnewRatesnext.mAmount eq "">
											<cfset newamountnext = 0>
										<cfelse>
											<cfset newamountnext = qnewRatesnext.mamount>
										</cfif>
										
										<cfset newctypenext=qnewRatesnext.ichargetype_id>
										<cfset newlevelnext=qnewRatesnext.level>
										<cfset newtypedescnext=qnewRatesnext.chargedesc>
											 												
							</CFIF>  
							
						<!--- Ganga End code --->
							<cfif isDefined("qnewRatesnext")>
							    <cfif qnewRatesold.mAmount eq "">
									<cfset newamount = 0>
								<cfelse>
									<cfset newamount = qnewRatesold.mamount>
								</cfif>
								
								<cfset newctype=qnewRatesold.ichargetype_id>
								<cfset newlevel=qnewRatesold.level>
								<cfset newtypedesc=qnewRatesold.chargedesc>
							<cfelse>						
						...
						<cfif activeMonthDiff neq 1 >						   
						   ...
						   <cfset remainder = evaluate(datediff("d",arguments.billingActiveDate,activePeriodEnd)+1)>
		      <!--- Ganga - Added to check dates and current and previous month --->
						<cfset ndate_1 = #dateformat(ndate,"yyyymm")#>
						<cfset ndate_2 = #dateformat(ndate,"yyyymm")#>
						<cfset lastmonth= #dateadd("M", -2, ndate)#> 
						<cfset firstOfThisMonth = createDate(year(now()), month(NOW()), 1)> 
				        <cfset lastOfNextMonth = dateAdd("d", -1, dateAdd("m", 1, firstOfThisMonth))>
				        <cfset totdaysofmonth = DaysInMonth(firstOfThisMonth)>
				        <cfset PrevCurrnetMonth = #dateformat(lastmonth, 'yyyymm')#>
				        <cfset numberOfDaysInMonthprev= DaysInMonth(now())>	 
						
						<cfif #activeMonthDiff# EQ -3>						
							<cfset remainder2 = DaysInMonth(now())>
						</cfif>	
						...
						<cftry>
							...
							<!--- MLAW END--->
							 <cfset oldamountnext = qMonthData1.mAmount>
							<cfset ratediff=(newamount-oldamount)>
							<cfif oldamountnext lt 0>
								<cfset oldamountnext=0>
							</cfif>							
							<cfset ratediffnext=(newamountnext-oldamountnext)>
			...
			<cfif not (#trim(TenantQuery.cBillingType)# eq 'M' and #TenantQuery.iProductline_ID# eq 1 )> <!--- Monthly AL no care charges should generate//added by mamta--->
					<!---Mamta// added to determine if resudent in memory care unit--->
					<cfquery name="isresidentmemorycare" datasource="#variables.dsn#">	
						Select * from assessmenttoolmaster atm join assessmenttooldetail atd 
						on atm.iassessmenttoolmaster_id=atd.iassessmenttoolmaster_ID
						where atm.itenant_ID= #variables.tenantid# 
						and atd.iservicelist_ID in (4027,4141)
						and atd.dtrowdeleted is null
						and atd.service_text='y'
						and atm.iassessmenttoolmaster_ID=#url.assessmentId#
					</cfquery>
				
				<cfif isresidentmemorycare.recordcount eq 0 > <!---cfif start for memorycare--->
				
					<cfif createodbcdate(nDate) eq activePeriodStart> <!---cfif start nadate eq active period--->
								<cfif TenantQuery.level eq 0>
									<cfset oldamount = newamount>
								</cfif>
						
						<cfif oldamount gt 0  <!---cfif start oldamount gt 0 --->
										and 
								...
								<cfif #TenantQuery.level# NEQ #newLevel#>	        <!--- Mamta --cfif start Added for avoid same lvl care LOC --->
								<cfif isDefined("qnewRatesnext")>
								     <cfquery name="qChargesOld" datasource="#variables.dsn#" result="qChargesOld1">
									...
									<cfset ndate_1 = #dateformat(ndate,"yyyymm")#>
											...
											<cfif activeMonthDiff neq 1 AND ndate_1 neq PreviousPeriod > <!---cfif start activeMonthDiff,ndate_1,neq PreviousPeriod--->
												<cfquery name="qChargesOld" datasource="#variables.dsn#" result="qChargesOld2">
													insert into 
								...
								<cfelse>
									...
									<cfset ndate_1 = #dateformat(ndate,"yyyymm")#>
											...
											</cfif>	<!---cfif end activeMonthDiff,ndate_1,neq PreviousPeriod--->
							<!--- Ganga added for current month missing care level when Assessment happen in back dated  --->
									<cfset ndate_1 = #dateformat(ndate,"yyyymm")#>
									
											<cfif activeMonthDiff eq -3 AND ndate_1 neq PreviousPeriod and PreviousPeriod neq PrevCurrnetMonth> <!---cfif start activeMonthDiff,ndate_1,neq PreviousPeriod--->
												<cfquery name="qChargesOld" datasource="#variables.dsn#" result="qChargesOld2">
													insert into 
														InvoiceDetail(iInvoiceMaster_ID
																	 ,iTenant_ID
													...
													values												
														...
														,#variables.tenantid#
														,#ct1657#
														,'#dateformat(now(),"yyyymm")#' <!---'#PrevCurrnetMonth#'/ '#dateformat(now(),"yyyymm")#' --->
														,1
														,#CreateODBCDateTime(now())#
														,#remainder2#
														,'Resident Care Level Change' 
														,#evaluate(ratediff)#
														,'lvl #TenantQuery.level# to lvl #newLevel# - #dateformat(arguments.billingActiveDate,"m/d/yy")#' <!---,'Resident Care Level Change'--->
														,'#AcctStamp.solomonPeriod#'
														,'#arguments.userid#'
								...
								<!--- Ganga added for current month missing care level when Assessment happen in back dated code END --->			
											
											
											
											
											</cfif>   <!--- ciif code of qnewRatesnext end  --->
										</cfif>      <!---mamta cfif end for same LOC--->
								</cfif><!---cfif end oldamount gt 0 --->
		
										<cfif newamount gt 0 and remainder gt 0 and (createodbcdate(arguments.currentPeriod) eq createodbcdate(ndate)) > <!---cfif start for newamount and remainder--->
									 ...
									 <cfquery name="qChargesNew" datasource="#variables.dsn#">
										...
										</cfif><!---cfif ends for newamount and remainder--->
							</cfif><!---cfif ends for ndate eq activeperiod--->
				    </cfif><!---cfif ends for memorycare --->
				</cfif><!---For piniconplace cfif ends--->
...
<!--- 			<cfquery name="qperson" datasource="#dsn#">
			...
			<cfif emailto neq "">
			<!---	<cfmail to="#emailto#" cc="CFDevelopers@enlivant.COM" FROM="AssessmentTool@enlivant.com" SUBJECT="Assessment Approval Request for #session.housename#" type="html">
					<html>
...
<!--- Ganga 06/08/2018  added for review type and moved query from cfm file  --->

<cffunction access="public" name="DCheck" output="false" returntype="query" hint="Gets Tenants diagnosis result from the database.">
...
<cffunction access="public" name="qGettenantid" output="false" returntype="query" hint="Gets Tenants active assessment record.">
	 <cfargument name="datasource" type="string" required="false" default="#application.datasource#">
	 <cfargument name="ASSESSMENTID" type="numeric" required="true" >

        <cfquery name="qGettenantid" datasource="#Application.Datasource#">
            SELECT  iTenant_id 
            FROM	AssessmentToolMaster with (NOLOCK) 
            WHERE iAssessmentToolMaster_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.ASSESSMENTID#">
        </cfquery> 

	     <cfreturn qGettenantid>
</cffunction>

<cffunction access="public" name="getassesst_p" output="false" returntype="query" hint="Gets Tenants active assessment record from P_table from the database.">
	 <cfargument name="datasource" type="string" required="false" default="#application.datasource#">
	 <cfargument name="tenantID" type="numeric" required="true" >
	 
		<cfquery name="getassesst_p" datasource="#Application.Datasource#">
			SELECT count(bBillingActive) as P_bBillingActive_cnt 
			FROM P_AssessmentToolMaster with (NOLOCK)
			WHERE bBillingActive = 1 
			AND iTenant_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.tenantID#">  <!---#URL.tenantId# --->
			--AND dtBillingActive >= (getdate() - 100)
		</cfquery> 

		<cfreturn getassesst_p>
</cffunction> 
 
 <cffunction access="public" name="getassesst" output="false" returntype="query" hint="Gets Tenants active assessment record from the database.">
	 <cfargument name="datasource" type="string" required="false" default="#application.datasource#">
	 <cfargument name="tenantID" type="numeric" required="true" >
	 
		 <cfquery name="getassesst" datasource="#Application.Datasource#">
	        select count(bBillingActive) as bBillingActive_cnt 
	        from AssessmentToolMaster with (NOLOCK)
			where bBillingActive = 1 
			and iTenant_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.tenantID#"> <!--- #URL.tenantId# --->
		 </cfquery>
